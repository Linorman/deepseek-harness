...
const sharedTheme: Pick<DefaultTheme.Config, 'search' | 'socialLinks' | 'editLink'> = {
  ...
  socialLinks: [
    { icon: 'github', link: 'https://github.com/Linorman/clocky' },
  ],
  editLink: {
    pattern: ({ frontmatter }: PageData) => {
      ...
      return `https://github.com/Linorman/clocky/edit/master/${editSource}`
    },
    text: '在 GitHub 上编辑此页',
  },
}
...
const siteIdentity = {
  title: 'Clocky',
  description: '用于构建 Clocky 的插件化 SDK',
}
...
const wordmark = readFileSync(resolve(import.meta.dirname, '../public/wordmark.svg'), 'utf8')
  .trim()
  .replace('<svg ', '<svg class="clocky-wordmark" ')
 ...
 */
const siteStyle = `
.clocky-lockup { display: inline-flex; align-items: center; gap: 8px; min-width: 0; }
.clocky-wordmark { display: block; height: 22px; width: auto; color: var(--vp-c-text-1); }
.clocky-tag {
  display: inline-flex;
  align-items: center;
...
function siteTitle(previewTag: string): string {
  return `<span class="clocky-lockup">${wordmark}<span class="clocky-tag">${previewTag}</span></span>`
}
