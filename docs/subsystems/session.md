...
The four message-producing types (`SurfaceEventType` — `user/message`, `team/channel-view`, `assistant/message`, `tool/result`) carry surface metadata declaring how they join the ordered derived surface. See the [session surface Agent Note](../../.agents/notes/implemented/architecture/2026-06-18-session-surface.md).
...
The `hook/invoked` / `hook/result` pair from `@clocky/clocky-hook-protocol` correlates by `handlerId` for bridge implementations that use it. `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, and `Stop` fire inside the loop's open turn, so their `hook/*` records are turn-enclosed by construction. `SessionStart` gets no `hook/*` record because it runs before turn 1; its context remains pending in the inbox until a waking delivery opens a turn.
