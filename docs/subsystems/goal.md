...
[`GoalService`](../../packages/compat/goal/src/index.ts) resolves creation defaults, folds strict replay from durable `goal/change` events, enforces exact-live-agent identity and compare-and-set mutations, and emits contained `goal/changed` notifications. The package [README](../../packages/compat/goal/README.md) defines the callable API and model-visible contract.
