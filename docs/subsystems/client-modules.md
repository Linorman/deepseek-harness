 ...
 * metadata (the authoritative edges live in each package's `clocky.client`
...
```ts type-equiv
/** The composed client entry graph the host injects as `window.__CLOCKY_BOOT__`. */
