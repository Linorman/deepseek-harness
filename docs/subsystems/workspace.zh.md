...
Session 的 cwd 由受信任创建者赋予，而不是由本注册表赋予。随附的 `team.start` 请求会在激活默认 Team coordinator 之前解析 cwd；TeamRun 将它记录到本地 Participant Session header 中，而不会使用 `WorkspaceRegistry` 创建或 attach 独立的用户可见会话。自定义／内部 Session 创建者可以调用 `attachSession`，它会把已存储的 header cwd 与工作区路径重新校验一遍。首次成功启动时，注册表仅凭已持久化的 header（`id`、`cwd`、`createdAt`——绝不读事件正文）引导历史：把规范 cwd 有效的会话按目录分组为工作区，最新的排在最前；「已初始化」标记最后写入，因此被中断的引导可以安全续跑。引导只发生这一次：没有 cwd 的历史遗留会话保持 Ungrouped，此后创建的会话只能通过 `attachSession` 加入工作区。

## 消费方

[clocky-host-apiproxy](../../packages/host/apiproxy) 是产品消费方：它经 `ctx.workspaceRegistry` 向 GUI 客户端提供工作区 CRUD，而 `team.start` 通过 TeamRun 创建用户工作，不走「创建会话再 attach」流程。[clocky-agent-instructions](../../packages/context/agent-instructions) 尽管名字如此，却**不是**消费方：它在 agent 自己的 cwd 下发现 AGENTS.md 风格的指令文件，从不触碰 `ctx.workspaceRegistry`——两者共用的这个词指的是用户的工作目录，而非本注册表的实体。
