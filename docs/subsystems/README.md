...
| [subprocess.md](subprocess.md) | the subprocess seam: fully-explicit `SubprocessSpawnSpec`, offset-based output readers, unclassified `SubprocessOutcome`, and the managed `CLOCKY_*` environment vocabulary |
...
| [agent-runtime.md](agent-runtime.md) | Participant-bound Agent activation providers, activation handles, and `ctx.agentRuntimes` registry behavior |
| [team.md](team.md) | the Team work-system Service Definition: independent identities, activation bindings, authenticated Envelope admission and receipts, versioned channel adapters, policy waterfalls, and post-commit Team/channel notifications |
| [team-link.md](team-link.md) | activation-bound local or remote Team Links, provider registration, authenticated operations, notification ownership, and closure |
| [team-workspace.md](team-workspace.md) | Team task workspace mode provider registration, eligibility, allocations, and release ownership |
| [team-artifact.md](team-artifact.md) | Provider-independent Team artifact references, content-addressed storage, and verified reads |
...
| [client-modules.md](client-modules.md) | the web plugin table: `clocky.client` declarations, `WebBootGraph` wire composition, the bundle route and index tap |
