...
- An OpenAI-compatible API endpoint and credential for the provider profile you choose
...
```sh
git clone https://github.com/Linorman/clocky.git
cd clocky
python -m venv .venv
. .venv/bin/activate
...
The checked-in composition uses `TEST_API_KEY` and a local placeholder `baseURL`; set the key and edit `minimal.cordis.yml` to point at the provider endpoint. For another provider, edit the profile and set the environment variable that profile names.

```sh
export TEST_API_KEY=your-key-here
# export CLOCKY_PROVIDER=test-provider
# export CLOCKY_MODEL=test-model
# export CLOCKY_SYSTEM_PROMPT='You are a helpful software engineer assistant.'
```
...
| Session persistence | Uncompressed JSONL under `CLOCKY_SESSION_ROOT` |
