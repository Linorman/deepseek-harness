...
- **Service Definition** (`clocky-shell`) — defines the Cordis service and Bash request and result types
- **Service Provider** (`clocky-bash-local`) — executes commands on the local machine
- **Consumer** (`clocky-tool-bash`) — exposes the capability as a model-callable tool

```
┌─────────────┐     ┌──────────────────┐     ┌──────────────┐
│  clocky-shell   │────▶│  clocky-bash-local  │     │ clocky-tool-bash│
...
import { MyCapService, type MyCapRequest, type MyCapResult } from '@clocky/clocky-my-cap'
...
```yaml
- name: '@clocky/clocky-my-cap-local'
- name: '@clocky/clocky-tool-my-cap'
```
