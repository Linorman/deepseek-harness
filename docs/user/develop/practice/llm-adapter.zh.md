...
`stream()` 接收仓库导出的 `GenerateOptions`。它包含模型、适配器拥有的推理强度 ID、对话历史、系统提示词、工具 schema、生成参数、停止序列和中止信号；完整字段以 `@clocky/clocky-llm` 导出的 TypeScript 类型为准。适配器必须将支持的字段映射到具体 API；如果无法支持某个字段，应抛出带稳定 code 的 `LlmError`，不得静默丢弃。
...
仓库中包含一个完整实现：`packages/llm/llm-pi-ai/`，即由维护中的 LLM 库驱动的、与提供方无关的适配器。提供方专属行为通过其中配置的路由 profile 选择，而不是通过独立的一方适配器包实现。
