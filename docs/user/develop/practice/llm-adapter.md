...
```ts
import { CallId, type StreamChunk } from '@clocky/clocky-llm'
...
The repository contains one complete implementation: `packages/llm/llm-pi-ai/`, a provider-neutral adapter backed by a maintained LLM library. Provider-specific behavior is selected through its configured route profiles rather than a separate first-party adapter package.
