...
Because unloading releases effects ([chapter 2](02-lifecycle-and-effects.md)) and loading follows dependencies ([chapter 3](03-services.md)), HMR can replace a running plugin by unloading and loading it. The `@clocky/cordis-plugin-hmr` plugin watches your files and does exactly that on save.

In `tmp/cordis-tutorial`, write `cordis.yml`:

```yaml
- id: logger
  name: '@clocky/cordis-plugin-logger-console'
- id: timer
  name: '@clocky/cordis-plugin-timer'
- id: hmr
  name: '@clocky/cordis-plugin-hmr'
  config:
    root: ['.']
- id: hello
  name: './hello.ts'
...
`inject: ['timer']` has no provider. Add `- name: '@clocky/cordis-plugin-timer'` to the list and the plugin loads. When a plugin does nothing and reports nothing, inspect its fiber state. Iterating without the PENDING filter also shows the loader's own plugins (Loader, Include) as ACTIVE fibers because plugins mount the config file itself.

Next: [Into the harness](07-into-the-harness.md) — the same patterns against real harness services.

![Agent harness robot badge](../assets/brand-placeholder-badge.svg)
