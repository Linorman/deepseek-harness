...
async function freshRoot(prefix = 'clocky-jsonl-zstd-'): Promise<string> {
...
runPersistenceContract('jsonl-zstd', async () => {
  const root = await mkdtemp(join(tmpdir(), 'clocky-jsonl-zstd-contract-'))
  const ctx = new Context()
  await ctx.plugin(SessionStore)
...
runCoordinatorContract('jsonl-zstd', async (): Promise<CoordinatorFixture> => {
  const root = await mkdtemp(join(tmpdir(), 'clocky-jsonl-zstd-coordinator-'))
  return {
...
describe('JsonlSessionPersistence: encoding selection', () => {
  it('rejects roots owned by the opposite encoding in both directions', async () => {
    const rawRoot = await freshRoot('clocky-jsonl-raw-mismatch-')
    ...
    const zstdRoot = await freshRoot('clocky-jsonl-zstd-mismatch-')
