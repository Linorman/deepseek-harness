...
import * as SessionStatsPlugin from '@clocky/clocky-session-stats'
...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'clocky-session-stats-loader-'))
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@clocky/clocky-session-projection', SessionProjectionRegistry],
    ['@clocky/clocky-session-stats', SessionStatsPlugin],
  ])
...
describe('real Loader composition', () => {
  it('loads the shipped session-stats YAML shape and serves whole-log counts', async () => {
    const loaded = await loadYaml([
      ...
      "- name: '@clocky/clocky-session-projection'",
