# @clocky/clocky-sdk-jsonrpc-demo

English | [中文](README.zh.md)

Bin-only app that boots an external `cordis.yml`; its [`jsonrpc`](../../sdk/server/README.md) entry serves SDK clients over newline-delimited stdio. A serving config mounts the local Team stack through `team-run` as well as the spine, persistence, and provider adapters. The published `clocky-jsonrpc-agent` bin resolves bare plugins from the configuration project. The Python SDK's `clocky-jsonrpc-agent-pkg` [single-executable runtime](../../../.agents/notes/implemented/architecture/2026-07-10-single-file-executable-sdk-runtime-distribution.md) uses `lib/packaged-bin.js` instead: packaged bare plugins resolve from its closed runtime tree, while relative plugins remain configuration-relative.

## Config discovery

The first non-empty channel wins: `$CLOCKY_CORDIS_CONFIG`, then positional `argv[2]`. If neither names an existing file, the bin prints one-line usage to stderr and exits 1; there is no working-directory or built-in fallback. [`clocky-app-boot`](../../boot/app-boot/README.md) makes plugin load failures fatal. This protocol does not use `CLOCKY_SNAPSHOT`.

A config without `clocky-sdk-jsonrpc-server` is valid and serves nothing; the bin does not designate a server plugin.
...
- **The bin cannot prove that the config serves JSON-RPC** — a valid config with no `clocky-sdk-jsonrpc-server` entry boots successfully and serves nothing.
- **No built-in or default config exists** — every launch must provide `CLOCKY_CORDIS_CONFIG` or a positional path, and deployment owns the complete plugin tree and stdout discipline.
