...
async function makeConsumer(): Promise<string> {
  ...
  for (const rel of clockyPackages) {
...
describe.skipIf(!existsSync(acpBin))('clocky-acp-demo BUILT bin (node lib/bin.js, no tsx)', () => {
  it('boots the published bin, completes a turn, and writes default Zstandard persistence', async () => {
    ...
    child = spawn(process.execPath, [acpBin, '--config', './cordis.yml'], {
      ...
      env: {
        ...process.env,
        CLOCKY_HOME: join(consumer, '.clocky'),
        CLOCKY_AGENTS_HOME: join(consumer, '.agents'),
      },
