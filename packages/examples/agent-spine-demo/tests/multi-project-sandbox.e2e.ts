...
async function projectDir(label: string): Promise<string> {
  const dir = await mkdtemp(join(homedir(), `clocky-${label}-`))
  tempDirs.push(dir)
  return dir
}
