...
import { renderPrompt, TOOL_ORDER_REST } from '@clocky/clocky-system-prompt'
...
/**
 * Unit coverage for the @clocky/clocky-agent-spine-demo bundle: mounting it brings
...
async function mount(config: agentCore.Config, withBash = false): Promise<Context> {
  ...
  process.env.CLOCKY_HOME = await mkdtemp(join(tmpdir(), 'clocky-agent-spine-demo-home-'))
  process.env.CLOCKY_AGENTS_HOME = await mkdtemp(join(tmpdir(), 'clocky-agent-spine-demo-agents-'))
...
describe('clocky-agent-spine-demo bundle', () => {
  ...
  it('mounts package companions and forwards invariant selection config', async () => {
    ...
    for (const invariants of [
      { enabled: false },
      { package_allowlist: ['^@clocky/clocky-agent$'] },
      { package_blocklist: ['^@clocky/clocky-session$'] },
    ]) {
  ...
  it('loads workspace instructions into requests through the bundled spine', async () => {
    const root = await mkdtemp(join(tmpdir(), 'clocky-agent-spine-demo-workspace-context-'))
    try {
      ...
      expect(adapter.requests[0]?.system).toContain('You are an AI agent running in a plugin-based harness.')
  ...
  it('forwards agent-instructions config to the bundled loader', async () => {
    const root = await mkdtemp(join(tmpdir(), 'clocky-agent-spine-demo-workspace-context-disabled-'))
    try {
      await mkdir(join(root, '.git'), { recursive: true })
  ...
  it('forwards skill config to the registry, local provider, and model-facing consumer', async () => {
    const home = await mkdtemp(join(tmpdir(), 'clocky-agent-spine-demo-skill-home-'))
    const agentsHome = await mkdtemp(join(tmpdir(), 'clocky-agent-spine-demo-skill-agents-'))
    const custom = await mkdtemp(join(tmpdir(), 'clocky-agent-spine-demo-skill-custom-'))
  ...
  it('snapshots a created project skill through catalog refresh and progressive loading', { timeout: 15_000 }, async () => {
    const root = await mkdtemp(join(tmpdir(), 'clocky-agent-spine-demo-skill-refresh-'))
    const home = await mkdtemp(join(tmpdir(), 'clocky-agent-spine-demo-skill-refresh-home-'))
    try {
      await mkdir(join(root, '.git'), { recursive: true })
  ...
  it('shares top-level clockyHome between local skills and the managed bash environment', async () => {
    const home = await mkdtemp(join(tmpdir(), 'clocky-agent-core-shared-home-'))
    const agentsHome = await mkdtemp(join(tmpdir(), 'clocky-agent-core-shared-agents-'))
    ...
    const execution: ToolExecution = {
      ...
      token: Symbol('agent-core-clocky-home-test') as ToolExecution['token'],
      callId: CallId('agent-core-clocky-home'),
      rootCallId: CallId('agent-core-clocky-home'),
      name: 'bash',
      arguments: { command: 'true' },
    }
    expect(ctx.shellEnv.collect(execution)).toMatchObject({ CLOCKY_HOME: home, CLOCKY_SHELL: '1' })
    await ctx.fiber.dispose()
  })

  it('rejects conflicting global and nested Clocky home directories', () => {
    expect(() => {
      agentCore.apply(new Context(), {
        clockyHome: '/global-clocky-home',
        workspaceContext: false,
        skills: { filesystem: { clockyHome: '/nested-clocky-home' } },
      })
    }).toThrow('agent-spine-demo: clockyHome and skills.filesystem.clockyHome must resolve to the same directory')
  })

  it('delivers workspace instructions ahead of the first-step skill catalog', async () => {
    const root = await mkdtemp(join(tmpdir(), 'clocky-agent-spine-demo-prefix-order-'))
    try {
      await mkdir(join(root, '.git'), { recursive: true })
  ...
  it('picks shared spine config without leaking entry-point fields', () => {
    ...
    expect(agentCore.pickSpineConfig(appConfig)).toEqual({
      ...
      clockyHome: appConfig.clockyHome,
