 ...
 *
 * @module @clocky/clocky-pwsh-local
 */

/* jscpd:ignore-start -- this executor mirrors clocky-bash-local call-for-call by
   design (see this package's README), so the two import the same seam surface */
import { Context } from '@clocky/cordis'
import z from '@clocky/schemastery'
import { SHELL_SETTINGS_NAMESPACE, ShellExecutor } from '@clocky/clocky-shell'
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellProcessRead, ShellRunResult, CollectedOutput } from '@clocky/clocky-shell'
import type { SubprocessCollect, SubprocessHandle, SubprocessOutputReader, SubprocessSpawnSpec } from '@clocky/clocky-subprocess'
import { installSettingsSection } from '@clocky/clocky-settings'
import { clampTimeout, deadline, MAX_TIMER_DELAY_MS, timeoutOf } from '@clocky/clocky-timeout'
/* jscpd:ignore-end */
import { resolvePwshPath } from './resolve.ts'

/* jscpd:ignore-start -- deliberate call-for-call mirror of clocky-bash-local (Agent Note: pwsh-tool-and-executor). */
/**
...
export class PwshLocalExecutor extends ShellExecutor {
   ...
   * `clocky-bash-local`'s `runArgv`/`startArgv` hooks; see
  ...
  private spawnSpec(
    ...
    return {
      ...
      signal,
      env: { ...ENV_OVERRIDES, ...spec.env, ...spec.clockyEnv },
    }
  }
   ...
   * `clocky-bash-local` (whose sandboxing subclass consumes the same hook); the
   * pwsh-confining consumer is `@clocky/clocky-pwsh-sandbox`.
