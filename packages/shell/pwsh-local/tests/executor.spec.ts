/**
 * Real-process tests for `@clocky/clocky-pwsh-local`: the LOCAL subprocess
...
import { PwshLocalExecutor, ENCODING_PREAMBLE, candidatePwshPaths, resolvePwshPath } from '@clocky/clocky-pwsh-local'
import LocalSubprocessRuntime from '@clocky/clocky-subprocess-local'
import SubprocessRuntime from '@clocky/clocky-subprocess'
import type { SubprocessHandle, SubprocessOutputReader, SubprocessSpawnSpec } from '@clocky/clocky-subprocess'
import { MAX_TIMER_DELAY_MS } from '@clocky/clocky-timeout'
import type { ShellProcess } from '@clocky/clocky-shell'

const spillDir = mkdtempSync(join(tmpdir(), 'clocky-pwsh-exec-spec-'))
...
describe('resolvePwshPath and candidatePwshPaths (pure, every platform)', () => {
  ...
  it('returns the first EXISTING win32 candidate, else pwsh', () => {
    const dir = mkdtempSync(join(tmpdir(), 'clocky-pwsh-resolve-'))
  ...
  it('accepts a link-shaped PATH candidate whose target cannot be stat-ed', () => {
    ...
    const dir = mkdtempSync(join(tmpdir(), 'clocky-pwsh-resolve-link-'))
    const store = join(dir, 'store')
    mkdirSync(store, { recursive: true })
  ...
  it('skips a directory candidate and falls through to the PATH-resolution default', () => {
    const dir = mkdtempSync(join(tmpdir(), 'clocky-pwsh-resolve-dir-'))
    const store = join(dir, 'store')
...
describe.skipIf(!hasPwsh)('PwshLocalExecutor.run', () => {
  ...
  it('uses config cwd, overridable per call', async () => {
    const first = mkdtempSync(join(tmpdir(), 'clocky-pwsh-cwd-a-'))
    const second = mkdtempSync(join(tmpdir(), 'clocky-pwsh-cwd-b-'))
  ...
  it('rejects on spawn failure (bad workdir)', async () => {
    const { bash } = await setup()
    await expect(bash.run(bash.resolve({ command: 'Write-Output ok', workdir: '/nonexistent-clocky' }))).rejects.toThrow(/ENOENT/)
  })

  it('resolve() carries stdin/env/clockyEnv onto the spec, and run() threads them to the command', async () => {
    const { bash } = await setup()
    const spec = bash.resolve({
      command: '$s = ([Console]::In.ReadToEnd()).TrimEnd(); Write-Output $s; Write-Output "[$env:SEAM_VAR][$env:CLOCKY_SEAM_VAR]"',
      stdin: 'piped\n',
      env: { SEAM_VAR: 'env-ok' },
      clockyEnv: { CLOCKY_SEAM_VAR: 'clocky-ok' },
    })
    ...
    expect(spec.clockyEnv).toEqual({ CLOCKY_SEAM_VAR: 'clocky-ok' })
    const result = await bash.run(spec)
    expect(lf(result.stdout.text)).toBe('piped\n[env-ok][clocky-ok]\n')
  })

  it('resolve() omits stdin/env/clockyEnv when the request supplies none', async () => {
    const { bash } = await setup()
    const spec = bash.resolve({ command: 'Write-Output ok' })
    expect('stdin' in spec).toBe(false)
    expect('env' in spec).toBe(false)
    expect('clockyEnv' in spec).toBe(false)
  })
})
...
describe.skipIf(!hasPwsh)('PwshLocalExecutor.start (background process handles)', () => {
  ...
  it('threads stdin and extra env into a background process', async () => {
    ...
    const proc = bash.start(bash.resolve({
      command: '$s = ([Console]::In.ReadToEnd()).TrimEnd(); Write-Output $s; Write-Output "[$env:BG_VAR][$env:CLOCKY_BG_VAR]"',
      stdin: 'bg-stdin\n',
      env: { BG_VAR: 'bg-env' },
      clockyEnv: { CLOCKY_BG_VAR: 'bg-clocky-env' },
    }))
    const partialOutput = await readUntil(proc, '[bg-env][bg-clocky-env]')
    await proc.done
    const output = partialOutput + lf(proc.readOutput().delta)
    expect(output).toBe('bg-stdin\n[bg-env][bg-clocky-env]\n')
    expect(proc.exitCode).toBe(0)
  })
  ...
  it('a background spawn failure settles as killed with the error readable on stderr', async () => {
    const { bash } = await setup()
    const proc = bash.start(bash.resolve({ command: 'Write-Output ok', workdir: '/nonexistent-clocky' }))
