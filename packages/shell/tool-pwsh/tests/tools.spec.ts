 ...
 * managed `CLOCKY_*` collection, abort translation, canonical result projection,
...
import SystemPrompt, { renderPrompt } from '@clocky/clocky-system-prompt'
...
import type { ShellExecRequest, ShellExecSpec, ShellProcess, ShellRunResult } from '@clocky/clocky-shell'
import SandboxPolicyService from '@clocky/clocky-sandbox-policy'
import * as ToolPwsh from '@clocky/clocky-tool-pwsh'
import * as BashEnvPlugin from '@clocky/clocky-shell-env'
import type { ShellProcessRead } from '@clocky/clocky-shell'
...
async function setup(toolConfig: Partial<ToolPwsh.Config> = {}, clockyHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
  await ctx.plugin(BashEnvPlugin, clockyHome === undefined ? {} : { clockyHome })
...
async function setupWithTasks(toolConfig: Partial<ToolPwsh.Config> = {}, clockyHome?: string) {
  const ctx = new Context()
  await ctx.plugin(SystemPrompt)
  await ctx.plugin(ToolRuntime)
  await ctx.plugin(AgentRegistry)
...
describe('execution through the bash seam', () => {
  it('forwards command, session cwd, timeout, and managed CLOCKY_* environment', async () => {
    const clockyHome = mkdtempSync(join(tmpdir(), 'clocky-tool-pwsh-home-'))
    const { ctx, bash } = await setup({}, clockyHome)
    ...
    expect(request?.clockyEnv).toEqual({
      CLOCKY_HOME: clockyHome,
      CLOCKY_SHELL: '1',
      CLOCKY_SESSION_ID: 'session-1',
    })
  ...
  it('omits workdir and the session id without an agent, so executor defaulting applies', async () => {
    ...
    const clockyEnv = bash.requests[0]?.clockyEnv
    expect(clockyEnv).toBeDefined()
    expect(clockyEnv?.['CLOCKY_SHELL']).toBe('1')
    expect(clockyEnv?.['CLOCKY_HOME']).toEqual(expect.any(String))
    expect(clockyEnv).not.toHaveProperty('CLOCKY_SESSION_ID')
  })
...
describe('per-call sandbox policy resolution', () => {
  it('stamps the CALLING SESSION\'s resolved policy onto the request (session cwd, not the server launch dir)', async () => {
    ...
    const sessionCwd = mkdtempSync(join(tmpdir(), 'clocky-tool-pwsh-policy-'))
