/**
 * Integration tests: the REAL `@clocky/clocky-pwsh-local` executor plus the
...
import ToolRuntime, { TOOL_ABORTED } from '@clocky/clocky-tools'
...
import { PwshLocalExecutor, resolvePwshPath } from '@clocky/clocky-pwsh-local'
...
describe.skipIf(!hasPwsh)('pwsh tool over the real pwsh executor', () => {
  beforeEach(async () => {
    dir = await mkdtemp(join(tmpdir(), 'clocky-tool-pwsh-'))
