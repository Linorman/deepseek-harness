 ...
 * `@clocky/clocky-pwsh-local`) backs `ctx.shell`; the tool contract is
 * PowerShell-dialect: native `C:\...` paths and `$env:NAME` variables.
 *
 * Behavior mirrors `clocky-tool-bash` call-for-call: foreground and
 * `run_in_background` execution (background handles register with the
 * generic `ctx.jobs` runtime), the managed `CLOCKY_*` environment through the
 ...
 * exit-status parse from `@clocky/clocky-shell`.
 *
 * @module @clocky/clocky-tool-pwsh
 */
...
import type { SandboxExecutionPolicy, SandboxMode } from '@clocky/clocky-sandbox'
import { ESCALATION_TARGETS, approveEscalation, validateEscalationArgs } from '@clocky/clocky-sandbox'
import type { SandboxPolicyService } from '@clocky/clocky-sandbox-policy'
import type { ShellRunResult } from '@clocky/clocky-shell'
import { parseExitStatus } from '@clocky/clocky-shell'
...
/* jscpd:ignore-start -- minimal mirror of clocky-tool-bash's validation and execute plumbing (Agent Note). */
...
function pwshDescription(backgroundEnabled: boolean, escalationModes: readonly SandboxMode[]): string {
  ...
  const base = 'Execute a PowerShell command (`pwsh -Command`) and return its stdout/stderr. '
    ...
    + 'Current harness environment facts are exposed through managed `$env:CLOCKY_*` variables; inspect them when needed. '
...
function canonicalPwshResult(result: ShellRunResult): PwshForegroundResult {
  ...
  return {
    ...
    /* jscpd:ignore-start -- the canonical projection and background-handle shape mirror clocky-tool-bash's by design (Agent Note). */
...
/* jscpd:ignore-start -- deliberate mirror of clocky-tool-bash's apply() preamble (pwsh-tool-and-executor Agent Note). */
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.tools.register(defineTool({
    ...
    /* jscpd:ignore-start -- deliberate mirror of clocky-tool-bash's parameter surface (pwsh-tool-and-executor Agent Note). */
    parameters: {
    ...
    /* jscpd:ignore-end */
    output: {
      // The foreground result wire shape mirrors clocky-tool-bash's by contract —
      // consumers of one must accept the other (see the pwsh-tool-and-executor
      // Agent Note).
      /* jscpd:ignore-start -- deliberate result-schema symmetry with clocky-tool-bash. */
      schema: {
        oneOf: [
          {
            type: 'object',
    ...
    /* jscpd:ignore-start -- the execute path mirrors clocky-tool-bash's by design (see the pwsh-tool-and-executor Agent Note). */
    async execute(args: PwshToolArgs, exec) {
      ...
      const request = {
        ...
        clockyEnv: ctx.shellEnv.collect(exec),
      ...
      if (args.run_in_background === true) {
        ...
        if (jobs === undefined) {
          throw new Error('background jobs unavailable: load @clocky/clocky-jobs and @clocky/clocky-tool-jobs')
        }
