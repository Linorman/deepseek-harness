...
suite('persistent Bash through a real cordis.yml Loader composition', () => {
  it('preserves cwd and environment across calls', async () => {
    root = await mkdtemp(join(tmpdir(), 'clocky-persistent-bash-loader-'))
    ...
    await writeFile(configPath, [
      ...
      '    timeoutMs: 2000',
      '    disposeGraceMs: 500',
      "- name: '@clocky/clocky-tool-bash-persistent'",
      '  config:',
      '    timeoutMs: 5000',
      '',
    ].join('\n'))
    ...
    const modules = new Map<string, unknown>([
      ...
      ['@clocky/clocky-tool-bash-persistent', ToolBashPersistent],
    ])
    ...
    expect(observed).not.toContain('CLOCKY_PERSISTENT_BASH')
    ...
    expect(multiline).not.toContain('CLOCKY_PERSISTENT_BASH')
