 ...
 * trusted, per-execution `CLOCKY_*` variables consumed by the model-facing shell
 * tools (`clocky-tool-bash`, `clocky-tool-pwsh`). Built-in shell facts are owned by
 * the registry itself while plugins can register additional, enumerable facts
 * with effect-scoped disposal.
 *
 * @module @clocky/clocky-shell-env
 */

import { Service, type Context } from '@clocky/cordis'
import z from '@clocky/schemastery'
import { CLOCKY_ENV_PREFIX } from '@clocky/clocky-shell'
import type { ClockyEnvironment, ClockyEnvironmentKey } from '@clocky/clocky-shell'
import { CLOCKY_HOME_ENV, resolveClockyHome } from '@clocky/clocky-home-paths'
...
/** Model-visible metadata for one managed `CLOCKY_*` environment variable. */
...
export interface BashEnvContributor {
  ...
  name: string
  /** Complete set of `CLOCKY_*` keys this contributor may return. */
...
export interface BashEnvVariableInfo extends BashEnvVariable {
  ...
  contributor: string
  /** Declared `CLOCKY_*` environment variable name. */
  key: ClockyEnvironmentKey
}

const CLOCKY_SHELL_KEY = `${CLOCKY_ENV_PREFIX}SHELL` as const
const CLOCKY_SESSION_ID_KEY = `${CLOCKY_ENV_PREFIX}SESSION_ID` as const
const CLOCKY_SESSION_JSONL_KEY = `${CLOCKY_ENV_PREFIX}SESSION_JSONL` as const
const RESERVED_BASH_ENV_KEYS = new Set<ClockyEnvironmentKey>([
  CLOCKY_HOME_ENV,
  CLOCKY_SHELL_KEY,
  CLOCKY_SESSION_ID_KEY,
])
const BASH_ENV_KEY_SUFFIX = /^[A-Z][A-Z0-9_]*$/

/**
 * Registry (`ctx.shellEnv`) for trusted, per-execution `CLOCKY_*` variables.
 * The namespace is rebuilt for every model shell call: ambient `CLOCKY_*` values
...
export class ShellEnvRegistry extends Service {
  ...
  private readonly keyOwners = new Map<ClockyEnvironmentKey, string>()
  private readonly clockyHome: string
  ...
  constructor(ctx: Context, config: Config = {}) {
    super(ctx, 'shellEnv')
    this.clockyHome = resolveClockyHome(config.clockyHome)
  }
  ...
  register(contributor: BashEnvContributor): () => void {
    const dispose = this.ctx.effect(function* (this: ShellEnvRegistry) {
      ...
      const variables = Object.entries(contributor.variables) as [ClockyEnvironmentKey, BashEnvVariable][]
      for (const [key, variable] of variables) {
        if (!key.startsWith(CLOCKY_ENV_PREFIX)
          || !BASH_ENV_KEY_SUFFIX.test(key.slice(CLOCKY_ENV_PREFIX.length))) {
  ...
  collect(execution: ToolExecution): ClockyEnvironment {
    const values: Record<ClockyEnvironmentKey, string> = {
      [CLOCKY_HOME_ENV]: this.clockyHome,
      [CLOCKY_SHELL_KEY]: '1',
    }
    if (execution.agent !== undefined) {
      values[CLOCKY_SESSION_ID_KEY] = execution.agent.session.header.id
    }
    ...
    for (const contributor of [...this.contributors.values()].sort((left, right) => left.name.localeCompare(right.name))) {
      ...
      for (const [rawKey, value] of Object.entries(resolved)) {
        const key = rawKey as ClockyEnvironmentKey
  ...
  list(): BashEnvVariableInfo[] {
    return [...this.contributors.values()]
      .flatMap(contributor => Object.entries(contributor.variables).map(([key, variable]) => ({
        ...
        key: key as ClockyEnvironmentKey,
      })))
 ...
 * shell-agnostic persistence contributor (`CLOCKY_SESSION_JSONL`).
 * @param ctx - Cordis context that owns the service and registrations.
 * @param config - home-directory configuration for the built-in variables.
 */
export function apply(ctx: Context, config: Config = {}): void {
  ...
  registry.register({
    ...
    resolve(execution) {
      ...
      return location?.kind === 'jsonl' ? { [CLOCKY_SESSION_JSONL_KEY]: location.path } : {}
    },
  })
}
