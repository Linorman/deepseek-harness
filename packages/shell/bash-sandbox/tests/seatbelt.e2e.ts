...
import { seatbeltProfileArgs } from '@clocky/clocky-sandbox-local/src/profiles.ts'
 ...
 * confinement is covered by `@clocky/clocky-sandbox-local`. Skips off macOS or when
...
async function tempDir(base: string): Promise<string> {
  const dir = await mkdtemp(join(base, 'clocky-seatbelt-e2e-'))
  tempDirs.push(dir)
  return dir
}
...
describe.skipIf(!seatbeltUsable)('bash-sandbox: real Seatbelt confinement through ctx.shell', () => {
  ...
  it('evaluates BASH_ENV only after Seatbelt confines the inner Bash', async () => {
    ...
    await writeFile(hook, [
      'printf hook > "$CLOCKY_BASH_ENV_INSIDE"',
      'printf escaped > "$CLOCKY_BASH_ENV_OUTSIDE"',
      '',
    ].join('\n'))
    const bash = await sandboxedBash(workdir, 'workspace-write')
    ...
    await bash.run(bash.resolve({
      ...
      env: { BASH_ENV: hook },
      clockyEnv: {
        CLOCKY_BASH_ENV_INSIDE: insideProbe,
        CLOCKY_BASH_ENV_OUTSIDE: outsideProbe,
      },
    }))
