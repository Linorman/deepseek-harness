...
import type { ShellRunResult, CollectedOutput } from '@clocky/clocky-shell'
import { SANDBOX_UNAVAILABLE, SandboxProvider, SandboxUnavailableError } from '@clocky/clocky-sandbox'
import type { ConfinedArgv, SandboxExecutionPolicy, SandboxMode, SandboxPolicy } from '@clocky/clocky-sandbox'
import { SandboxPolicyService } from '@clocky/clocky-sandbox-policy'
import { SandboxBashExecutor } from '@clocky/clocky-bash-sandbox'
...
import type { Config } from '@clocky/clocky-bash-sandbox'

const spillDir = mkdtempSync(join(tmpdir(), 'clocky-bash-sandbox-spec-'))
...
describe('the provider hand-off', () => {
  ...
  it('hands the provider\'s returned argv directly to ctx.subprocess.spawn', async () => {
    const returnedArgv = ['env', 'CLOCKY_WRAP=1', 'bash', '-c', 'printf "%s" "$CLOCKY_WRAP"']
    ...
    const result = await bash.run(bash.resolve({ command: 'printf "%s" "$CLOCKY_WRAP"' }))
  ...
  it('starts a non-Bash runner before the confined inner Bash evaluates BASH_ENV', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'clocky-bash-env-order-'))
    ...
    const runnerScript = [
      ...
      'appendFileSync(process.env.CLOCKY_ORDER_FILE, "runner\\n");',
    ...
    try {
      const result = await bash.run(bash.resolve({
        command: 'true',
        env: { BASH_ENV: hook },
        clockyEnv: { CLOCKY_ORDER_FILE: order },
      }))
...
describe('fail closed', () => {
  ...
  it.each(RUNNER_FORMS)(
    ...
    async (_form, runner) => {
      ...
      const parent = mkdtempSync(join(tmpdir(), 'clocky-sandbox-missing-cwd-'))
      try {
...
describe('result facts', () => {
  ...
  it('reports a real permission failure as a sandbox denial with the mode it ran under', async () => {
    const { bash } = await setup()
    const lockedDir = join(mkdtempSync(join(tmpdir(), 'clocky-sandbox-denied-')), 'locked')
    mkdirSync(lockedDir)
