...
import TerminalSessionService, { TerminalSessionId } from '@clocky/clocky-terminal'
import { BashTerminalBackend } from '@clocky/clocky-terminal-bash'
...
describe.skipIf(!process.env.E2B_API_KEY)('E2B live Loader composition', () => {
  it('scrubs credentials before actual E2B command and PTY login shells', async () => {
    ...
    const sandbox = await Sandbox.create({
      apiKey,
      envs: { NPM_TOKEN: 'sentinel-secret', CLOCKY_STALE: 'sentinel-stale', KEEP: 'visible' },
      timeoutMs: 60_000,
      secure: true,
      lifecycle: { onTimeout: 'kill' },
    })
    try {
      const profileLeakPath = '/home/user/clocky-e2b-bootstrap-profile-leak'
      ...
      const environmentProbe = ctx.subprocess.spawn({
        argv: ['/bin/bash', '-c', [
          'clocky_leak=0',
          'for clocky_pid in "$PPID" $(ps -o pid= --ppid "$PPID"); do',
          '  [[ "$clocky_pid" == "$$" ]] && continue',
          '  if tr "\\0" "\\n" < "/proc/$clocky_pid/environ" 2>/dev/null | grep -Fqx "NPM_TOKEN=sentinel-secret"; then clocky_leak=1; fi',
          'done',
          'printf "DIRECT=<%s> LEAK=<%s>\\n" "${NPM_TOKEN-}" "$clocky_leak"',
        ].join('\n')],
        cwd: '/home/user',
      ...
      const result = await session.startSend({
        text: "printf 'NPM=<%s> Clocky=<%s> KEEP=<%s>\\n' \"$NPM_TOKEN\" \"$CLOCKY_STALE\" \"$KEEP\"",
        submit: true,
      }).done
      expect(result.viewport).toContain('NPM=<> Clocky=<> KEEP=<visible>')
  ...
  it('runs FS, Bash, PTY, and LSP in one sandbox and deletes it', async () => {
    const { stdout, stderr } = await runLoaderSmoke({
      ...
      tempDirPrefix: 'clocky-e2b-composition-',
      binScript,
    ...
    expect(terminalMotd).not.toContain('.clocky-e2b/terminals/')
