...
import { FsTargetKey, FsVersion } from '@clocky/clocky-fs'
import E2BFileSystem from '@clocky/clocky-fs-e2b'
  ...
  readonly sandbox = {
    ...
    commands: {
      run: async (
        ...
        expect(home).toMatch(/^\/\.clocky-e2b-control-/)
...
describe('E2BFileSystem atomic writes and edits', () => {
  it('creates owner-only files and returns metadata after the committed move', async () => {
    ...
    expect(remote.nodes.get('/workspace/new.txt')?.metadata?.['clocky-version']).toBeDefined()
