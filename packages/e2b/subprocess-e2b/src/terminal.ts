...
const TERMINAL_RUNNER_SOURCE = [
  ...
  'set -euo pipefail',
  'clocky_state=$1',
  'mapfile -d \'\' -t clocky_env < "$clocky_state/environment"',
  'mapfile -d \'\' -t clocky_argv < "$clocky_state/argv"',
  'clocky_output_marker=$(<"$clocky_state/output-marker")',
  'rm -f -- "$clocky_state/environment" "$clocky_state/argv" "$clocky_state/output-marker" "$clocky_state/runner.bash"',
  'if (( ${#clocky_argv[@]} == 0 )); then',
  "  printf 'terminal runner received empty argv\\n' >&2",
  '  exit 125',
  'fi',
  'printf \'%s\' "$clocky_output_marker"',
  'exec env -i -- "${clocky_env[@]}" "${clocky_argv[@]}"',
  '',
].join('\n')
...
export async function spawnE2BTerminal(
  ...
  const outputMarker = Buffer.from(`clocky-e2b-bootstrap:${randomUUID()}`)
