...
describe('web-app runtime glue', () => {
  it('mounts dist serving, prompt section, bash variables, and publishes the URL with the LAN snapshot', async () => {
    ...
    ctx.provide(CLOCKY_LAUNCH_ENVIRONMENT_KEY, createLaunchEnvironmentSnapshot([
    ...
    expect(assembly.sections.find(entry => entry.name === 'harness:source')?.text).toContain('Clocky implementation checkout')
    ...
    expect(webRuntime?.resolve()).toEqual({ CLOCKY_WEB_URL: 'http://127.0.0.1:4567' })
