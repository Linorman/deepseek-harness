...
import { createScope, type ScopeKey } from '@clocky/clocky-scope'
import { describe, expect, it } from 'vitest'
import * as Persona from '@clocky/clocky-persona'
import { PERSONA_SECTION } from '@clocky/clocky-persona'
...
describe('the persona row', () => {
  ...
  it('interpolates prompt variables strictly, like any other section', async () => {
    const ctx = await harness('')
    const key: ScopeKey = { agent: 'a1' }
    ctx.systemPrompt.variable('model', () => 'test-model-pro')
    ...
    expect(renderPrompt(await ctx.systemPrompt.assemble({ scope: key })))
      .toContain('You run on test-model-pro.')
  })
