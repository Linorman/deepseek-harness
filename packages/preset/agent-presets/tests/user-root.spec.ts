 ...
 * authors into `<clockyHome>/.agent-presets`, the way `clocky-skill-filesystem` owns
 * `<clockyHome>/skills`. `includeUserRoot: false` is how a deployment — or a test
 * pinning an exact roster — opts out.
 *
 * `$CLOCKY_HOME` is repointed per test because the derived root is resolved in the
...
import AgentPresets, { COMPOSITION_FILE, type Config } from '@clocky/clocky-agent-presets'
...
beforeEach(async () => {
  home = await mkdtemp(join(tmpdir(), 'clocky-preset-home-'))
...
describe('the harness-home preset root', () => {
  ...
  it('yields to a configured user root for authoring, which writableRoot takes first', async () => {
    const explicit = await mkdtemp(join(tmpdir(), 'clocky-preset-explicit-'))
    const ctx = await roster({
      roots: [
