...
npm scope: `@clocky/clocky-*`; Cordis `Service` subclasses and function plugins contribute through `ctx.effect()`, `ctx.on()`, or `ctx.waterfall()`. Rules: [package](AGENTS.md), [root](../AGENTS.md#conventions).

## Hierarchy

Groups hold `packages/<group>/<pkg>/`; names stay `@clocky/clocky-<pkg>`. **Group READMEs own package/ctx-key maps.**
...
| [`team/`](team/README.md) | Durable Team work-system providers and local activation/delivery Consumers | Product — stable API |
...
| [`bundle/`](bundle/README.md) | Installable `clocky --profile` patch layers | Product — stable API |
...
| [`hooks/`](hooks/README.md) | Shared shell-hook wire-protocol library | Product — stable API |
...
New groups update their README and this table.
