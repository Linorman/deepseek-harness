...
适配器在 seam 上注册提供方路由；重试与 token 测量仍是独立消费方。子 README 负责路由、元数据、回放和提供方协议细节；[LLM 架构决策](../../.agents/notes/implemented/simplification/2026-08-27-remove-first-party-integrations.zh.md)说明设计原理。

子系统参考——消息与内容块、模型请求、`StreamChunk` 协议、适配器约定（adapter contract）——见 [docs/subsystems/llm-streaming.md](../../docs/subsystems/llm-streaming.zh.md)（token 计量：[token-meter.md](../../docs/subsystems/token-meter.zh.md)）；另见[第一方产品集成移除](../../.agents/notes/implemented/simplification/2026-08-27-remove-first-party-integrations.zh.md)、[回放 token 计量](../../.agents/notes/implemented/architecture/2026-07-15-replay-token-meter-service.zh.md)与[按路由模型上下文](../../.agents/notes/implemented/architecture/2026-07-20-routed-model-context-and-compaction-policy.zh.md) Agent Note。
