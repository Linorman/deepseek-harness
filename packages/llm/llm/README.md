...
- `CONTEXT_WINDOW_EXCEEDED_CODE` — the provider-neutral code adapters use when a request exceeds the model context window, regardless of thrown-HTTP versus in-band finish delivery. `isContextWindowExceededError(detail)` is the shared conservative classifier for OpenAI-compatible provider detail.
...
- `EMPTY_RESPONSE_CODE` — the provider-neutral code adapters use for a degenerate provider completion: a terminal `stop` that carried no content blocks at all. Classified as an error finish (not a successful empty message) because the attempt produced nothing durable; `clocky-llm-retry` retries it by default.
...
The repository's shipped provider adapter is [`@clocky/clocky-llm-pi-ai`](../llm-pi-ai), which dynamically resolves configured provider/model pairs through `@earendil-works/pi-ai`. It follows the `StreamChunk` conventions in `types.ts`: usage precedes finish and tool arguments remain raw strings. Adapter implementations may throw or emit a failure finish internally; `LlmRuntime` exposes both as a terminal failure finish. See the [terminal-failure decision](../../../.agents/notes/implemented/architecture/2026-07-29-terminal-llm-stream-failures.md) for the service boundary.
...
- **No retry execution, caching, or rate limiting ships in this service** — provider registration stores retry policy, but `llm/stream` remains a single-attempt call wrapper. The agent loop separately offers proven model-request failures to `agent/request-error`, whose default preserves the original failure; `@clocky/clocky-llm-retry` is the optional executor loaded by the shared example spine.
...
- **`GenerateOptions.sessionId` is a locally-declared brand** — importing clocky-session's `SessionId` would cycle; a future ids-owning package would dissolve the workaround.
