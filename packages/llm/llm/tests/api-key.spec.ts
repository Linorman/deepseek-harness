...
import { assertUsableApiKey, INVALID_CREDENTIAL_CODE, normalizeApiKey } from '@clocky/clocky-llm'
...
describe('assertUsableApiKey', () => {
  it('returns the trimmed key when it is usable', () => {
    expect(assertUsableApiKey('  sk-abc  ', 'llm-test-adapter', 'TEST_API_KEY')).toBe('sk-abc')
  })

  it('refuses a blank stored credential, naming the reference', () => {
    expect(() => assertUsableApiKey('   ', 'llm-test-adapter', 'TEST_API_KEY'))
      .toThrow(/llm-test-adapter: the API key resolved from TEST_API_KEY is blank/)
  })
  ...
  it('never echoes the key it refuses', () => {
    try {
      assertUsableApiKey('sk-\u{1F600}supersecret', 'llm-test-adapter', 'TEST_API_KEY')
      expect.fail('an illegal key must throw')
    } catch (error) {
