...
import LlmRuntime, { LlmAdapter, LlmError } from '@clocky/clocky-llm'
import type { GenerateOptions, LlmConfigurableProvider, StreamChunk } from '@clocky/clocky-llm'
...
describe('llm/adapters-updated', () => {
  ...
  it('replaces a route set in one event, never publishing an empty registry between the two', async () => {
    // The retry-policy swap in llm-test-adapter: disposing and re-registering
...
describe('configurable-provider directory', () => {
  it('registers entries, lists detached copies in order, and fires the topology event', async () => {
    ...
    ctx.llm.registerConfigurableProviders([
      entry({ provider: 'test-provider', displayName: 'Test Provider', settingsNs: 'llm-test-adapter', settingsPath: [] }),
      entry(),
    ])
    ...
    expect(ctx.llm.listConfigurableProviders()[0]!.displayName).toBe('Test Provider')
