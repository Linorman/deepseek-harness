 ...
 *
 * @module clocky-llm-pi-ai/config
 */
...
import { resolveRetryPolicy, RetryPolicySchema } from '@clocky/clocky-llm'
import type { ResolvedRetryPolicy, RetryPolicyConfig } from '@clocky/clocky-llm'
import {
...
function rejectRemovedFields(provider: string, source: PiAiProviderProfile): void {
  ...
  if ('maxRetries' in legacy || 'maxRetryDelayMs' in legacy) {
    throw new Error(
      ...
      + ' compose agent recovery with clocky-llm-retry',
    )
  }
}
