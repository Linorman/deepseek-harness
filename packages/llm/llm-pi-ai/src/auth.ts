...
import type { CredentialKey, CredentialProvider, CredentialRecord } from '@clocky/clocky-credentials'
import { launchEnvironmentOf } from '@clocky/clocky-launch-environment'
import { LlmError } from '@clocky/clocky-llm'
...
function writableStore(ctx: Context): CredentialProvider {
  ...
  if (credentials === undefined) {
    throw new LlmError(
      ...
      + ' credential a sign-in produces; mount one (clocky-credentials-local) to sign in',
      'NO_CREDENTIAL_STORE',
    )
  }
  return credentials
