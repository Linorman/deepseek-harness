...
import LlmRuntime, { createMessage, createUserMessage } from '@clocky/clocky-llm'
import LocalCredentialProvider from '@clocky/clocky-credentials-local'
...
async function loadComposition(): Promise<{ ctx: Context; settingsPath: string }> {
  root = await mkdtemp(join(tmpdir(), 'clocky-pi-composition-'))
  ...
  await writeFile(configPath, [
    ...
    '- id: settings',
    "  name: '@clocky/clocky-settings-file'",
    '  config:',
    `    path: ${JSON.stringify(settingsPath)}`,
    '    debounceMs: 10',
    '- id: credentials',
    "  name: '@clocky/clocky-credentials-local'",
    '  config:',
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@clocky/clocky-settings-file', FileSettingsProvider],
    ['@clocky/clocky-credentials-local', LocalCredentialProvider],
    ['@clocky/clocky-llm-pi-ai', LlmPiAi],
  ])
  ctx.loader.internal = {
    version: 'v2',
