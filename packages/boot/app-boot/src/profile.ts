 ...
 * `clocky --profile` launcher family.
 *
 * A profile is a directory under `$CLOCKY_HOME/profiles/<name>` holding a
 * `package.json` (out-of-tree plugin dependencies plus the profile manifest
 * `clocky.profile` with its ordered `bundles` list) and a `cordis.patch.yml`
 * (the user's own patch layer, applied after every bundle layer). Bundles are
 * npm packages whose manifest declares
 * `"clocky": { "bundle": { "patch": "./cordis.patch.yml" } }`; the tree is
 * composed by applying each bundle's patch list in `clocky.profile.bundles` order over
 ...
 * first from the clocky installation (the launcher's own package), then from the
 ...
 * flat fallback directory `$CLOCKY_HOME/profiles/node_modules` (one symlink per
 ...
 * @module @clocky/clocky-app-boot/profile
 */
...
import type { EntryOptions } from '@clocky/cordis-plugin-loader'
import { applyEntryPatches, type PatchOptions } from '@clocky/cordis-plugin-include'
...
/** The bundle half of the `clocky` manifest section: what a bundle package exports. */
export interface ClockyBundleManifest {
  /** The patch layer this bundle exports, relative to its package root. */
  patch: string
}

/** The profile half of the `clocky` manifest section: what a profile directory composes. */
export interface ClockyProfileManifest {
  /** Ordered bundle layer list (package names). */
  bundles?: string[]
}

/**
 * The profile-launcher slice of the `clocky`-owned package.json section. A
 * manifest may declare both roles; other consumers own additional keys.
 */
export interface ClockyManifestSection {
  /** Bundle metadata consumed by the profile launcher. */
  bundle?: ClockyBundleManifest
  /** Profile metadata consumed by the profile launcher. */
  profile?: ClockyProfileManifest
}
...
export interface ProfileManifest {
  ...
  clocky?: ClockyManifestSection
}

/** One resolved bundle layer of a profile. */
export interface ProfileLayer {
  /** The bundle's package name, as listed in `clocky.profile.bundles`. */
  packageName: string
...
export interface Profile {
  ...
  dir: string
  /** Bundle layers in `clocky.profile.bundles` order. */
  layers: ProfileLayer[]
 ...
 * @param name - the profile name (`clocky --profile <name>`).
 * @param home - the Harness home; defaults to {@link resolveClockyHome}.
 * @returns the absolute profile directory (which may not exist yet).
 */
export function resolveProfileDir(name: string, home: string = resolveClockyHome()): string {
  if (name === '' || name.includes('/') || name.includes('\\') || name === '.' || name === '..'
    // The launcher-maintained flat module fallback lives at this sibling path.
    || name === 'node_modules') {
    throw new Error(`clocky: invalid profile name ${JSON.stringify(name)}`)
  }
...
export const PROFILE_TEMPLATES: Record<string, readonly string[]> = {
  web: ['@clocky/clocky-base', '@clocky/clocky-web-app'],
  headless: ['@clocky/clocky-base', '@clocky/clocky-headless'],
}

/** Installation-owned bundle tuples normalized to the shipped template. */
const INSTALLATION_OWNED_PROFILE_TUPLES: Record<string, readonly string[]> = {
  headless: ['@clocky/clocky-base', '@clocky/clocky-web-app', '@clocky/clocky-headless'],
}

/** The bundle list a `clocky plugin` init uses for a name with no shipped template. */
export const DEFAULT_PROFILE_BUNDLES: readonly string[] = ['@clocky/clocky-base']

const PROFILE_PATCH_TEMPLATE = `# Your patch layer for this clocky profile, applied after every bundle layer:
 ...
 * @param bundles - the initial `clocky.profile.bundles` layer list.
 */
export function initProfile(dir: string, bundles: readonly string[]): void {
  ...
  if (!existsSync(manifestPath)) {
    const manifest: ProfileManifest & { private: boolean } = {
      name: `clocky-profile-${basename(dir)}`,
      private: true,
      dependencies: {},
      clocky: { profile: { bundles: [...bundles] } },
    }
...
function ensureSymlink(link: string, target: string): void {
  ...
  if (stat !== undefined) {
    if (!stat.isSymbolicLink()) {
      throw new Error(`clocky: ${link} exists and is not a symlink; remove it so clocky can manage the installation fallback`)
    }
...
/**
 * Maintain the flat module fallback `$CLOCKY_HOME/profiles/node_modules`: one
 * symlink per package in the clocky app's resolvable dependency CLOSURE (BFS
 ...
 * Definition packages (`clocky-compaction`, `clocky-invariants`, ...) that the app
 ...
 * @param installAnchor - absolute path of the clocky app's package.json.
 * @param home - the Harness home; defaults to {@link resolveClockyHome}.
 */
export function healProfilesModuleFallback(installAnchor: string, home: string = resolveClockyHome()): void {
  ...
  for (let next = queue.shift(); next !== undefined; next = queue.shift()) {
    // Peer dependencies participate: Service Definition packages (clocky-subprocess,
    // clocky-compaction, ...) are peers of their implementations, never plain
...
function normalizeShippedProfile(name: string, dir: string, manifest: ProfileManifest): ProfileManifest {
  ...
  const bundles = manifest.clocky?.profile?.bundles
  ...
  const normalized: ProfileManifest = {
    ...
    clocky: {
      ...manifest.clocky,
      profile: { ...manifest.clocky?.profile, bundles: [...current] },
    },
  }
 ...
 * `@clocky/clocky-base` (and every other in-box bundle) always comes from
 * the same installation as the running clocky, never from a profile-local copy.
 * Resolution does not require the package to export `./package.json`.
 * @param binName - the diagnostic prefix on the thrown error.
 * @param packageName - the bundle's package name from `clocky.profile.bundles`.
 * @param installAnchor - absolute path of a file inside the clocky app package (its package.json).
...
export function resolveBundleDir(
  ...
  throw new Error(
    `${binName}: cannot resolve profile bundle ${JSON.stringify(packageName)} from the clocky installation or ${profileDir}; `
    + `run 'clocky plugin --profile ${basename(profileDir)} install' if its dependency is not installed`,
  )
}

/**
 * Load a profile: resolve every `clocky.profile.bundles` entry to its patch
 * layer and parse the profile's own patch file. A listed bundle without a
 * `clocky.bundle` manifest fails loud — naming a bundle-less package as a layer
 * is a misconfiguration, not "no patches".
 * @param binName - the diagnostic prefix on thrown errors.
 * @param name - the profile name.
 * @param installAnchor - absolute path of the clocky app's package.json (first resolution anchor).
 * @param home - the Harness home; defaults to {@link resolveClockyHome}.
...
export function loadProfile(
  binName: string, name: string, installAnchor: string, home: string = resolveClockyHome(),
  ...
  if (!existsSync(join(dir, 'package.json'))) {
    ...
    if (template === undefined) {
      throw new Error(
        `${binName}: profile ${JSON.stringify(name)} does not exist; create it with 'clocky plugin --profile ${name} add <package>'`,
      )
    }
    initProfile(dir, template)
  }
  const manifest = normalizeShippedProfile(name, dir, readProfileManifest(binName, dir))
  // A hand-written profile manifest may omit the clocky section entirely.
  const bundles = manifest.clocky?.profile?.bundles ?? []
  const layers = bundles.map((packageName): ProfileLayer => {
    const packageDir = resolveBundleDir(binName, packageName, installAnchor, dir)
    const bundleManifest = JSON.parse(readFileSync(join(packageDir, 'package.json'), 'utf8')) as ProfileManifest
    const declared = bundleManifest.clocky?.bundle?.patch
    if (declared === undefined) {
      throw new Error(`${binName}: profile bundle ${JSON.stringify(packageName)} declares no clocky.bundle in its package.json`)
    }
