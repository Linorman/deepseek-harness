/**
 * Shared boot glue for the app bins (`clocky`, `clocky-acp-demo`): load the gitignored
 * `.env`, install the fail-loud Loader guards, resolve the config path (snapshot-aware), load the
 * optional user patch layers from the Harness home (`~/.clocky`), expose its path resolver to
 * config expressions, and drive the Cordis Loader against a leaf `cordis.yml` until the tree settles.
 * @module @clocky/clocky-app-boot
 */
...
import { Context, type FiberState } from '@clocky/cordis'
import Loader, { type Entry, type EntryOptions } from '@clocky/cordis-plugin-loader'
import Include, { applyEntryPatches, entryListSchema, type PatchOptions } from '@clocky/cordis-plugin-include'
import Group from '@clocky/cordis-plugin-group'
import { clockyHomePath, resolveClockyHome } from '@clocky/clocky-home-paths'
import { createLaunchEnvironmentSnapshot, type LaunchEnvironmentSnapshot } from '@clocky/clocky-launch-environment'
...
declare module '@clocky/cordis' {
  interface Context {
    ...
    clockyHomePath?: typeof clockyHomePath
  }
}
...
export {
  ...
  type ClockyBundleManifest,
  type ClockyManifestSection,
  type ClockyProfileManifest,
  type Profile,
  type ProfileLayer,
 ...
 * @param snapshotMode - the bin's `$CLOCKY_SNAPSHOT` value; only `'replay'` swaps the
 *   basename.
...
const BOOTSTRAP_PREFIXES = ['CLOCKY_', 'XDG_', 'DYLD_', 'BASH_FUNC_']
 ...
 * entries (`@clocky/cordis-plugin-include`'s `PatchOptions`): id-targeted config
 ...
 * `@clocky/cordis-plugin-include` `PatchOptions` (id-targeted config overrides and
...
export async function mountRootInclude(
  ...
  // preset living outside this workspace cannot resolve `@clocky/cordis-plugin-group`
 ...
 * directory. The self-referential `clocky-tool-cordis` toolset reads and edits this
...
export function addHarnessSourceSection(ctx: Context, sourceRoot: string): (() => void) | undefined {
  ...
  return systemPrompt.section({
    ...
    order: -99,
    text: `The Clocky implementation checkout is at ${sourceRoot}. The checkout location and current working directory are separate values and may differ; never infer the working directory from this path. Use pwd to determine the current working directory. Use this checkout only to inspect or extend Clocky itself.`,
  })
}
