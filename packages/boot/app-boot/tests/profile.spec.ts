/**
 * Profile machinery of `clocky-app-boot`: directory resolution and init,
...
const tmp = (): string => mkdtempSync(join(tmpdir(), 'clocky-profile-'))
...
function stageInstallation(bundles: Record<string, { patch?: string; deps?: Record<string, string> }>): string {
  ...
  for (const [name, spec] of Object.entries(bundles)) {
    ...
    writeFileSync(join(dir, 'package.json'), JSON.stringify({
      ...
      ...spec.patch === undefined ? {} : { clocky: { bundle: { patch: './cordis.patch.yml' } } },
    }))
    if (spec.patch !== undefined) writeFileSync(join(dir, 'cordis.patch.yml'), spec.patch)
  }
  writeFileSync(join(appDir, 'package.json'), JSON.stringify({ name: 'clocky-app', dependencies: appDeps }))
...
describe('initProfile', () => {
  it('creates manifest, user patch layer, and pnpm workspace once, never overwriting', () => {
    ...
    initProfile(dir, ['@clocky/clocky-base'])
    ...
    expect(readProfileManifest('t', dir).clocky?.profile?.bundles).toEqual(['@clocky/clocky-base'])
...
describe('manifest round-trip', () => {
  it('writes and reads back, and fails loud on a broken manifest', () => {
    const dir = tmp()
    writeProfileManifest(dir, { name: 'p', clocky: { profile: { bundles: ['a'] } } })
    expect(readProfileManifest('t', dir).clocky?.profile?.bundles).toEqual(['a'])
...
describe('loadProfile', () => {
  it('resolves each clocky.profile.bundles entry to its patch layer in order, plus the user layer', () => {
    const anchor = stageInstallation({
    ...
    // A hand-made profile without the user layer file or clocky section: empty layers, no throw.
  ...
  it('auto-initializes only shipped templates and fails loud otherwise', () => {
    ...
    // @clocky/* through tsconfig paths regardless of the staged anchor.
    expect(PROFILE_TEMPLATES.web).toContain('@clocky/clocky-base')
    try {
      loadProfile('t', 'web', anchor, home)
    } catch {
      // Resolution failure is the plain-Node outcome for this empty anchor.
    }
    expect(readProfileManifest('t', resolveProfileDir('web', home)).clocky?.profile?.bundles)
      .toEqual([...PROFILE_TEMPLATES.web ?? []])
  })

  it('normalizes only the exact installation-owned headless bundle tuple', () => {
    const anchor = stageInstallation({
      '@clocky/clocky-base': { patch: '[]\n' },
      '@clocky/clocky-web-app': { patch: '[]\n' },
      '@clocky/clocky-headless': { patch: '[]\n' },
      'custom-bundle': { patch: '[]\n' },
    })
    const home = tmp()
    const stock = resolveProfileDir('headless', home)
    initProfile(stock, [
      '@clocky/clocky-base', '@clocky/clocky-web-app', '@clocky/clocky-headless',
    ])
    loadProfile('t', 'headless', anchor, home)
    expect(readProfileManifest('t', stock).clocky?.profile?.bundles)
      .toEqual(['@clocky/clocky-base', '@clocky/clocky-headless'])

    const customHome = tmp()
    const custom = resolveProfileDir('headless', customHome)
    initProfile(custom, [
      '@clocky/clocky-base', '@clocky/clocky-web-app', '@clocky/clocky-headless', 'custom-bundle',
    ])
    loadProfile('t', 'headless', anchor, customHome)
    expect(readProfileManifest('t', custom).clocky?.profile?.bundles).toEqual([
      '@clocky/clocky-base', '@clocky/clocky-web-app', '@clocky/clocky-headless', 'custom-bundle',
    ])
  })

  it('fails loud when a listed bundle declares no clocky.bundle', () => {
    const anchor = stageInstallation({ 'not-a-bundle': {} })
    const home = tmp()
    const dir = resolveProfileDir('demo', home)
    initProfile(dir, ['not-a-bundle'])
    expect(() => loadProfile('t', 'demo', anchor, home)).toThrow('declares no clocky.bundle')
  })
})
...
describe('healProfilesModuleFallback', () => {
  it('links the app and bundle dependency surface flat under profiles/node_modules', () => {
    ...
    for (const name of ['bundle-a', 'plain-lib', 'dep-of-a', 'clocky-app']) {
  ...
  it('throws when a fallback entry is a real directory', () => {
    const anchor = stageInstallation({})
    const home = tmp()
    mkdirSync(join(home, 'profiles', 'node_modules', 'clocky-app'), { recursive: true })
  ...
  it('replaces a wrong symlink', () => {
    ...
    healProfilesModuleFallback(anchor, home)
    expect(readlinkSync(join(fallback, 'clocky-app'))).toContain('app')
  })
  ...
  it('tolerates losing the concurrent-heal race to an identical link and rejects a different one', () => {
    ...
    const fallback = join(home, 'profiles', 'node_modules')
    expect(lstatSync(join(fallback, 'clocky-app')).isSymbolicLink()).toBe(true)
  })
})
