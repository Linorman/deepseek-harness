...
describe('loadOptionalPatches', () => {
  ...
  it('parses a patch list and preserves !!js expressions as loader expression nodes', () => {
    ...
    writeFileSync(join(dir, PROFILE_PATCH_FILENAME), [
      ...
      '  config:',
      '    model: !!js process.env.CLOCKY_SPEC_MODEL',
      '- insert:',
      '    - id: llm',
    ...
    expect(patches?.[0]).toMatchObject({
      id: 'agent-loop',
      config: { model: { __jsExpr: 'process.env.CLOCKY_SPEC_MODEL' } },
    })
...
describe('boot with user patches', () => {
  it('applies id-targeted overrides, inserts, and interpolates !!js from the environment', async () => {
    ...
    writeFileSync(join(userDir, PROFILE_PATCH_FILENAME), [
      ...
      '  name: ./noop.mjs',
      '  config:',
      '    value: !!js process.env.CLOCKY_APP_BOOT_USER_SPEC',
      '- insert:',
      '    - id: user-extra',
      '      name: ./noop.mjs',
      '',
    ].join('\n'))
    process.env['CLOCKY_APP_BOOT_USER_SPEC'] = 'user-value'
    ...
    try {
      ...
      await ctx.fiber.dispose()
      delete process.env['CLOCKY_APP_BOOT_USER_SPEC']
    }
  })
  ...
  it('watches add, failure, recovery, and removal through transactional HMR', { timeout: 60_000 }, async () => {
    const dir = tmp()
    const userDir = tmp()
