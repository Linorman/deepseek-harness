...
const HMR_EVENT_TIMEOUT_MS = 30_000
...
describe('HMR exact config paths', () => {
  it('observes module changes when its watch base is a filesystem alias', { timeout: 30_000 }, async () => {
    const target = mkdtempSync(join(tmpdir(), 'clocky-hmr-module-canonical-'))
  ...
  it('collapses filesystem aliases before registering an exact watch', async () => {
    const target = mkdtempSync(join(tmpdir(), 'clocky-hmr-canonical-'))
    const alias = `${target}-alias`
    symlinkSync(target, alias, process.platform === 'win32' ? 'junction' : 'dir')
  ...
  it('observes creation when the config parent did not exist at registration', { timeout: 60_000 }, async () => {
    const root = mkdtempSync(join(tmpdir(), 'clocky-hmr-config-'))
