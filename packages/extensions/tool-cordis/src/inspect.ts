 ...
 * `name` adds.
 * @module @clocky/clocky-tool-cordis/inspect
 */
...
import type {} from '@clocky/clocky-cordis-host-runner'
...
export function describeDynamic(ctx: Context, agent?: Agent): string[] {
  ...
  if (rows.length === 0) {
    return ['No dynamic Plugins are defined in this session. Definitions live only in this process\'s memory, so a Clocky restart clears them.']
  }
