...
`@clocky/clocky-workflow-worker-thread` is the current engine and `@clocky/clocky-tool-workflow` is the model-facing consumer. A future process or sandbox engine can replace the implementation without changing the tool.
