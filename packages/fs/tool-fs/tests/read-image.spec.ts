...
import { CodeRuntime } from '@clocky/clocky-code-runtime'
import type { CodeRunRequest, CodeRunResult } from '@clocky/clocky-code-runtime'
import { CallId, LlmAdapter, LlmRuntime } from '@clocky/clocky-llm'
import type { GenerateOptions, LlmModelInfo, LlmResolvedModelInfo, Message, StreamChunk } from '@clocky/clocky-llm'
import SystemPrompt from '@clocky/clocky-system-prompt'
import ToolRuntime, { RUN_CODE_NAME } from '@clocky/clocky-tools'
import type { Config as ToolConfig } from '@clocky/clocky-tools'
import LocalFileSystem from '@clocky/clocky-fs-local'
import * as FsPolicy from '@clocky/clocky-fs-observation-policy'
import LocalAttachmentStore from '@clocky/clocky-attachment-local'
import { AttachmentError, AttachmentId, AttachmentStore } from '@clocky/clocky-attachment'
...
beforeEach(async () => {
  dir = await mkdtemp(join(tmpdir(), 'clocky-read-image-'))
  home = await mkdtemp(join(tmpdir(), 'clocky-read-image-home-'))
})
afterEach(async () => {
...
async function setup(options: SetupOptions = {}) {
  ...
  if (options.attachments !== false) {
    await ctx.plugin(LocalAttachmentStore, { clockyHome: home, ...options.storeConfig })
  }
...
describe('registration surface', () => {
  it('withdraws read_image when the tool-fs fiber or the attachment store is disposed (HMR safety)', async () => {
    ...
    await ctx.plugin(LocalFileSystem, { cwd: dir })
    await ctx.plugin(FsPolicy)
    const attachmentsFiber = await ctx.plugin(LocalAttachmentStore, { clockyHome: home })
    ...
    const remounted = await ctx.plugin(LocalAttachmentStore, { clockyHome: home })
    expect(names()).toEqual(['edit', 'read', 'read_image', 'write'])
