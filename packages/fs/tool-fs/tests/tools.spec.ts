...
import ToolRuntime, { type ToolResult } from '@clocky/clocky-tools'
import { FileSystem, FsError, FsTargetKey, FsVersion } from '@clocky/clocky-fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
describe('session cwd resolution', () => {
  ...
  it('retains ordinary spelling but resolves the cwd before parent traversal', () => {
    ...
    const root = mkdtempSync(join(tmpdir(), 'clocky-tool-fs-session-cwd-'))
