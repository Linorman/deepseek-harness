 ...
 * `@clocky/clocky-fs` Service Definition. It extends `LocalFileSystem` so all
 ...
 * stays `ctx.shell`'s job (`@clocky/clocky-bash-sandbox`). This mirrors the
 ...
 * escalation retry lives in the tool layer (`@clocky/clocky-tool-fs`),
 * exactly as bash's does.
 *
 * @module @clocky/clocky-fs-sandbox
 */

import { Context } from '@clocky/cordis'
import { LocalFileSystem } from '@clocky/clocky-fs-local'
import type { Config as LocalConfig } from '@clocky/clocky-fs-local'
import { FsError } from '@clocky/clocky-fs'
import type { FsEditOutcome, FsEditRequest, FsTarget, FsVersion, FsWriteIntent, FsWriteOutcome } from '@clocky/clocky-fs'
import { writableRoots } from '@clocky/clocky-sandbox'
 ...
 * INSTEAD OF `clocky-fs-local`, together with a `ctx.sandboxPolicy`, is the whole
 * swap — the model-facing tools are untouched). Its configured default mode is
 * the capability fact exposed by {@link sandboxMode}; `clocky-tool-fs` resolves
