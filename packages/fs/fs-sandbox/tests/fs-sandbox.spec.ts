...
import { FsError, FsTargetKey } from '@clocky/clocky-fs'
import type { FsTarget } from '@clocky/clocky-fs'
...
beforeEach(async () => {
  ...
  base = await mkdtemp(join(homedir(), '.clocky-fssbx-'))
...
describe('workspace-write containment', () => {
  ...
  it('a write to the platform temp area lands (parity with the bash runner grant)', async () => {
    const path = join(await mkdtemp(join(tmpdir(), 'clocky-fssbx-tmp-')), 'temp.txt')
