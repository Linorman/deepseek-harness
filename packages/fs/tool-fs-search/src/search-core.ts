 ...
 *
 * @module @clocky/clocky-tool-fs-search/search-core
 */
...
import type { SubprocessHandle, SubprocessOutcome, SubprocessOutputRead, SubprocessSpawnSpec } from '@clocky/clocky-subprocess'
 ...
 * `@clocky/clocky-tool-call-timeout-policy` to enforce through `exec.signal`.
 */
 ...
 * (`clocky-spill-policy`) only shrinks a result's `content`, never its `meta`, so the
 ...
 * (`@clocky/clocky-tool-call-timeout-policy`) and caller cancellation terminate the
 * process tree.
 *
 ...
 * retention pass both the model-facing render ({@link module:@clocky/clocky-tool-fs-search/grep}
 * `formatGrepOutput`) and the search-card projection
 * ({@link module:@clocky/clocky-tool-fs-search/presentation} `grepSearchMeta`)
