 ...
 * model-facing render consumes ({@link module:@clocky/clocky-tool-fs-search/search-core}
 ...
 * final output budget (`clocky-spill-policy`) only shrinks `content`, never `meta`,
 * so this projection owns keeping `meta` bounded.
 *
 * @module @clocky/clocky-tool-fs-search/presentation
 */
 ...
 * ({@link module:@clocky/clocky-tool-fs-search/grep} `formatGrepMatches`), so
