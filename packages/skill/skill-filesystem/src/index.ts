 ...
 *
 * @module @clocky/clocky-skill-filesystem
 */
...
import type Schema from '@clocky/schemastery'
import { parse as parseYaml } from 'yaml'
import type { FileSystem, FsDirEntry, FsTarget } from '@clocky/clocky-fs'
...
import {
  ...
  type SkillSource,
} from '@clocky/clocky-skill'

const PROJECT_CLOCKY_RANK = 100
const PROJECT_AGENTS_RANK = 200
const CUSTOM_RANK = 300
const USER_CLOCKY_RANK = 400
...
export class FileSystemSkillProvider implements SkillProvider {
  ...
  constructor(
    ...
    this.agentsHome = resolve(config.agentsHome ?? process.env.CLOCKY_AGENTS_HOME ?? join(homedir(), '.agents'))
    ...
    const bundledSkillDir = config.bundledSkillDir
      ?? (this.includeDefaultRoots ? process.env.CLOCKY_BUNDLED_SKILL_DIR : undefined)
  ...
  private async roots(cwd: string | undefined): Promise<SkillRoot[]> {
    ...
    if (this.includeDefaultRoots && cwd !== undefined) {
      ...
      roots.push(
        { path: join(projectRoot, '.clocky/skills'), source: 'project-clocky', rank: PROJECT_CLOCKY_RANK, projectRoot },
    ...
    if (this.includeDefaultRoots) {
      roots.push(
        { path: join(this.clockyHome, 'skills'), source: 'user-clocky', rank: USER_CLOCKY_RANK, skipSystem: true },
