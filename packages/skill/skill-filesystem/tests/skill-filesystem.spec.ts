...
import { FileSystem, FsError, FsVersion, type FsDirEntry, type FsEditOutcome, type FsEditRequest, type FsInfo, type FsPathInfo, type FsTarget, type FsWriteOutcome } from '@clocky/clocky-fs'
import * as SkillFileSystem from '../src/index.ts'

async function tempDir(name: string): Promise<string> {
  return await import('node:fs/promises').then(fs => fs.mkdtemp(join(tmpdir(), `clocky-${name}-`)))
}
...
describe('clocky-skill-filesystem plugin exports', () => {
...
describe('FileSystemSkillProvider', () => {
  it('discovers project, custom, user, and agents skill roots in priority order', async () => {
    ...
    await writeSkill(join(home, '.clocky/skills'), 'same', 'user clocky skill')
    await writeSkill(custom, 'same', 'custom skill')
    await writeSkill(join(project, '.agents/skills'), 'same', 'project agents skill')
    await writeSkill(join(project, '.clocky/skills'), 'same', 'project clocky skill')
    await writeSkill(custom, 'custom-only', 'custom only')
    await writeSkill(join(home, '.clocky/skills/.system'), 'hidden-system', 'hidden system')
    ...
    expect(skills.find(skill => skill.name === 'same')?.description).toBe('project clocky skill')
    expect(skills.find(skill => skill.name === 'same')?.source).toBe('project-clocky')
  ...
  it('lets project skills override runtime while runtime overrides custom and user skills', async () => {
    ...
    await writeSkill(join(project, '.clocky/skills'), 'project-name', 'Project wins')
    await writeSkill(custom, 'runtime-name', 'Custom loses')
    await writeSkill(join(home, '.clocky/skills'), 'runtime-name', 'User loses')
  ...
  it('parses flat skills and filters invalid skills from the invocation-neutral listing', async () => {
    ...
    const root = join(home, '.clocky/skills')
  ...
  it('discovers symlinked skill directories and flat files', async () => {
    ...
    await mkdir(join(home, '.clocky/skills'), { recursive: true })
    await symlink(join(external, 'linked-dir'), join(home, '.clocky/skills/linked-dir'))
    await symlink(join(external, 'linked-flat.md'), join(home, '.clocky/skills/linked-flat.md'))
    await symlink(join(external, 'missing'), join(home, '.clocky/skills/broken-link'))
    await symlink('/dev/null', join(home, '.clocky/skills/device-link'))
  ...
  it('uses the filesystem service for discovery, reads, and project-root lookup', async () => {
    ...
    await ctx.plugin(SkillRegistry)
    await ctx.plugin(SkillFileSystem, { clockyHome: join(home, '.clocky'), agentsHome: join(home, '.agents'), watch: false })

    expect((await ctx.skills.list({ cwd: nestedCwd })).map(skill => [skill.name, skill.source])).toEqual([
      ['backend-root', 'project-agents'],
      ['text-skill', 'user-clocky'],
    ])
  ...
  it('forwards cancellation to filesystem reads while loading a skill', async () => {
    ...
    await writeSkill(join(home, '.clocky/skills'), 'abortable-skill', 'Abortable skill')
  ...
  it('uses fs/observed as a synchronous first-party invalidation path without a watcher', async () => {
    ...
    emitObserved(join(home, '.clocky/skills/.system/SKILL.md'), { name: 'write' })
  ...
  it('uses default home root resolution without exposing builtin skills', async () => {
    ...
    const previousAgentsHome = process.env.CLOCKY_AGENTS_HOME
    const previousBundledSkillDir = process.env.CLOCKY_BUNDLED_SKILL_DIR
    const envHome = await tempDir('skill-env-home')
    try {
      process.env.CLOCKY_HOME = join(envHome, '.clocky')
      process.env.CLOCKY_AGENTS_HOME = join(envHome, '.agents')
      const bundled = join(envHome, 'bundled-skills')
      process.env.CLOCKY_BUNDLED_SKILL_DIR = bundled
      await writeSkill(join(envHome, '.clocky/skills'), 'env-skill', 'Env skill')
      ...
      process.env.CLOCKY_HOME = join(envHome, 'empty-clocky')
      delete process.env.CLOCKY_BUNDLED_SKILL_DIR
      process.env.CLOCKY_AGENTS_HOME = join(envHome, 'empty-agents')
      ...
      expect(new SkillFileSystem.FileSystemSkillProvider(empty, {
        ...
        invalidate() {},
      }, { clockyHome: join(envHome, 'empty-clocky') }).name).toBe('filesystem')
    } finally {
      if (previousClockyHome === undefined) {
        delete process.env.CLOCKY_HOME
      } else {
        process.env.CLOCKY_HOME = previousClockyHome
      }
      if (previousAgentsHome === undefined) {
        delete process.env.CLOCKY_AGENTS_HOME
      } else {
        process.env.CLOCKY_AGENTS_HOME = previousAgentsHome
      }
      if (previousBundledSkillDir === undefined) {
        delete process.env.CLOCKY_BUNDLED_SKILL_DIR
      } else {
        process.env.CLOCKY_BUNDLED_SKILL_DIR = previousBundledSkillDir
      }
    }
  })
})
