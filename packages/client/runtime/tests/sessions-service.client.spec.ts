...
import { SessionRuntime, scopeOf } from '../src/client/sessions/service.ts'
...
describe('list store projection', () => {
  it('projects durable titles separately from cwd/id display fallbacks', async () => {
    ...
    expect(state.byId[sid('s2')]).toMatchObject({ displayTitle: 's2', running: true })
...
describe('current selection (migrated from ui-layout, arbitrated into the list snapshot)', () => {
  ...
  it('clear() blanks list.current and the persisted selection', async () => {
    ...
    await feedList(b, [{ id: 's1' }])
    b.svc.open(sid('s1'))
    expect(storage.get('clocky.sessions.current')).toContain('s1')
    b.svc.clear()
    expect(b.svc.list.getSnapshot().current).toBeUndefined()
  ...
  it('persists the selection under clocky.sessions.current and rehydrates it into a fresh service', async () => {
    const storage = new Map<string, string>()
    vi.stubGlobal('localStorage', {
      getItem: (k: string) => storage.get(k) ?? null,
      setItem: (k: string, v: string) => { storage.set(k, v) },
...
describe('cell (render-layer session kit)', () => {
  ...
  it('startup restore: a persisted selection validated by the first projection opens its window unprompted', async () => {
    const storage = new Map<string, string>([
      ['clocky.sessions.current', JSON.stringify({ sessionId: 's1' })],
    ])
    vi.stubGlobal('localStorage', {
      getItem: (k: string) => storage.get(k) ?? null,
      setItem: (k: string, v: string) => { storage.set(k, v) },
