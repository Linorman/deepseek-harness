...
import type { RunningToolCall, ToolResultNode } from '@clocky/clocky-client-runtime/client'
...
function settled(over: Partial<ToolResultNode> = {}): ToolResultNode {
  return {
    ...
    time: 3_000,
    callId: 'call-skill',
    call: { name: 'skill', argsRaw: '{"name":"manage-issues"}' },
    callTime: 2_000,
...
function running(argsRaw = '{"name":"manage-issues"}'): RunningToolCall {
  return {
...
describe('SkillRow', () => {
  it('renders a compact Bash-shaped summary and discloses the exact instructions', () => {
    ...
    const row = screen.getByRole('button', { name: 'Skillmanage-issues' })
    ...
    expect(view.container.textContent).not.toContain('{"name":"manage-issues"}')
  ...
  it('keeps a running call compact and announces its state', () => {
    ...
    expect(view.container.textContent).toContain('manage-issues')
