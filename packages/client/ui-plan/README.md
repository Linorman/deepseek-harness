...
Team-owned coordinator Sessions do not mount this generic command chip, even when the `plan` projection is active: their direct Session command RPC is intentionally fenced, and the Team control operation that will own plan changes is not part of this surface yet.
...
Indirectly, through the `/plan off` command line the chip dispatches: `@clocky/clocky-plan-mode` owns the model-visible policy section, the exit-tool schema, and the logged state that line drives, while this package only renders the projection and sends what a user could equally type.
