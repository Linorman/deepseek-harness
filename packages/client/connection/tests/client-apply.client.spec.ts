...
describe('connection client apply', () => {
  ...
  it('carries RPC calls without requiring secure-context randomUUID', async () => {
    ...
    expect(seen[0]?.url).toBe('http://clocky.internal/api/goals/create')
  ...
  it('validates generic RPC transport failures, correlation, and targets', async () => {
    ...
    try {
      ...
      expect(fetch.mock.calls[0]?.[0]).toEqual(new URL('http://clocky.internal/api/goals/create'))
  ...
  it.each(['create', 'edit', 'pause', 'resume', 'complete', 'clear'])('rejects the retired goals/%s fixture endpoint', async (verb) => {
    ;(globalThis as Win).location = { hostname: 'localhost', search: '?fixture' }
    const handle = await mount()
    await expect(handle.rpc.call('/api', `goals/${verb}`, { args: { agentId: 'fx-alpha' } }))
