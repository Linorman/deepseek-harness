...
} from '@clocky/clocky-client-ui-input-trigger/client'
import type { FileReferenceCandidate } from '@clocky/clocky-file-reference/types'
import type { SessionReferenceMentionCandidate } from '@clocky/clocky-session-reference/types'
...
async function bench(
  ...
  sessions: RemoteLookup<SessionReferenceMentionCandidate> = vi.fn(() => Promise.resolve({
    ...
    value: [{
      ...
      mention: '@[Research](clocky-session:InNvdXJjZSI)',
    }],
  })),
  ...
  it('omits redundant session ids and labels sessions without a cwd', async () => {
    ...
    const sessions = vi.fn(() => Promise.resolve({
      ...
      value: [{
        ...
        label: 'same',
        createdAt: 0,
        mention: '@[same](clocky-session:InNhbWUi)',
      }],
    }))
    const { source } = await bench(files, sessions)
...
describe('pick and codec', () => {
  ...
  it('inserts sessions as atomic chips whose clipboard and model forms are canonical mentions', async () => {
    ...
    const mention = '@[Research](clocky-session:InNvdXJjZSI)'
