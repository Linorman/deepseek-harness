 ...
 */
const CSS_VIRTUAL_PREFIX = '\0clocky-css:'
const GLOBAL_CSS_VIRTUAL_PREFIX = '\0clocky-global-css:'
const INLINE_CSS_VIRTUAL_PREFIX = '\0clocky-inline-css:'
 ...
 * Everything else under @clocky/* is either a module-table entry
 * (external) or a leak the purity gate rejects.
 */
export const INLINE_SAFE = /^@clocky\/clocky-(host-apiproxy|file-reference|session|llm|tools|brand|timeout|team\/(schema|selection))(\/|$)/
...
const GENERATED_REMOTE = /^@clocky\/clocky-[a-z0-9]+(?:-[a-z0-9]+)*\/remote$/
...
export function clientBundle(
  ...
  return ({ env }) => {
    const face = buildFace(env?.CLOCKY_BUILD_FACE)
...
export function clientOnly(configs: readonly UserConfig[]): BuildFaceConfig {
  return ({ env }) => buildFace(env?.CLOCKY_BUILD_FACE) === 'host'
...
interface WorkspaceManifest {
  ...
  readonly clocky?: { readonly client?: { readonly external?: unknown } }
}
...
/**
 * Module-table specifiers one `clocky.client` declaration requests. Matching is
 ...
 * @param declaration - the package's `clocky.client` object.
...
export function requestedExternals(
  ...
  return new Set(optionalStringArray(subject, 'clocky.client.external', declaration.external) ?? [])
}

/**
 * Module-table specifiers one package requests. The shell baseline is implicit
 * for every dynamic bundle; `clocky.client.external` only adds package-specific
...
function clientExternals(id: string): ReadonlySet<string> {
  ...
  const externals = new Set([
    ...
    ...requestedExternals(id, workspaceManifest(id).clocky?.client ?? {}),
  ])
...
function clientConfig(id: string, entry: string): UserConfig {
  ...
  return {
    ...
    plugins: [{
      ...
      // inline, and every other @clocky value import is a build error — a
      ...
      name: 'clocky-client-bundle-purity',
      resolveId(source: string) {
        if (!source.startsWith('@clocky/')) return null
        if (isRequested(source)) return null // requested module-table row: external wins
        if (VENDORED_LIBRARY.test(source)) return null // vendored library: inline, no shared identity
        if (INLINE_SAFE.test(source) || GENERATED_REMOTE.test(source)) return null // wire contribution: inline is the point
        throw new Error(
          `client bundle purity: "${source}" is not in the default client externals or ${id}'s clocky.client.external, an inline-safe wire layer, or a generated /remote contribution — `
      ...
      name: 'clocky-css-modules-inline',
...
const STATIC_LINKED_PLUGIN = 'clocky-static-linked-external'
