...
export function ConversationRoot({
  ...
  useTeamTasks, renderSlot, renderSlotChain, t,
  ...
  // Publishes the seat's live height as --clocky-composer-height on the scroll
  ...
  const seatResizeRef = useCallback((seat: HTMLDivElement | null): void => {
    ...
    seatObserver.current = new ResizeObserver(() => {
      scroller.style.setProperty('--clocky-composer-height', `${seat.offsetHeight}px`)
    })
  ...
  // A Team draft is the no-session input machine for the shipped composition;
  // an absent draft keeps the resident textarea inert.
  const inert = sessionId === undefined && teamDraft === undefined
  // A raised block uses the same textarea with the blocker's own reason.
