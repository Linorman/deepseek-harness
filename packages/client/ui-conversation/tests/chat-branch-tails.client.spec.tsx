  ...
  it('the snapshot form attributes each part to the subsystem that produced it', () => {
    const view = render(
      <MessageItem t={t} node={{
        ...
        source: {
          kind: 'plugin',
          plugin: '@clocky/clocky-system-prompt',
          form: 'snapshot',
          sections: [{ name: 'sandbox:policy', text: 'workspace-write' }, { name: 'workspace', text: '/repo' }],
        },
        provenance: { role: 'inject', label: '@clocky/clocky-system-prompt' },
        form: 'snapshot',
      } as never}
      />,
    )
    fireEvent.click(view.getByRole('button', { name: /^上下文注入\s*@clocky\/clocky-system-prompt$/ }))
