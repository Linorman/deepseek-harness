# clocky-client-ui-settings-plugins
...
The cards this package ships cover the shell executor (`bash`) and the agent loop's tool-call parallelism (`agent-loop`).
...
A field's presence in the raw user layer — not its value — is what marks it overridden; a reset clears that field so it re-inherits the composition layer.
