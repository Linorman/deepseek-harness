...
} from '@clocky/clocky-client-ui-settings/src/client/settings-mirror.ts'
...
describe('CardForm', () => {
  ...
  it('stages, writes, and clears a write-only secret independently', async () => {
    const host = stubSettingsScope<Record<string, unknown>>()
    const write = vi.fn((text: string) => Promise.resolve(text === 'accepted'))
    const subject = new CardForm(host.scope, [textField('baseURL')], [{ field: 'apiKey', write }])
    host.publish({ status: 'ready', writable: true, value: { baseURL: 'https://proxy' }, user: {} })

    expect(subject.field('apiKey')).toEqual({ text: '', overridden: false, invalid: false })
    subject.actions().edit('apiKey', ' accepted ')
    expect(subject.field('apiKey')).toEqual({ text: ' accepted ', overridden: false, invalid: false })
    await subject.save()
    expect(write).toHaveBeenCalledWith('accepted')
    expect(subject.shell()).toMatchObject({ dirty: false, failed: false })

    subject.actions().edit('apiKey', '   ')
    await subject.save()
...
describe('ConfigurablePluginsTabController', () => {
  ...
  it('never dispatches a card whose namespace this deployment does not serve', async () => {
    ...
    const controller = new ConfigurablePluginsTabController(settings.mirror, () => ledger('bash', 'removed-card'))
