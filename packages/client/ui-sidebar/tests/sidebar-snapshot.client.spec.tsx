 ...
 * holes (sidebar.teamTasks / sidebar.settings) have no registrant here, so
 * the snapshots pin the shell chrome itself.
 */
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest'
import { act, cleanup, waitFor } from '@testing-library/react'
import { SlotTestRuntime, usePinnedBrowserLanguages } from '@clocky/clocky-client-test-runtime'
import { LocaleRuntime } from '@clocky/clocky-client-locale/client'
import { apply, inject } from '@clocky/clocky-client-ui-sidebar/client'

// The service reads its initial locale from the browser; these specs assert
// the shipped Chinese copy, so they state the browser they assume.
usePinnedBrowserLanguages('zh-CN')

beforeEach(() => { vi.stubEnv('CLOCKY_CLIENT_COMMIT_HASH', 'abc1234') })
...
describe('sidebar shell snapshots', () => {
  it('renders the expanded column in the default locale (zh, no setLocale)', async () => {
    ...
    // Wordmark + capsule both start a task draft in the expanded state.
