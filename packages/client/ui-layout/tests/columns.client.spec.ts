...
} from '@clocky/clocky-client-ui-layout/src/client/columns.ts'
...
describe('computeColumns', () => {
  ...
  it('step 2: details shrinks first, center pinned at min', () => {
    const cols = computeColumns(SIDEBAR_DEFAULT + CENTER_MIN + 330, open(SIDEBAR_DEFAULT), open(DETAILS_DEFAULT))
    expect(cols).toEqual({ sidebar: SIDEBAR_DEFAULT, center: CENTER_MIN, details: 330 })
  })
  ...
  it('step 3: details auto-closes when its min still starves center — sidebar holds its preference', () => {
    const cols = computeColumns(SIDEBAR_DEFAULT + CENTER_MIN + DETAILS_MIN - 10, open(SIDEBAR_DEFAULT), open(DETAILS_DEFAULT))
    expect(cols).toEqual({ sidebar: SIDEBAR_DEFAULT, center: CENTER_MIN + DETAILS_MIN - 10, details: 0 })
  })
