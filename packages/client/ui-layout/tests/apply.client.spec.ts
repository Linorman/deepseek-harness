...
import { apply as themeApply, inject as themeInject, ThemeRuntime } from '@clocky/clocky-client-ui-theme/client'
import { apply, inject, LayoutController } from '@clocky/clocky-client-ui-layout/client'
import { apply as nodeApply } from '@clocky/clocky-client-ui-layout'
import * as invariant from '@clocky/clocky-client-ui-layout/invariant'
...
async function bench() {
  ...
  ctx.provide('sessions', { clear: vi.fn() } as never)
...
describe('ui-layout client apply', () => {
  ...
  it('injects Session closing and attaches the layout actions', async () => {
    const { ctx, slots } = await bench()
    const fiber = ctx.plugin({ inject: [...inject], apply })
    await fiber.await()
    const actions = {
    ...
    expect(Object.keys(injected)).toEqual(['closeSession'])
    expect(injected).toHaveProperty('closeSession', expect.any(Function))
...
describe('node half + invariant companion', () => {
  ...
  it('invariant companion registers under the package name', async () => {
    ...
    expect(register).toHaveBeenCalledWith('@clocky/clocky-client-ui-layout', expect.any(Function))
