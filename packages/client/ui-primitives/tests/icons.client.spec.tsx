...
import * as primitives from '@clocky/clocky-client-ui-primitives'
import {
...
describe('shared icon set', () => {
  it('exports the full icon set (46 base icons + 20 design extracts + four shared glyphs)', () => {
...
describe('robot brand fallback artwork', () => {
  it('renders visible geometry at the requested size for every persistent seat', () => {
    const view = render(<>
      <primitives.BrandPlaceholderMark size={34} />
      <primitives.BrandPlaceholderWordmark size={24} />
      <primitives.BrandPlaceholderBadge size={20} />
    </>)
    const svgs = [...view.container.querySelectorAll('svg')]
    expect(svgs.map(svg => [svg.getAttribute('width'), svg.getAttribute('height')])).toEqual([
      ['34', '34'], ['156', '24'], ['121', '20'],
    ])
    expect(svgs.every(svg => svg.querySelectorAll('path, rect, circle').length > 0)).toBe(true)
    expect(view.container.innerHTML).toContain('M12 5V2.75')
