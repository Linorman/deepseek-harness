 ...
 * `window.__CLOCKY_BOOT__` entry graph (wire single source: {@link WebBootEntry}
 ...
 * @module @clocky/clocky-client-modules
 */
...
/** package.json `clocky.client` declaration fields, validated one by one after reading the file. */
interface ClockyClientDeclaration {
  inject?: string[]
  platform: string
...
/** Resolved package metadata for one `clocky.client` package (cached per name, never expires). */
...
/** Narrow an unknown parsed JSON value to the `clocky.client` declaration, throwing on malformed fields. */
function parseClockyClient(pkgName: string, value: unknown): ClockyClientDeclaration | undefined {
  if (value === undefined) return undefined
  if (typeof value !== 'object' || value === null) {
    throw new Error(`client-modules: ${pkgName} has a non-object clocky.client declaration`)
  }
  const decl = value as Record<string, unknown>
  if (typeof decl.platform !== 'string') {
    throw new Error(`client-modules: ${pkgName} clocky.client.platform must be a string`)
  }
  const inject = optionalStringArray(pkgName, 'clocky.client.inject', decl.inject)
  const external = optionalStringArray(pkgName, 'clocky.client.external', decl.external)
  if (decl.immediately !== undefined && typeof decl.immediately !== 'boolean') {
    throw new Error(`client-modules: ${pkgName} clocky.client.immediately must be a boolean`)
  }
  return {
    ...
    for (const name of entry.external ?? []) {
      ...
      if (dependency === entry) {
        throw new Error(
          ...
          + '— a row must not declare its own package in clocky.client.external',
        )
      }
...
const CLIENT_MODULES_ID = '@clocky/clocky-client-modules'

/** Dynamic package whose ordinary client bundle must be registered before plugin boot starts. */
const CLIENT_RUNTIME_ID = '@clocky/clocky-client-runtime'
...
window.__ModuleLoader__={
  ...
  return [
    ...
    ...preload,
    { kind: 'global', name: '__CLOCKY_BOOT__', value: graph },
  ]
}

/**
 * The web plugin table service: incremental `clocky.client` scan + wire composition
...
export class ClientModuleRegistry extends Service {
  ...
  // subpath rows — or a package without a web `clocky.client` declaration) are
  ...
  private resolveMeta(pkgName: string): PkgMeta | null {
    ...
    const clocky = pkg.clocky
    const decl = parseClockyClient(
      pkgName,
      clocky !== null && typeof clocky === 'object' ? (clocky as Record<string, unknown>).client : undefined,
    )
    ...
    if (clientRel === undefined) {
      throw new Error(`client-modules: ${pkgName} declares clocky.client but exports no "./client" bundle`)
    }
    const meta: PkgMeta = {
