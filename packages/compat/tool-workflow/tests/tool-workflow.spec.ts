...
import type { ToolExecutionResult, ToolExecutionToken } from '@clocky/clocky-tools'
import type { Agent } from '@clocky/clocky-agent'
import { WorkflowRunId, WorkflowEngine } from '@clocky/clocky-compat-workflow'
import type {
  WorkflowAgentEndInfo, WorkflowAgentInfo, WorkflowResult, WorkflowRun,
  WorkflowRunId as WorkflowRunIdType, WorkflowStartRequest,
} from '@clocky/clocky-compat-workflow'
import { CallId } from '@clocky/clocky-llm'
import SubagentRuntime from '@clocky/clocky-compat-subagent'
import WorkerThreadWorkflowEngine from '@clocky/clocky-compat-workflow-worker-thread'
...
describe('clocky-tool-workflow', () => {
  it.each(['workflow', 'team_workflow_start', 'legacy_'])('rejects a non-compatibility tool name %s before registration', (toolName) => {
    expect(() => { toolWorkflow.apply(new Context(), { toolName, maxResultChars: 100 }) }).toThrow('Compatibility tools require a legacy_* name')
  })
  ...
  it('registers under a configured toolName and unregisters on fiber dispose (HMR safety)', async () => {
    ...
    await ctx.plugin(ToolRuntime)
    await ctx.plugin(StubEngine)
    const fiber = await ctx.plugin(toolWorkflow, { toolName: 'legacy_orchestrate' })
    expect(ctx.tools.get('legacy_orchestrate')).toBeDefined()
    expect(ctx.tools.get('legacy_workflow')).toBeUndefined()
    ...
    const section = sections.find(s => s.name === 'tool:legacy_orchestrate')
    expect(section?.text).toContain('legacy_orchestrate')
    expect(sections.some(s => s.name === 'tool:legacy_workflow')).toBe(false)
    await fiber.dispose()
    expect(ctx.tools.get('legacy_orchestrate')).toBeUndefined()
    // …and gone with the fiber — a reload must not leak a stale section.
    expect((await ctx.systemPrompt.assemble()).sections.some(s => s.name === 'tool:legacy_orchestrate')).toBe(false)
  })

  it('presents a generic pending card titled by the meta name, with the script as rawInput', async () => {
    const { ctx } = await setup()
    const tool = ctx.tools.get('legacy_workflow')!
