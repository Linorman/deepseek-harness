...
| Key | Default | Meaning |
|---|---|---|
| `toolName` | `legacy_workflow` | The model-facing tool name to register; must start with `legacy_`. |
...
When visible, the generated default `legacy_workflow` tool definition carries the complete JavaScript hook and metadata contract; `toolName` can rename the definition, and the model submits script, metadata, and optional args.
