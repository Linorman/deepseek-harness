...
面向模型的 **`legacy_workflow` 工具**：运行一段扇出 subagent 的 JavaScript 编排脚本，并返回脚本的最终值。本包负责基于 [`ctx.workflowEngine`](../workflow/README.zh.md) 定义面向模型的 schema 和运行生命周期；脚本解析、执行、上限与取消位于 seam 之后，消费方仍负责面向父级的 schema 和结果包络。
...
浏览器安全的 `@clocky/clocky-compat-tool-workflow/types` 子路径拥有这四类 log-only 事件 payload 及其 `SessionEventMap` 声明。包 invariant 会在冷加载和实时追加时拒绝重复 start、未配对成员、仍有开放成员的终点和 run-end 后更新，同时允许缺失终态后缀的连续前缀。
...
| 键 | 默认值 | 含义 |
|---|---|---|
| `toolName` | `legacy_workflow` | 要注册的面向模型工具名称。 必须以 `legacy_` 开头。 |
...
工具可见时，已生成的默认 `legacy_workflow` 工具定义包含完整的 JavaScript 钩子与元数据约定；`toolName` 可以重命名该定义，模型会提交脚本、元数据和可选 args。
