 ...
 * (`@clocky/clocky-compat-workflow`), so a hardened engine swaps in without touching what the model
 ...
 * @module @clocky/clocky-compat-tool-workflow
 */
...
import type { JsonValue, Session, SessionEventMap } from '@clocky/clocky-session'
import type {
...
export interface Config {
  /** The model-facing tool name to register (default `legacy_workflow`). */
  toolName?: string
...
export const Config: z<Config> = z.object({
  toolName: z.string().pattern(/^legacy_[a-zA-Z0-9_]+$/).default('legacy_workflow'),
