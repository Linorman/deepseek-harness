...
import { CallId, LlmAdapter, createUserMessage } from '@clocky/clocky-llm'
...
import * as control from '@clocky/clocky-compat-tool-subagent-control'
...
async function setup(options: { load?: boolean; config?: tool.Config } = {}) {
  ...
  const root = mkdtempSync(join(tmpdir(), 'clocky-tool-subagent-report-'))
...
describe('clocky-tool-subagent-report', () => {
  it('registers report only in continuable child scopes', async () => {
    const { ctx, parent } = await setup()
    expect(ctx.tools.schemas().map(schema => schema.name)).not.toContain('legacy_report')
    expect(ctx.tools.schemas(parent).map(schema => schema.name)).not.toContain('legacy_report')

    const { child } = await startChild(ctx, parent)
    const schemas = ctx.tools.schemas(child).filter(schema => schema.name === 'legacy_report')
  ...
  it('adds no implicit capability when the package is absent', async () => {
    ...
    const { child } = await startChild(ctx, parent)
    expect(ctx.tools.schemas(child).map(schema => schema.name)).not.toContain('legacy_report')
  ...
  it('does not imply parent controls and survives a global-tool allow-list', async () => {
    const { ctx, parent } = await setup()
    expect(ctx.tools.schemas().map(schema => schema.name)).not.toContain('legacy_send_message')
    await ctx.plugin(control)
    expect(ctx.tools.schemas().map(schema => schema.name)).toContain('legacy_send_message')
    ...
    expect(names).toContain('legacy_report')
    expect(names).not.toContain('legacy_send_message')
  })
  ...
  it('revokes resident installations and defers later grants to the next Activation', async () => {
    ...
    const { child } = await startChild(ctx, parent)
    expect(ctx.tools.schemas(child).map(schema => schema.name)).toContain('legacy_report')
    expect(await sectionNames(ctx, child)).toContain('tool:legacy_report')

    await fiber?.dispose()
    expect(ctx.tools.schemas(child).map(schema => schema.name)).not.toContain('legacy_report')
    expect(await sectionNames(ctx, child)).not.toContain('tool:legacy_report')
  ...
  it('scopes the report guidance to the child that owns it', async () => {
    ...
    const guidance = assembly.sections.find(section => section.name === 'tool:legacy_report')
    ...
    expect(await sectionNames(ctx, parent)).not.toContain('tool:legacy_report')
    // A sibling installs its own copy; neither child can observe the other's.
    expect(await sectionNames(ctx, sibling)).toContain('tool:legacy_report')
    expect((await ctx.systemPrompt.assemble()).sections.map(section => section.name))
      .not.toContain('tool:legacy_report')
  })
  ...
  it('rolls back materialization when a setup contribution revokes itself', async () => {
    ...
    // ghost that `legacy_list_agents` surfaces and `send_message` can resurrect.
...
describe('clocky-tool-subagent-report result independence', () => {
