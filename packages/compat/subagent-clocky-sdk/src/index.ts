 ...
 * @module @clocky/clocky-compat-subagent-clocky-sdk
 */

import type { Context } from '@clocky/cordis'
import z from '@clocky/schemastery'
import { resolveAgentWorkspaceRoot } from '@clocky/clocky-agent'
import type { SubagentCapabilities, SubagentProvider, SubagentStartRequest } from '@clocky/clocky-compat-subagent'
import { assertPositiveFinite, NO_START_CAPABILITIES, resolveChildCwd, validateConfiguredCwd } from '@clocky/clocky-compat-subagent'
import {
...
export const name = 'subagent-clocky-sdk'
...
export const Config: z<Config> = z.object({
  providerName: z.string().default('clocky-sdk'),
  ...
  start(request: SubagentStartRequest) {
    const spec: SdkRunSpec = {
      ...
      onError: (error, stopReason) => {
        ...
        this.ctx.logger.warn(`subagent-clocky-sdk "${this.name}": child run failed (${stopReason}): ${error.message}`)
      },
    }
...
export function apply(ctx: Context, config: Config): void {
  ...
  assertPositiveFinite('subagent-clocky-sdk', 'shutdownTimeoutMs', resolved.shutdownTimeoutMs)
  assertPositiveFinite('subagent-clocky-sdk', 'disposeEofGraceMs', resolved.disposeEofGraceMs)
  assertPositiveFinite('subagent-clocky-sdk', 'disposeGraceMs', resolved.disposeGraceMs)
  if (resolved.maxTokens !== undefined && (!Number.isSafeInteger(resolved.maxTokens) || resolved.maxTokens <= 0)) {
    throw new TypeError('subagent-clocky-sdk maxTokens must be a positive safe integer')
  }
  ...
  const configuredCwd = validateConfiguredCwd('subagent-clocky-sdk', resolved.cwd)
