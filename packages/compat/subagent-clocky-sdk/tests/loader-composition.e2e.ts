...
import { resolveExampleLaunch, runLoaderSmoke } from '@clocky/clocky-loader-smoke'

const fixtureDir = new URL('../../../../examples/jsonrpc-agent/tests/fixtures/compat/subagent-clocky-sdk/', import.meta.url)
...
describe('SDK subagent cwd inheritance through a real cordis.yml', () => {
  it('runs the child runtime in the parent session workspace', async () => {
    ...
    const { stderr } = await runLoaderSmoke({
      label: 'clocky-sdk-subagent cwd composition smoke',
      tempDirPrefix: 'clocky-sdk-subagent-cwd-e2e-',
      binScript: driver,
      libBinScript: driver,
      configPath,
      tsconfigPath: repoTsconfig,
      ...
      env: {
        CLOCKY_TEST_CHILD_COMMAND: childLaunch.command,
        CLOCKY_TEST_CHILD_ARGS: JSON.stringify(childLaunch.args),
        CLOCKY_TEST_CHILD_ENV: JSON.stringify({
      ...
      inspect: async (cwd) => {
        ...
        expect(result?.data.message.content[0].content).toEqual([{ type: 'text', text: `child cwd: ${workspace}` }])
