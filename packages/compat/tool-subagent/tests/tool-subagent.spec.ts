...
import { assembleContextFor, type Agent } from '@clocky/clocky-agent'
...
import type { SubagentStartRequest } from '@clocky/clocky-compat-subagent'
import LocalJobRegistry from '@clocky/clocky-jobs-local'
import * as SubagentSpawn from '@clocky/clocky-compat-subagent-spawn-in-process'
...
/**
 * Drives the REAL plugin body: mounts `clocky-tool-subagent` on a real
...
describe('clocky-tool-subagent', () => {
  it.each(['subagent', 'team_task_delegate', 'legacy_'])('rejects a non-compatibility tool name %s before registration', (toolName) => {
    expect(() => { tool.apply(new Context(), { provider: 'mock', toolName }) }).toThrow('Compatibility tools require a legacy_* name')
  })
  ...
  it('exposes description + prompt + run_in_background to the model (no provider/type parameter)', async () => {
    ...
    const schema = ctx.tools.schemas().find(s => s.name === 'legacy_subagent')
  ...
  it('registers under a configurable toolName so multiple providers can coexist', async () => {
    ...
    await ctx.plugin(tool, { provider: 'spawn', toolName: 'legacy_subagent' })
    await ctx.plugin(tool, { provider: 'acp', toolName: 'legacy_subagent_acp' })

    const names = ctx.tools.schemas().map(s => s.name).filter(n => n.startsWith('legacy_subagent')).sort()
    expect(names).toEqual(['legacy_subagent', 'legacy_subagent_acp'])

    const viaSpawn = await ctx.tools.execute({ signal: testToolSignal, callId: CallId('c-spawn'), name: 'legacy_subagent', arguments: { description: 'd', prompt: 'p' }, agent: fakeAgent() })
    const viaAcp = await ctx.tools.execute({ signal: testToolSignal, callId: CallId('c-acp'), name: 'legacy_subagent_acp', arguments: { description: 'd', prompt: 'p' }, agent: fakeAgent() })
  ...
  it('defaults toolName and omits agentOptions when apply() is called directly (schema bypass)', async () => {
    // `ctx.plugin` validates+defaults config first (toolName→'legacy_subagent', the
    // agentOptions object→{}), so the runtime `?? 'legacy_subagent'` fallback and the
    ...
    expect(ctx.tools.schemas().some(s => s.name === 'legacy_subagent')).toBe(true)
    await callSubagent(ctx, { description: 'd', prompt: 'p' })
  ...
  it('registers when the provider appears LATER — no load-order requirement (Loader starts siblings concurrently)', async () => {
    ...
    expect(ctx.tools.schemas().some(s => s.name === 'legacy_subagent')).toBe(false)
  ...
  it('keeps continuable guidance empty while its provider is absent', async () => {
    ...
    expect(assembly.sections.find(section => section.name === 'tool:legacy_subagent')?.text).toBe('')
    expect(ctx.tools.schemas().some(schema => schema.name === 'legacy_subagent')).toBe(false)
  })
  ...
  it('mirrors the provider lifecycle: gone on backend dispose, re-derived wording on re-registration', async () => {
    ...
    expect(ctx.tools.schemas().find(s => s.name === 'legacy_subagent')!.description).toContain('does not see this conversation')
    ...
    expect(ctx.tools.schemas().find(s => s.name === 'legacy_subagent')!.description).toContain('inherits this conversation')
  })
  ...
  it('the tool PLUGIN fiber owns its lifecycle listeners: disposal unmounts, and a disposed fiber never zombie-mounts', async () => {
    ...
    expect(ctx.tools.schemas().some(s => s.name === 'legacy_subagent')).toBe(true)
    expect((await ctx.systemPrompt.assemble()).sections.some(s => s.name === 'tool:legacy_subagent')).toBe(true)
    await mounted.dispose()
    expect(ctx.tools.schemas().some(s => s.name === 'legacy_subagent')).toBe(false)
    expect((await ctx.systemPrompt.assemble()).sections.some(s => s.name === 'tool:legacy_subagent')).toBe(false)
    ...
    const waiting = await ctx.plugin(tool, { provider: 'later', toolName: 'legacy_subagent_later' })
    await waiting.dispose()
    await mock.mountScriptedProvider(ctx, { name: 'later' })
    expect(ctx.tools.schemas().some(s => s.name === 'legacy_subagent_later')).toBe(false)
  })
  ...
  it('ignores lifecycle events for OTHER providers', async () => {
    ...
    expect(ctx.tools.schemas().filter(s => s.name === 'legacy_subagent')).toHaveLength(1)
    expect(ctx.tools.schemas().find(s => s.name === 'legacy_subagent')!.description).toContain('does not see this conversation')
    await other.dispose()
    expect(ctx.tools.schemas().some(s => s.name === 'legacy_subagent')).toBe(true)
  })
  ...
  it('derives inherited-context wording from a seeded-conversation provider', async () => {
    const ctx = await setup({ provider: 'mock', toolName: 'legacy_subagent' }, { inheritsParentContext: true })
    const schema = ctx.tools.schemas().find(s => s.name === 'legacy_subagent')!
  ...
  it('tools depend on the service: no `subagent` tool without ctx.subagents', async () => {
    ...
    const present = ctx.get('tools')?.schemas().some(s => s.name === 'legacy_subagent') ?? false
  ...
  it('passes persona/toolFilter/maxDepth config through to the start request', async () => {
    ...
    await ctx.plugin(tool, {
      ...
      toolFilter: { deny: ['legacy_subagent'] },
      maxDepth: 2,
    })

    await callSubagent(ctx, { description: 'd', prompt: 'p' })
    expect(seen?.persona).toBe('You are the child.')
    expect(seen?.toolFilter).toMatchObject({ deny: ['legacy_subagent'] })
  ...
  it('a partial toolFilter (deny only) does not materialize an empty allow-list (deny-all trap)', async () => {
    ...
    await ctx.plugin(tool, { provider: 'capture3', toolFilter: { deny: ['legacy_subagent'] }, maxDepth: 'provider-managed' })
    await callSubagent(ctx, { description: 'd', prompt: 'p' })
    expect(seen?.toolFilter).toEqual({ deny: ['legacy_subagent'] })
...
describe('clocky-tool-subagent background mode', () => {
  ...
  it('keeps a continuable-capable provider one-shot when backgroundMode selects one-shot', async () => {
    ...
    tool.apply(ctx, {
      provider: 'resumable',
      toolName: 'legacy_subagent_resumable',
    ...
    const started = await ctx.tools.execute({
      ...
      name: 'legacy_subagent_resumable',
  ...
  it('fails loud when the tasks runtime is not loaded', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(text(result)).toContain('background jobs unavailable: load @clocky/clocky-jobs')
  })
  ...
  it('settles an asynchronous provider-start failure as a failed task', async () => {
    ...
    tool.apply(ctx, { provider: 'broken-start', toolName: 'legacy_subagent_broken' })

    const started = await ctx.tools.execute({
      signal: testToolSignal,
      callId: CallId('broken-start'),
      name: 'legacy_subagent_broken',
  ...
  it('kills a subagent task while provider readiness is still pending', async () => {
    ...
    tool.apply(ctx, { provider: 'pending-start', toolName: 'legacy_subagent_pending' })

    await ctx.tools.execute({
      signal: testToolSignal,
      callId: CallId('pending-start'),
      name: 'legacy_subagent_pending',
  ...
  it('reports startup rollback failure after cancellation as a failed job', async () => {
    ...
    tool.apply(ctx, { provider: 'broken-start-rollback', toolName: 'legacy_subagent_broken_rollback' })

    await ctx.tools.execute({
      signal: testToolSignal,
      callId: CallId('broken-rollback-start'),
      name: 'legacy_subagent_broken_rollback',
  ...
  it('forwards job_kill reasons through the run signal (and defaults one when absent)', async () => {
    ...
    tool.apply(ctx, { provider: 'hanging', toolName: 'legacy_subagent_hang' })

    const startOne = await ctx.tools.execute({ signal: testToolSignal, callId: CallId('h1'), name: 'legacy_subagent_hang', arguments: { description: 'one', prompt: 'p', run_in_background: true }, agent: parent })
    const startTwo = await ctx.tools.execute({ signal: testToolSignal, callId: CallId('h2'), name: 'legacy_subagent_hang', arguments: { description: 'two', prompt: 'p', run_in_background: true }, agent: parent })
...
describe('clocky-tool-subagent continuable background mode', () => {
  const roots: string[] = []
  afterEach(() => {
  ...
  async function continuableSetup() {
    ...
    const root = mkdtempSync(path.join(tmpdir(), 'clocky-tool-subagent-continuable-'))
    roots.push(root)
  ...
  it('defaults continuable delegation to background and returns only its durable id', async () => {
    ...
    expect(schema.description).toContain('legacy_send_message')
    ...
    const guidance = assembly.sections.find(section => section.name === 'tool:legacy_subagent')
    expect(guidance?.text).toContain('Use legacy_subagent in the background by default')
  ...
  it('hides continuable guidance when the current agent cannot see the tool', async () => {
    const { ctx, parent } = await continuableSetup()
    parent.ctx.tools.restrict({ deny: ['legacy_subagent'] })

    expect(ctx.tools.get('legacy_subagent', parent)).toBeUndefined()
    const assembly = await ctx.systemPrompt.assemble(assembleContextFor(parent))
    expect(assembly.sections.find(section => section.name === 'tool:legacy_subagent')?.text).toBe('')
  })
  ...
  it('isolates a cancelled continuable preparation from a concurrent sibling', async () => {
    ...
    tool.apply(ctx, {
      provider: 'gated',
      toolName: 'legacy_subagent_gated',
      backgroundMode: 'continuable',
      maxDepth: 3,
    })
...
describe('background preflight failure (no orphaned child, by construction)', () => {
  it('never starts the child when tasks.start preflight throws', async () => {
    ...
    tool.apply(ctx, { provider: 'probe', toolName: 'legacy_subagent_probe' })

    const result = await ctx.tools.execute({
      signal: testToolSignal,
      callId: CallId('probe-1'),
      name: 'legacy_subagent_probe',
      arguments: { description: 'd', prompt: 'p', run_in_background: true },
      agent: parent,
    })
    expect(result.isError).toBe(true)
