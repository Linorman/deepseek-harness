 ...
 * @module @clocky/clocky-compat-tool-subagent
 */

import type { Context } from '@clocky/cordis'
import z from '@clocky/schemastery'
import { defineTool } from '@clocky/clocky-tools'
import type { AgentOptions } from '@clocky/clocky-agent'
import type { ContentBlock } from '@clocky/clocky-llm'
import type { JsonValue } from '@clocky/clocky-session'
import { assertSubagentMaxDepth, settleRun } from '@clocky/clocky-compat-subagent'
import type { SubagentProvider, SubagentResult, SubagentRun } from '@clocky/clocky-compat-subagent'
...
export interface Config {
  ...
  provider: string
  /**
   * Model-facing tool name (default `legacy_subagent`). Each loaded instance must use
   * a distinct name.
   */
  toolName?: string
  /**
...
export const Config: z<Config> = z.object({
  ...
  toolName: z.string().pattern(/^legacy_[a-zA-Z0-9_]+$/).default('legacy_subagent'),
...
export function apply(ctx: Context, config: Config): void {
  ...
  const toolName = config.toolName ?? 'legacy_subagent'
  if (!/^legacy_[a-zA-Z0-9_]+$/.test(toolName)) throw new Error('Compatibility tools require a legacy_* name')
  ...
  const mount = (provider: SubagentProvider): void => {
    ...
    disposeTool = ctx.tools.register(defineTool({
      ...
      description: wording.description + (backgroundEnabled
        ...
        ? continuable
          ? ' This tool runs in the background by default, immediately returns a durable subagent id, and keeps the child conversation available for later turns. When that run settles, the runtime sends the parent a notice containing its outcome and any final assistant message; `legacy_send_message` starts a later turn in the same child conversation. Set `run_in_background: false` only when your next action depends on receiving the result.'
  ...
  if (present !== undefined) {
    ...
    ctx.logger.info(`subagent provider "${config.provider}" not registered yet; the "${config.toolName ?? 'legacy_subagent'}" tool will register when it appears`)
  }
