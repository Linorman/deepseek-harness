/**
 * Model-facing `legacy_get_goal`, `legacy_create_goal`, and `legacy_update_goal` tools over the
 * persisted same-session goal domain.
 * @module @clocky/clocky-compat-tool-goal
 */

import type { Context } from '@clocky/cordis'
import z from '@clocky/schemastery'
import { GoalId } from '@clocky/clocky-compat-goal'
import type { GoalRef, GoalView } from '@clocky/clocky-compat-goal'
import { boundContextSummary, createUserMessage, HarnessError } from '@clocky/clocky-llm'
import { defineTool } from '@clocky/clocky-tools'
import type { GenericCallView } from '@clocky/clocky-tools'
...
function guidance(blockedAfter: number): string {
  return 'Use goal tools for one long-running completion objective in the current session. '
    + 'legacy_create_goal may infer goal intent from a direct human request in any language; do not '
    + 'create a goal for routine single-turn work. Call legacy_get_goal before legacy_update_goal and copy its '
    + 'exact goal_id and revision. After session resume or fork, an active goal is disarmed: when '
    + 'a human asks to continue or resume in any wording or language, use legacy_update_goal action '
...
export function apply(ctx: Context, config: Config): void {
  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      goal_id: { type: 'string', required: true, description: 'Exact id returned by legacy_get_goal.' },
      revision: { type: 'number', required: true, description: 'Exact positive revision returned by legacy_get_goal.' },
      action: {
        type: 'string',
        required: true,
        enum: UPDATE_ACTIONS,
