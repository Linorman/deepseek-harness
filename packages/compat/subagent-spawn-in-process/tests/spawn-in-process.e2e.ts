...
describe.skipIf(!process.env.DEEPSEEK_API_KEY)('spawn backend with-key smoke', () => {
  it('a parent delegates to a child that writes a file on disk', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'clocky-subagent-spawn-e2e-'))
    ctx = await spawnHarness(workdir)
    const parent = ctx.agentLoop.create(SessionId('e2e-parent'), { provider: 'test-provider', model: 'test-model' })

    parent.followup(createUserMessage({
      content: [{ type: 'text', text:
      'Use the legacy_subagent tool to delegate this exact task: "Use the bash tool to write the text '
