/**
 * The globally named `legacy_send_message` and `legacy_interrupt_agent` tools: thin
 ...
 * `@clocky/clocky-compat-tool-subagent` instances so multiple delegation tools share
 * one control API.
 * @module @clocky/clocky-compat-tool-subagent-control
 */
...
/**
 * Register the `legacy_send_message` and `legacy_interrupt_agent` tools.
...
export function apply(ctx: Context): void {
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      ...
      if (!parent) {
        ...
        throw new Error('legacy_send_message requires a calling agent (exec.agent was undefined)')
      }
  ...
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      + 'queued for the agent stay parked until a later legacy_send_message, agents it started keep running, and '
    ...
    execute(args, exec) {
      ...
      if (!caller) {
        ...
        throw new Error('legacy_interrupt_agent requires a calling agent (exec.agent was undefined)')
      }
