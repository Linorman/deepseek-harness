/**
 * The globally named `legacy_list_agents` tool: a thin model-facing adapter over
 ...
 * loadable from the root `legacy_send_message` plugin so a deployment can register
 * continuation delivery without exposing discovery.
 * @module @clocky/clocky-compat-tool-subagent-control/list-agents
 */
...
import { assertNever } from '@clocky/clocky-llm'
import type { SubagentDescendantListEntry, SubagentListEntry } from '@clocky/clocky-compat-subagent'
...
function project(
  ...
  // One-shot children cannot be continued by legacy_send_message, so the model
...
/**
 * Register the `legacy_list_agents` tool.
...
export function apply(ctx: Context): void {
  ctx.tools.register(defineTool({
    ...
    description:
      ...
      + 'terminal, and not a result waiting to be collected; a `legacy_send_message` starts a new turn on the same '
      + 'conversation, and a direct child remains a `legacy_send_message` candidate in every status. The snapshot is not a delivery '
      + 'promise — `legacy_send_message` performs the authoritative check and may still fail. Children that could '
      ...
      + 'session id and depth. You may use `legacy_send_message` only for depth-1 entries; deeper entries are '
      + 'candidates for `legacy_interrupt_agent` only.',
    parameters: {
      scope: {
        type: 'string',
    ...
    async execute(args, exec) {
      ...
      if (!parent) {
        ...
        throw new Error('legacy_list_agents requires a calling agent (exec.agent was undefined)')
      }
      const request = resolveListAgentsRequest(args)
      ...
      switch (request.scope) {
        ...
        default:
          return assertNever(request.scope, 'legacy_list_agents scope')
      }
    },
  }))
}
