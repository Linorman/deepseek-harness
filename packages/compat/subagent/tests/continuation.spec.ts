...
import * as SubagentFork from '@clocky/clocky-compat-subagent-fork-in-process'
import type { GenerateOptions, MessageId, StreamChunk } from '@clocky/clocky-llm'
...
async function setupWith(adapter: LlmAdapter, options: { persistence?: boolean } = {}) {
  ...
  if (options.persistence !== false) {
    root = mkdtempSync(join(tmpdir(), 'clocky-subagent-continuation-'))
...
describe('continuable settlement delivery', () => {
  ...
  it('reports a turn that failed before reaching its first step', async () => {
    ...
    const { ctx, parent } = await setupWith(adapter)
    // The shipped durability checkpoint (`clocky-session-checkpoint-policy`) is
...
describe('continuable errors', () => {
  ...
  it('unloading the manager drains its live activations', async () => {
    ...
    const ctx = new Context()
    await mountAgentLoopTestDependencies(ctx)
    const root = mkdtempSync(join(tmpdir(), 'clocky-subagent-continuation-'))
    const persistenceFiber = await ctx.plugin(JsonlSessionPersistence, { root })
    cleanups.push(async () => {
      await persistenceFiber.dispose()
