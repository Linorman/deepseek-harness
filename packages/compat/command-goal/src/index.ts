/**
 * Human-facing `/legacy_goal` command over the persisted same-session goal domain.
 * @module @clocky/clocky-compat-command-goal
 */

import type { Context } from '@clocky/cordis'
import type { CommandInvocation, CommandResult } from '@clocky/clocky-commands'
import { GoalError } from '@clocky/clocky-compat-goal'
import type { GoalPhase, GoalRef, GoalView } from '@clocky/clocky-compat-goal'
import { createUserMessage } from '@clocky/clocky-llm'

export const name = 'command-goal'
export const inject = ['commands', 'goals']

const USAGE = 'Usage: /legacy_goal [<objective>|clear|edit <objective>|pause|resume]'
...
/** Parse only the grammar owned by `/legacy_goal`; arbitrary other input is an objective. */
...
function commandHint(goal: GoalView): string {
  if (goal.phase === 'active') {
    return goal.activation === 'armed'
      ? '/legacy_goal edit <objective>, /legacy_goal pause, /legacy_goal clear'
      : '/legacy_goal edit <objective>, /legacy_goal resume, /legacy_goal clear'
  }
  switch (goal.phase) {
    case 'paused':
    case 'blocked':
      return '/legacy_goal edit <objective>, /legacy_goal resume, /legacy_goal clear'
    case 'complete':
      return '/legacy_goal <objective>, /legacy_goal clear'
...
function missingGoal(action: string): CommandResult {
  return {
    kind: 'error',
    text: `No goal is currently set; /legacy_goal ${action} requires one. ${USAGE}`,
  }
}
    ...
    switch (command.kind) {
      ...
      case 'create': {
        if (current !== undefined && current.phase !== 'complete') {
          return {
            kind: 'error',
            text: `A goal is already ${phaseLabel(current.phase)}. Use /legacy_goal edit <objective> to change it or /legacy_goal clear before replacing it.`,
          }
        }
...
/** Register the Codex-shaped `/legacy_goal` command for every composed command adapter. */
