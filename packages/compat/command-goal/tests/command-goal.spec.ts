...
/** Execute `/legacy_goal` through the same registry boundary as a UI adapter. */
...
describe('@clocky/clocky-compat-command-goal registration', () => {
  it('registers one global command with Loader-safe exports and disposes it', async () => {
    ...
    expect(test.ctx.commands.find(test.agent, 'legacy_goal')).toBeDefined()

    await test.plugin.dispose()
    expect(test.ctx.commands.find(test.agent, 'legacy_goal')).toBeUndefined()
  })
})

describe('/legacy_goal human command', () => {
  it('shows an empty status without mutating the session', async () => {
    const test = await harness()
    await expect(run(test)).resolves.toEqual({
      kind: 'success',
      text: 'No goal is currently set.\nUsage: /legacy_goal [<objective>|clear|edit <objective>|pause|resume]',
    })
  ...
  it('creates a trimmed objective and refuses silent replacement of unfinished work', async () => {
    ...
    await expect(run(test, ' replacement')).resolves.toEqual({
      kind: 'error',
      text: 'A goal is already active. Use /legacy_goal edit <objective> to change it or /legacy_goal clear before replacing it.',
    })
  ...
  it('edits inline, requires an objective, and starts a new goal when the old one is complete', async () => {
    ...
    expect(missingEdit.text).toContain('/legacy_goal edit requires one')
  ...
  it('returns direct missing-state results for pause, resume, and clear', async () => {
    ...
    expect(missingPause.text).toContain('/legacy_goal pause requires one')
    const missingResume = await run(test, ' resume')
    expect(missingResume.kind).toBe('error')
    expect(missingResume.text).toContain('/legacy_goal resume requires one')
  ...
  it('pauses, resumes, clears, and converts expected domain rejections to command errors', async () => {
    ...
    expect(redundantResume).toEqual({
      kind: 'error',
      text: 'The goal command is not valid for the current state. Run /legacy_goal to view available commands.',
    })
  ...
  it('shows every durable phase and distinguishes disarmed active state', async () => {
    ...
    expect((await run(test)).text).toContain('/legacy_goal resume')
    ...
    expect(complete.text).toContain('Commands: /legacy_goal <objective>, /legacy_goal clear')
  })
...
describe('/legacy_goal image attachments', () => {
  const PNG = 'AAAA'
  ...
  /** Run /legacy_goal with `count` composer images through the executor boundary. */
  async function runWithImages(test: Harness, suffix: string, count: number) {
    ...
    const execution = await test.ctx.commands.execute(test.agent, `/legacy_goal${suffix}`, images, new AbortController().signal)
    if (execution === undefined) throw new Error('goal command was not registered')
    return execution.result
  }
  ...
  it('rejects attachments on sub-commands that cannot use them, leaving the domain untouched', async () => {
    ...
    for (const suffix of [' pause', '', ' clear']) {
      ...
      expect(result).toEqual({
        kind: 'error',
        text: 'Image attachments only accompany a goal objective: /legacy_goal <objective> or /legacy_goal edit <objective>.',
      })
    }
