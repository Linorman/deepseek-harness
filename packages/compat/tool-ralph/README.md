...
The pending call is a `generic` card titled `legacy_ralph`; the immutable objective is its `rawInput`. The result keeps the generic card. Both presentation functions depend only on tool arguments and the settled tool envelope.
...
The `legacy_ralph` tool definition exposes one required `objective` string and one optional `maxRounds` number. Provider choice, handoff size, report definition, workflow script, and orchestration behavior are deployment-owned and absent from the call definition.
