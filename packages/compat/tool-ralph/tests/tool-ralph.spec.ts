...
import type { SubagentCapabilities, SubagentProvider, SubagentRun, SubagentStartRequest } from '@clocky/clocky-compat-subagent'
...
import type { WorkflowResult, WorkflowRun, WorkflowStartRequest } from '@clocky/clocky-compat-workflow'
...
describe('clocky-tool-ralph', () => {
  ...
  it('registers scoped guidance and pure replay-safe generic presentation', async () => {
    ...
    const section = (await ctx.systemPrompt.assemble()).sections.find(candidate => candidate.name === 'tool:legacy_ralph')
    ...
    const tool = ctx.tools.get('legacy_ralph')!
    ...
    await fiber.dispose()
    expect(ctx.tools.get('legacy_ralph')).toBeUndefined()
    expect((await ctx.systemPrompt.assemble()).sections.some(candidate => candidate.name === 'tool:legacy_ralph')).toBe(false)
  })
