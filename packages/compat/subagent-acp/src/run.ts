 ...
 * @module @clocky/clocky-compat-subagent-acp/run
 */
...
import { AssistantOutputFold } from '@clocky/clocky-compat-subagent'
import type { SubagentResult, SubagentRun, SubagentStartRequest, SubagentStopReason } from '@clocky/clocky-compat-subagent'
...
export interface AcpRunSpec {
   ...
   * Explicit `CLOCKY_*` entries are deployment-owned facts for the child harness
   * (e.g. `CLOCKY_PERMISSION_MODE`); they simply merge after the scrub that
...
export async function startAcpRun(request: SubagentStartRequest, spec: AcpRunSpec): Promise<SubagentRun> {
  ...
  // to the result. The seam's scrub drops ambient credentials and CLOCKY_* names
