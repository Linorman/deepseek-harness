...
const driver = fileURLToPath(new URL(
  '../../../../examples/acp-agent/tests/fixtures/compat/subagent-acp/driver.ts',
  import.meta.url,
))
const configPath = fileURLToPath(new URL(
  '../../../../examples/acp-agent/tests/fixtures/compat/subagent-acp/cordis.yml',
  import.meta.url,
))
...
describe('ACP subagent cwd inheritance through a real cordis.yml', () => {
  it('runs the child in the parent session workspace and announces it as the ACP session cwd', async () => {
    ...
    const { stderr } = await runLoaderSmoke({
      ...
      env: { CLOCKY_TEST_MOCK_ACP_SERVER: mockServer },
      inspect: async (cwd) => {
