...
import { spawnSubprocess } from '@clocky/clocky-subprocess-local/src/spawn.ts'
...
describe('child env layering (through the subprocess seam)', () => {
  ...
  it('forwards explicit CLOCKY_* config entries to the child', async () => {
    // A deployment sets child-harness facts like CLOCKY_PERMISSION_MODE in
    ...
    const ctx = await setup({ MOCK_ECHO_ENV: 'CLOCKY_ACP_TEST_FACT', CLOCKY_ACP_TEST_FACT: 'managed' })
...
describe('disposeAcpChild (the backend-owned teardown ladder over seam verbs)', () => {
  ...
  it('observes a spawn-level rejection and returns without a process to reap', async () => {
    const child = spawnSubprocess({
      ...
      cwd: '/nonexistent-dir-clocky-acp-ladder-test',
      stdio: { stdin: 'ignore', stdout: { maxBytes: 1000 }, stderr: { maxBytes: 1000 } },
      graceMs: 200,
    })
...
describe('cwd resolution', () => {
  ...
  it('resolves a relative config cwd against the launch directory at load', async () => {
    ...
    // ABSOLUTE form — Clocky's own ACP server rejects a relative session cwd, and
    // deferring resolution to spawn would hide the launch-dir dependency.
    const relative = 'packages/compat/subagent-acp'
