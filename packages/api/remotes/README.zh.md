...
`createApiRemoteAgentResolver()` 会复用 live 的普通 Agent、恢复普通冷会话，并对并发恢复去重。带 Team provenance 的身份会在通用解析恢复它们之前以 `team-run-unavailable` 返回；subagent-owned 身份仍保留 `agent-busy` fence。同一个 resolver 服务 Typert `agent` 和 `session` lookup。标准 Web API Proxy 提供 Agent 默认值和 scope 设置，再将返回的 resolver 用于旧的普通 Session 方法。

当前 Client 组合挂载 Goal Remote 贡献和只读 Host 插件清单贡献（`pluginInventory/list`）。该组合卸载时，Cordis effect 的所有权机制会撤回所有贡献；`@clocky/clocky-api-gateway/client` 负责描述符校验、可追踪 namespace Service、直接与作用域方法、调用与取消。Client 入口通过 Cordis 消费共享的 `TypertClientRemote` 接口，不导入具体 Gateway；它只以 type-only 形式重新导出 Gateway Client face 的声明合并，因此消费端经由本外观取到转发事件词汇时，运行时不会多出一条通往 Gateway 实现的边。
...
监听器签名不在此处重写。名单内每条事件的 cordis `Events` 声明都住在其 owner 包 client-safe 的 `./types` 出口（`clocky-agent-presets`、`clocky-commands`、`clocky-credentials`、`clocky-llm`、`clocky-settings`），本包两个 face 都把那些声明纳入编译面，因此「原样转发」是构造性成立的，不需要另立证明。Host face 还额外把名单断言给 `TypertForwardableEvent`：未声明的事件名、绑定 AgentScope 的事件、以及形状不是单向的事件都会在此被拒绝。
