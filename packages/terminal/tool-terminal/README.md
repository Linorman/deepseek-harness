...
The six generated schemas are listed in the [`clocky-tool-terminal` catalog section](../../../docs/tool-catalog.md#clockyclocky-tool-terminal). Their fixed schema tokens are present whenever this plugin is active; agent-scoped tool filtering may hide them.
...
- Background mode requires both `@clocky/clocky-jobs` and its model-facing controller.
