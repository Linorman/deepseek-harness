...
import AgentRegistry, { Inbox, type Agent } from '@clocky/clocky-agent'
import SandboxProvider from '@clocky/clocky-sandbox'
import type { ConfinedArgv, SandboxPolicy } from '@clocky/clocky-sandbox'
import SandboxPolicyService, { setSandboxMode } from '@clocky/clocky-sandbox-policy'
import TerminalSessionService, { TerminalBackendCleanupError, TerminalSessionId } from '@clocky/clocky-terminal'
import type { TerminalSendRequest, TerminalWaitReason } from '@clocky/clocky-terminal'
import { BashTerminalBackend, PWSH_PROMPT_SETUP } from '@clocky/clocky-terminal-bash'
import { ENCODING_PREAMBLE } from '@clocky/clocky-pwsh-local'
import * as ptyLocal from '@clocky/clocky-terminal-bash'
import type { ResolvedConfig } from '@clocky/clocky-terminal-bash/src/config.ts'
import type { LocalPtySession } from '@clocky/clocky-terminal-bash/src/session.ts'
...
describe('BashTerminalBackend startup rollback', () => {
  ...
  it('wraps confined argv, scrubs the environment, and returns initialized sessions', async () => {
    ...
    expect(spawned).toMatchObject({
      ...
      graceMs: 10,
      env: {
        TERM: 'dumb', PAGER: 'cat', GIT_PAGER: 'cat', PS1: 'clocky> ', BASH_SILENCE_DEPRECATION_WARNING: '1',
        PROMPT_COMMAND: 'printf "\\033]133;D;%s\\007" "$?"; PS1=\'clocky> \'',
        CLOCKY_SHELL: '1', CLOCKY_SESSION_ID: 'agent', CLOCKY_PTY_SESSION_ID: 'pty-1',
      },
    })
  ...
  it('composes the default local session around a spawned terminal', async () => {
    ...
    queueMicrotask(() => { output.write(Buffer.from('\x1b]133;D;0\x07clocky> ')) })
    const backend = new BashTerminalBackend(
      ctx,
      config(),
      async () => terminal,
    ...
    expect(session.motd).toBe('clocky> ')
  ...
  it('bootstraps a pwsh dialect through the prompt function and scrubs bash-only env', async () => {
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: 'setup-echo clocky> ', waitReason: 'stdin_read' as const,
    ...
    expect(session.motd).toBe('setup-echo clocky> ')
    expect(spawned?.env).toMatchObject({
      TERM: 'dumb', NO_COLOR: '1', CLOCKY_SHELL: '1', CLOCKY_SESSION_ID: 'agent', CLOCKY_PTY_SESSION_ID: 'pty-1',
    })
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: second ? 'clocky> ' : 'PowerShell 7.6.4\n',
  ...
  it('forwards the spawn signal into the pwsh bootstrap sends', async () => {
    ...
    const session = {
      ...
      startSend: (request: TerminalSendRequest) => {
        ...
        return {
            ...
            viewport: 'clocky> ', waitReason: 'stdin_read' as const,
            sessionStatus: { kind: 'running' as const }, truncated: false,
          }),
          readOutput: () => ({ delta: '', truncated: false }),
          cancel: () => false,
    ...
    expect(spawned.motd).toBe('clocky> ')
