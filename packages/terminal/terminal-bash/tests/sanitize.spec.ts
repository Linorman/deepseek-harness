...
import { normalizeTerminalText, TerminalSanitizer } from '@clocky/clocky-terminal-bash/src/sanitize.ts'
...
describe('TerminalSanitizer', () => {
  it('removes split CSI and owned OSC prompt markers', () => {
    ...
    expect(sanitizer.push('D;0\x07clocky> ')).toEqual({ text: 'clocky> ', prompt: true, promptTail: 'clocky> ' })
  })
  ...
  it('reports printable prompt text that follows a marker in a later chunk', () => {
    ...
    expect(sanitizer.push('clocky> ')).toEqual({ text: 'clocky> ', prompt: false, promptTail: 'clocky> ' })
  })
