 ...
 *
 * `@clocky/clocky-tools`' `ToolExecution` satisfies this shape, so the policy
 * reads `exec` straight through without importing `clocky-tools` or `clocky-agent`.
 * Only the session HEADER id is read — the same identity every other subsystem
 * keys off (see `clocky-tool-bash`'s owner derivation).
 *
 * @module @clocky/clocky-spill-policy/types
 */
