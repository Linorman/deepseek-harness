...
import { scrubbedParentEnv, SubprocessRuntime } from '@clocky/clocky-subprocess'
import type {
  SubprocessHandle,
...
describe('SubprocessRuntime seam', () => {
  ...
  it('scrubbedParentEnv drops credential-shaped and CLOCKY_ names (case-insensitively) but keeps PATH', () => {
    process.env.CLOCKY_SCRUB_PROBE = 'stale'
    process.env.clocky_scrub_probe_lower = 'stale'
    process.env.SCRUB_PROBE_TOKEN = 'secret'
    process.env.SCRUB_PROBE_PASSWORD = 'secret'
    process.env.SCRUB_PROBE_PLAIN = 'visible'
    try {
      const env = scrubbedParentEnv()
      expect(env.CLOCKY_SCRUB_PROBE).toBeUndefined()
      expect(env.clocky_scrub_probe_lower).toBeUndefined()
      ...
      delete process.env.CLOCKY_SCRUB_PROBE
      delete process.env.clocky_scrub_probe_lower
