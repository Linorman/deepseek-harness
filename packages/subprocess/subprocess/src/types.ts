 ...
 * @module clocky-subprocess/types
 */

import type { Readable, Writable } from 'node:stream'

/** Namespace prefix reserved for Clocky-managed child environment facts. */
export const CLOCKY_ENV_PREFIX = 'CLOCKY_' as const

/** One environment key inside the managed {@link CLOCKY_ENV_PREFIX} namespace. */
export type ClockyEnvironmentKey = `${typeof CLOCKY_ENV_PREFIX}${string}`

/** Trusted Clocky variables for one child-process execution. */
export type ClockyEnvironment = Readonly<Record<ClockyEnvironmentKey, string>>
 ...
 * to `TerminalSignal` in `@clocky/clocky-terminal` without a cross-seam dependency;
