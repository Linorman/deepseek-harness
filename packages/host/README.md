...
The host side of the clocky web GUI: the API gateway every client shape shares, and the plain HTTP server it rides on. The browser side lives in [`client/`](../client/README.md); the composed application is [`apps/cli`](../../apps/cli/README.md) booting the [`clocky-base` bundle](../bundle/base/cordis.patch.yml) serving [`apps/web`](../../apps/web/). All **product** packages.

| Package | Role | ctx key |
|---|---|---|
| [`apiproxy/`](apiproxy/README.md) | Shared host API gateway and wire contract | `ctx.apiProxy` |
| [`product-principal-local/`](product-principal-local/README.md) | Local single-user credential provider | registers `ctx.productPrincipals` |
| [`product-principal-digest/`](product-principal-digest/README.md) | Configured SHA-256 digest credential provider | registers `ctx.productPrincipals` |
...
`apiproxy` remains transport-independent; [`client/connection`](../client/connection/README.md) supplies the browser/HTTP carrier. `product-principal-local` supplies the shipped local credential boundary without routing requests itself. Picker implementations replace one another behind the shared seam.
