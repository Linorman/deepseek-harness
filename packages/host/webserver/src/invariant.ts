/**
 * Package-owned invariant companion for `@clocky/clocky-host-webserver`.
 * @module @clocky/clocky-host-webserver/invariant
 */
...
const PACKAGE_NAME = '@clocky/clocky-host-webserver'
...
const install: InvariantInstaller = (ctx, fail) => {
  ctx.on('internal/plugin', () => {
    ...
    const probe = { kind: 'exact' as const, path: '/__clocky_invariant_probe__', handler: () => {} }
    try {
      server.register(probe)()
      server.register(probe)()
      const upgradeProbe = { path: '/__clocky_invariant_upgrade_probe__', handler: () => {} }
