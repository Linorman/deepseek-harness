 ...
 * assertions observe the durable outcome — which backend the chooser mounted
 * into the Loader store, the capability the seam then serves, and that
 * disposing the chooser removes the mounted entry again (HMR safety), joining
 * the backend's own teardown before the disposer settles.
 */
...
import HttpServer from '@clocky/clocky-host-webserver'
import type { DirectoryPicker } from '@clocky/clocky-host-directory-picker'
import BrowseDirectoryPicker from '@clocky/clocky-host-directory-picker-browse'
import NativeDirectoryPicker from '@clocky/clocky-host-directory-picker-native'
...
const AUTO = '@clocky/clocky-host-directory-picker-auto'
const NATIVE = '@clocky/clocky-host-directory-picker-native'
const BROWSE = '@clocky/clocky-host-directory-picker-browse'
...
async function loadComposition(
  ...
  root = await mkdtemp(join(tmpdir(), 'clocky-directory-picker-auto-'))
  const configPath = join(root, 'cordis.yml')
  await writeFile(configPath, [
    "- name: '@clocky/clocky-host-webserver'",
    '  config:',
    `    host: '${bindHost}'`,
    '    port: 0',
    `- name: '${AUTO}'`,
  ...
  const modules = new Map<string, unknown>([
    ['@clocky/clocky-host-webserver', HttpServer],
...
function stubAttendedHost(): void {
  fakeBin = mkdtempSync(join(tmpdir(), 'clocky-picker-bin-'))
