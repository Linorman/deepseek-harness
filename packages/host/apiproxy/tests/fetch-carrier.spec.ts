...
describe('unary round trip (handler ⇄ client, no network)', () => {
  ...
  it('covers Session control and Host describe passthrough', async () => {
    const c = client()
...
describe('resolveBase', () => {
  it('prefers a real location.origin and falls back to the internal authority', async () => {
    ...
    expect(probe.urls[0]).toMatch(/^http:\/\/clocky\.internal\//)
    ...
    try {
      ...
      expect(probe3.urls[0]).toMatch(/^http:\/\/clocky\.internal\//)
    } finally {
