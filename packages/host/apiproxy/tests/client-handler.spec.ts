...
import type { ApiProxy, HostFrame, MuxFrame, RpcMessage, RpcRequest, RpcResponse } from '@clocky/clocky-host-apiproxy'
...
describe('unary round trip', () => {
  ...
  it.each(['create', 'edit', 'pause', 'resume', 'complete', 'clear'])('does not expose the same-Session goal.%s endpoint', async (verb) => {
    const api = scriptedApi()
    expect(api).not.toHaveProperty('goals')
    expect(client(api)).not.toHaveProperty('goals')
    const handler = toFetchHandler(api)
    const response = await handler.fetch(`http://clocky.internal/api/goal.${verb}`, {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ type: 'client-request', rpcId: 'legacy-goal', method: `goal.${verb}`, payload: {} }),
    })
  ...
  it('rejects a malformed envelope as bad-request, salvaging the rpcId or falling back to the sentinel', async () => {
    ...
    const noId = await handler.fetch('http://clocky.internal/api/session.list', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ nonsense: true }) })
    ...
    const withId = await handler.fetch('http://clocky.internal/api/session.list', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ rpcId: 'salvage-me', nonsense: true }) })
  ...
  it('rejects non-JSON media types before executing anything (cross-site simple-request fence)', async () => {
    ...
    const plain = await handler.fetch('http://clocky.internal/api/session.list', { method: 'POST', headers: { 'content-type': 'text/plain' }, body })
    expect(plain.status).toBe(415)
    // A string body with no explicit header defaults to text/plain — same fence.
    const unlabelled = await handler.fetch('http://clocky.internal/api/session.list', { method: 'POST', body })
    expect(unlabelled.status).toBe(415)
    expect(list).not.toHaveBeenCalled()
    // Media-type parameters pass: the fence checks the type, not the exact string.
    const charset = await handler.fetch('http://clocky.internal/api/session.list', { method: 'POST', headers: { 'content-type': 'application/json; charset=utf-8' }, body })
  ...
  it('passes a signal-less doFetch straight through to the handler', async () => {
    ...
    const response = await probe.direct(new URL('http://clocky.internal/probe'))
...
describe('respond path', () => {
  ...
  it('returns bad-response for a malformed client-response without reaching the impl', async () => {
    ...
    const response = await handler.fetch('http://clocky.internal/api/respond', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ type: 'client-response' }) })
...
describe('config unary surface', () => {
  it('round-trips every settings/credentials/llm method with its own payload and value shape', async () => {
    ...
    const group = { id: 'test-provider', name: 'Test Provider', models: [{ id: 'test-model', name: 'Flash' }] }
    const api = scriptedApi({
      settings: {
    ...
    const updated = await c.settings.update({ ns: 'llm-test-adapter', patch: { baseURL: 'https://next' } })
    expect(updated.result).toEqual({ ok: true, value: view })
    const replaced = await c.settings.replace({ ns: 'llm-test-adapter', section: {} })
    ...
    expect(seen[2]?.payload).toEqual({ ns: 'llm-test-adapter', patch: { baseURL: 'https://next' } })
    expect(seen[4]?.payload)
      .toEqual({ ns: 'llm-test-adapter', ops: [{ op: 'unset', path: ['baseURL'] }], expectedRevision: 0 })
