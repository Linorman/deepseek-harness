...
import type { SessionHeader, SessionId } from '@clocky/clocky-session'
import type { SessionLineageNode } from '@clocky/clocky-session-query'
import type { SessionRawArtifact } from '@clocky/clocky-session-persistence'
import ApiProxyService, { createApiProxy, toFetchHandler } from '@clocky/clocky-host-apiproxy'
...
describe('session.export download endpoint', () => {
  it('streams a ZIP with the root artifact verbatim under its original filename', async () => {
    ...
    expect(response.headers.get('content-disposition')).toContain('clocky-session-session-root.zip')
