...
import { withFileLock } from '@clocky/clocky-atomic-write'
...
async function tempDir(): Promise<string> {
  const dir = await mkdtemp(join(tmpdir(), 'clocky-cred-migration-'))
...
const FLAT = [
  ...
  "CLOCKY_CRED_OTHER: 'quoted value'",
  'CLOCKY_CRED_BLOCK: |',
  '  first line',
  '  second line',
  'records: tricky',
...
describe('flat-layout boot migration', () => {
  it('upgrades the flat document in place, byte for byte, and serves its keys', async () => {
    ...
    expect(await ctx.credentials.resolve(credentialRef('CLOCKY_CRED_TEST'))).toEqual({ value: 'stored', source: 'file' })
    expect(await ctx.credentials.resolve(credentialRef('CLOCKY_CRED_OTHER')))
      .toEqual({ value: 'quoted value', source: 'file' })
    expect(await ctx.credentials.resolve(credentialRef('CLOCKY_CRED_BLOCK')))
  ...
  it('yields to a concurrent migrator under the writer lock', async () => {
    ...
    expect(await ctx.credentials.resolve(credentialRef('CLOCKY_CRED_TEST'))).toEqual({ value: 'winner', source: 'file' })
  })
  ...
  it('leaves an empty flow mapping alone', async () => {
    ...
    expect(await ctx.credentials.resolve(credentialRef('CLOCKY_CRED_TEST'))).toBeUndefined()
  })
  ...
  it('renders a final newline for a document that lacks one', () => {
    expect(renderFlatLayoutMigration('CLOCKY_CRED_TEST: bare')).toBe('version: 1\nrefs:\n  CLOCKY_CRED_TEST: bare\n')
  })
})
