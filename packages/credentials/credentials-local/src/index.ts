/**
 * File-backed credentials provider over `$CLOCKY_HOME/.credentials.yaml`, layered
 ...
 * > $CLOCKY_HOME/.env                   (read-only fallback)
 * ```
 *
 * The inherited environment wins because `TEST_API_KEY=… clocky`, a CI
 ...
 * @module @clocky/clocky-credentials-local
 */
...
import { canonicalizeWatchPath, resolveClockyHome } from '@clocky/clocky-home-paths'
import { launchEnvironmentOf } from '@clocky/clocky-launch-environment'
import { CredentialProvider, credentialRef, parseCredentialKey } from '@clocky/clocky-credentials'
import type {
  ApiKeyRecord,
  CredentialInfo,
  CredentialKey,
...
import type { LaunchEnvironmentEntry } from '@clocky/clocky-launch-environment'
...
export function resolveSpec(config: Config): ResolvedSpec {
  return {
    filename: resolve(config.path ?? join(resolveClockyHome(config.clockyHome), CREDENTIALS_FILENAME)),
 ...
 * holds it. Like the retry cadence in `clocky-atomic-write`, this is a
...
/** File-backed credentials provider (`$CLOCKY_HOME/.credentials.yaml`). */
  ...
  private assertUnshadowed(ref: CredentialRef, verb: 'set' | 'unset'): void {
    if (this.inherited(ref) !== undefined) {
      throw new Error(
        ...
        + ' shadowed; unset it in the shell you start clocky from instead',
      )
    }
  }
