...
import * as StorageDomain from '@clocky/clocky-storage-domain'
import * as StorageJson from '@clocky/clocky-storage-json'
import { remoteMethods } from '@clocky/clocky-typert-protocol'
...
async function loadComposition(configPath: string): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@clocky/clocky-session-persistence-jsonl', JsonlSessionPersistence],
    ['@clocky/clocky-storage', Storage],
    ['@clocky/clocky-storage-json', StorageJson],
    ['@clocky/clocky-storage-domain', StorageDomain],
    ['@clocky/clocky-message-feedback', MessageFeedbackService],
  ])
  ctx.loader.internal = {
    version: 'v2',
...
describe('message feedback through a real Loader composition', () => {
  it('persists a checkpointed target and its sidecar across a cold restart', async () => {
    root = await mkdtemp(join(tmpdir(), 'clocky-message-feedback-loader-'))
    ...
    await writeFile(configPath, [
      ...
      '  config:',
      '    backend: json',
      "- name: '@clocky/clocky-message-feedback'",
      '  config:',
      '    maxNoteBytes: 32',
      '',
    ].join('\n'))
