...
export class WebRuntime extends Service {
   ...
   * `$CLOCKY_WEB_SEARCH_PROVIDER` / `$CLOCKY_WEB_FETCH_PROVIDER` are equivalent to
  ...
  constructor(ctx: Context, config: WebRuntimeConfig = {}) {
    super(ctx, 'web')
    this.searchProviderId = config.searchProvider ?? process.env.CLOCKY_WEB_SEARCH_PROVIDER
    this.fetchProviderId = config.fetchProvider ?? process.env.CLOCKY_WEB_FETCH_PROVIDER
  }
