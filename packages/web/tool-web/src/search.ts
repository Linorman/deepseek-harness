...
import type { GenericCallView, JsonValue, ToolResult, WebSearchResultView, WebSource } from '@clocky/clocky-tools'
import type { WebSearchResult, WebSearchSource } from '@clocky/clocky-web'
import type {} from '@clocky/clocky-system-prompt'

/**
 * Default upper bound on returned sources (the `searchMaxResults` config).
 * Owned by the consumer (not the provider or model), mirroring `clocky-tool-fs`'s
