...
} from '@clocky/clocky-typert-registry'
import type {
...
describe('TypertRegistry', () => {
  it('registers and queries generated schemas separately from package reflection', async () => {
    ...
    expect(typertKey('@clocky/clocky-tools', 'ToolInput')).toBe('@clocky/clocky-tools#ToolInput')
    expect(typertPackageKey('@clocky/clocky-tools', 'host')).toBe('@clocky/clocky-tools#host')
    expect(ctx.typert.get('@clocky/clocky-tools#ToolInput')).toMatchObject({
      package: '@clocky/clocky-tools',
      face: 'host',
      name: 'ToolInput',
    })
    expect(ctx.typert.get('@clocky/clocky-tools#ToolInput')?.schema).toBe(contribution.schemas[0]?.schema)
    expect(ctx.typert.getPackage('@clocky/clocky-tools', 'host')).toMatchObject({
      key: '@clocky/clocky-tools#host',
  ...
  it('withdraws schemas and package metadata through the exact contribution disposer', async () => {
    ...
    expect(ctx.typert.getPackage('@clocky/clocky-tools')).toBeDefined()

    await dispose()

    expect(ctx.typert.get('@clocky/clocky-tools#ToolInput')).toBeUndefined()
    expect(ctx.typert.getPackage('@clocky/clocky-tools')).toBeUndefined()
  ...
  it('rejects duplicate package faces and schema keys before committing', async () => {
    ...
    expect(ctx.typert.get('@clocky/clocky-tools#ToolInput')?.schema).toBe(original.schemas[0]?.schema)
  ...
  it('resolves required schemas and projects fresh JSON Schema documents', async () => {
    ...
    expect(ctx.typert.resolve('@clocky/clocky-tools#ToolInput').name).toBe('ToolInput')
    expect(() => ctx.typert.resolve('@clocky/clocky-tools#Missing')).toThrow('contributes no schema named "Missing"')
    ...
    const projected = ctx.typert.toJSONSchema('@clocky/clocky-tools#ToolInput')
    expect(projected).toMatchObject({ type: 'object', properties: { name: { type: 'string' } } })
    expect(ctx.typert.toJSONSchema('@clocky/clocky-tools#ToolInput')).not.toBe(projected)
  })
