...
import type { TypertContribution } from '@clocky/clocky-typert-registry/types'
import { EVENT_API, SERVICE_API, TYPE_API } from '@clocky/clocky-tool-cordis/src/api-catalog.ts'
...
describe('model-driven clocky-tools generation', () => {
  it('round-trips the complete service and event structure through the runtime registry', { timeout: 30_000 }, async () => {
    const workspace = new WorkspaceAnalyzer({
      root: workspaceRoot,
      faces: ['host'],
      packages: ['@clocky/clocky-tools'],
    }).analyze()
    const host = workspace.faces.find(candidate => candidate.face === 'host')
    if (host === undefined) throw new Error('clocky-tools has no analyzed host face')
    const artifact = new FaceModelEmitter(host).emit('@clocky/clocky-tools')
    ...
    const record = ctx.typert.getPackage('@clocky/clocky-tools', 'host')
    ...
    await dispose()
    expect(ctx.typert.getPackage('@clocky/clocky-tools', 'host')).toBeUndefined()
  })
})
