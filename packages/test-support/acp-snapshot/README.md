...
- **Session harvest requires raw JSONL mode** — `runScenario` collects the Team coordinator's persisted `.jsonl` log when Team mode is selected, so snapshot configs set `persistenceCompression: 'none'`; compressed JSONL and SQLite compositions have no snapshot-harvest path.
- **Built mode requires current artifacts** — run `pnpm run build` before selecting `CLOCKY_EXAMPLE_MODE=lib`; source mode remains the zero-build path.
