 ...
 *
 * @module @clocky/clocky-acp-snapshot/harness
 */
...
export interface RunOptions {
   ...
   * them to `clocky-llm-replay` via `$CLOCKY_SNAPSHOT_CHILD_FILES` so each child
...
export function snapshotSpillRoot(
  ...
  return `${root}/clocky-acp-snap-${key}`
}
...
export async function runScenario(input: InputScript, opts: RunOptions): Promise<RunResult> {
  ...
  const outcome = await (async (): Promise<RunResult> => {
    ...
    const env: NodeJS.ProcessEnv = {
      ...opts.env,
      CLOCKY_SNAPSHOT: opts.mode,
      CLOCKY_SNAPSHOT_FILE: opts.fixtureFile,
      CLOCKY_SNAPSHOT_SESSIONS_ROOT: sessionsRoot,
      CLOCKY_SNAPSHOT_SPILL_ROOT: spillRoot,
      CLOCKY_HOME: join(cwd, '.clocky'),
      CLOCKY_AGENTS_HOME: join(cwd, '.agents'),
      ...opts.overrideFile !== undefined ? { CLOCKY_SNAPSHOT_OVERRIDE: opts.overrideFile } : {},
      ...opts.childFiles !== undefined && opts.childFiles.length > 0
        ? { CLOCKY_SNAPSHOT_CHILD_FILES: opts.childFiles.join(delimiter) }
        : {},
    }
...
/** Retain the scenario diagnostic even when an asynchronous first check outlives the deadline. */
async function waitForSnapshotCondition(message: string, timeoutMs: number, check: () => Promise<void>): Promise<void> {
  try { await vi.waitFor(check, { interval: WAIT_POLL_INTERVAL_MS, timeout: timeoutMs }) }
...
async function waitForPersistedTurnEnd(
  ...
  await waitForSnapshotCondition(timeoutMessage, timeoutMs, async () => {
    const log = (await harvestSessionLogs(root)).find(candidate => candidate.id === sessionId)
...
async function harvestSessionLogs(root: string): Promise<HarvestedLog[]> {
  ...
  // order must match the replay load order in clocky-llm-replay's loadSessionScripts
