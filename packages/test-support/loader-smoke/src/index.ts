 ...
 * zero-build dev path, resolving `@clocky/clocky-*` / `@cordisjs/*` through the tsconfig `paths`
 ...
 *
 * @module @clocky/clocky-loader-smoke
 */
...
export const EXAMPLE_MODE_ENV = 'CLOCKY_EXAMPLE_MODE'
 ...
 * @param raw - the raw value; defaults to `process.env.CLOCKY_EXAMPLE_MODE`.
...
export interface LoaderSmokeOptions {
  ...
  readonly mode?: ExampleMode
  /** Environment overrides layered over the parent and isolated Clocky homes. */
...
export async function runLoaderSmoke(options: LoaderSmokeOptions): Promise<LoaderSmokeResult> {
  ...
  try {
    ...
    const launch = resolveExampleLaunch({
      ...
      env: { CLOCKY_HOME: join(cwd, '.clocky'), CLOCKY_AGENTS_HOME: join(cwd, '.agents'), ...options.env },
    })
