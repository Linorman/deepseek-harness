 ...
 * @module @clocky/clocky-llm-replay
 */

import { existsSync, readFileSync, writeFileSync } from 'node:fs'
import { delimiter as pathDelimiter } from 'node:path'
import type { Context } from '@clocky/cordis'
import type {} from '@clocky/clocky-compaction'
import { decodeStorageRecord, type SessionEvent } from '@clocky/clocky-session'
import type {
  ContentBlock,
  GenerateOptions,
  LlmModelInfo,
...
import { LlmAdapter, LlmError, ReasoningEffortId, assertNever, resolveRetryPolicy } from '@clocky/clocky-llm'
...
export function apply(ctx: Context, config: Config = {}): void {
  const file = config.file ?? process.env.CLOCKY_SNAPSHOT_FILE
  if (file === undefined || file.length === 0) {
    throw new Error('llm-replay: a fixture path is required (Config.file or $CLOCKY_SNAPSHOT_FILE)')
  }
  validateConfiguredModalities(config.providers)
  const overrideFile = config.overrideFile ?? process.env.CLOCKY_SNAPSHOT_OVERRIDE
  const childEnv = process.env.CLOCKY_SNAPSHOT_CHILD_FILES
