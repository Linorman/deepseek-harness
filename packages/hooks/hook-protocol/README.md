# @clocky/clocky-hook-protocol

English | [中文](README.zh.md)

The shared core of a shell-hook wire protocol. NOT a Cordis plugin — it registers nothing and injects nothing. It is a library of dialect-neutral primitives for bridge implementations.
...
| Concern | Here (`clocky-hook-protocol`) | A bridge implementation |
|---|---|---|
...
Hook invocation/result records must sit inside an open turn. `UserPromptSubmit`, `PreToolUse`, `PostToolUse`, and `Stop` satisfy that owner-defined relation by construction. `SessionStart` runs before turn 1 and gets no `hook/*` record; its allowed context remains pending in the inbox until a waking delivery opens a turn — see the interception extension-points Agent Note.

## Model Experience

Indirectly, through bridge implementations that turn parsed hook output into prompt context, blocked outcomes, or continuation feedback.
