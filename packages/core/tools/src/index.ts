 ...
 * execution pipeline.
 * @module @clocky/clocky-tools
 */

import { Context, Service } from '@clocky/cordis'
import z from '@clocky/schemastery'
import { AnonymousEntries, NamedEntries, ScopedLayers, scopeOf, scopeTarget } from '@clocky/clocky-scope'
import type { ScopeKey, ScopeLayer, Scoped } from '@clocky/clocky-scope'
import type { CallId, ContentBlock, ToolSchema } from '@clocky/clocky-llm'
import { assertNever, deepFreeze, HarnessError } from '@clocky/clocky-llm'
import type { Agent } from '@clocky/clocky-agent'
import { snapshotJsonValue } from '@clocky/clocky-session'
import type { JsonValue, UserMessage } from '@clocky/clocky-session'
import type { ToolProviderResult } from '@clocky/clocky-system-prompt'
import type { CodeRuntime } from '@clocky/clocky-code-runtime'
 ...
 * instead of deriving them: the seam's `clocky-code-runtime` README pair, its
...
export type { JsonValue } from '@clocky/clocky-session'
...
// lives in its own UI-facing module; re-export it so `@clocky/clocky-tools`
...
declare module '@clocky/cordis' {
  ...
  interface Events {
     ...
     * Scope-filtered dispatch (`@clocky/clocky-scope`): agent-scoped listeners receive only that agent's calls.
     ...
     * Scope-filtered dispatch (`@clocky/clocky-scope`): agent-scoped listeners receive only that agent's dispatches.
     ...
     * Scope-filtered dispatch (`@clocky/clocky-scope`): keyed by `exec.agent`.
...
export interface ToolDefinition extends ToolSchema {
   ...
   * Enforced by `@clocky/clocky-tool-call-timeout-policy` (a `tools/execute` wrapper); it
 ...
 * @internal
 */
export const TOOL_RUNTIME_SCHEDULER: unique symbol = Symbol('@clocky/clocky-tools.scheduler')
...
export class ToolRuntime extends Service {
  ...
  /** Internal staged view consumed by `clocky-agent-loop`'s parallel scheduler. */
  ...
  private sdkSection(): { name: string; order: number; text: (context: { scope?: ScopeKey }) => string } {
    return {
      ...
      text: (context) => {
        ...
        if (render === undefined) throw new Error(`clocky-tools: no SDK renderer for ${runtime.language}`)
  ...
  private requireCodeRuntime(mode: ToolPresentationMode): CodeRuntime {
    ...
    if (!runtime) {
      throw new Error(`clocky-tools: mode "${mode}" requires a code runtime — load a ctx.codeRuntime implementation (e.g. @clocky/clocky-code-runtime-worker-thread) or set tools mode to "native"`)
    }
    if (!Object.hasOwn(SDK_RENDERERS, runtime.language)) {
      const known = Object.keys(SDK_RENDERERS).map(name => JSON.stringify(name)).join(', ')
      throw new Error(`clocky-tools: no SDK renderer registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
    }
    return runtime
  }
   ...
   * `clocky-agent-tool-presentation` exists for, and reading the deployment default would
...
function createExecutionToken(): ToolExecutionToken {
  return Symbol('clocky.tool.execution') as ToolExecutionToken
}
