 ...
 * @module @clocky/clocky-tools/src/code-mode
 */

import { CallId, createUserMessage, HarnessError } from '@clocky/clocky-llm'
import type { ContentBlock } from '@clocky/clocky-llm'
import type { CodeBindingFunction, CodeRunResult, CodeRuntime } from '@clocky/clocky-code-runtime'
...
function resolveFlavor(peekRuntime: () => CodeRuntime | undefined): RunCodeFlavor {
  ...
  if (!Object.hasOwn(RUN_CODE_FLAVORS, runtime.language) || flavor === undefined) {
    ...
    throw new Error(`clocky-tools: no run_code schema flavor registered for runtime language ${JSON.stringify(runtime.language)} (known: ${known})`)
  }
  return flavor
}
