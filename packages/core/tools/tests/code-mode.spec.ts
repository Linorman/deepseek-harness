...
import type { Scope } from '@clocky/clocky-scope'
...
import ToolRuntime, { CodeRunFailedError, RUN_CODE_NAME, TOOL_ABORTED_BEFORE_DISPATCH, defineContentToolFixture, defineTool } from '@clocky/clocky-tools'
import type { Config, JsonSchemaNode, PostToolDecision, ToolExecutionResult } from '@clocky/clocky-tools'
import type { Agent } from '@clocky/clocky-agent'
import { Session, SessionId } from '@clocky/clocky-session'
import type { JsonValue, SessionEventMap } from '@clocky/clocky-session'
...
describe('per-agent presentation', () => {
  ...
  it('inherits a STANDING preset scope\'s mode down the chain, agents beside it unaffected', async () => {
    const { bindScopeParent } = await import('@clocky/clocky-scope')
    const { ctx, systemPrompt } = await setup({ mode: 'native' })
    const calls = registerEcho(ctx)
    ...
    // `clocky-agent-tool-presentation` produces.
