 ...
 *
 * @module @clocky/clocky-agent
 */

import { Context, FiberState, getTraceable, Service, symbols } from '@clocky/cordis'
import type { Fiber } from '@clocky/cordis'
import { AsyncLocalStorage } from 'node:async_hooks'
import { isPromise } from 'node:util/types'
import { scopeTarget } from '@clocky/clocky-scope'
import type { Scoped } from '@clocky/clocky-scope'
import type { SessionEvent, SessionId } from '@clocky/clocky-session'
import type { TypertContext, TypertLookup } from '@clocky/clocky-typert-protocol'
...
declare module '@clocky/cordis' {
  interface Context {
     ...
     * `clocky-scope` tag while retaining it, so this field is DX context rather
 ...
 * via {@link AgentRegistry.setFactory}. Kept on the `clocky-agent` interface so
 * consumers (e.g. the ACP bridge) program against `ctx.agents` without
 * depending on the concrete `clocky-agent-loop` package.
 */
 ...
 * (`@clocky/clocky-agent-loop`), registered via {@link setFactory}.
 *
  ...
  constructor(ctx: Context) {
    ...
    ctx.inject(['typert'], (typeCtx) => {
      typeCtx.typert.lookups.register('agent', {
        parameter: 'agent',
        wire: 'agentId',
        hostTypeSymbol: '@clocky/clocky-agent#Agent',
