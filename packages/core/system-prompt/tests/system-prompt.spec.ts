...
import SystemPrompt, { AssembleContext, PromptAssembly, renderContextSnapshot, renderPrompt } from '@clocky/clocky-system-prompt'
...
const IDENTITY = 'You are an AI agent running in a plugin-based harness.'
...
describe('SystemPrompt', () => {
  describe('built-in sections', () => {
    it('registers the harness identity and the configured deployment persona', async () => {
      ...
      await ctx.plugin(SystemPrompt, { persona: 'You are a test harness.' })
  ...
  it('assembles sections in order with context-resolved text and collected tools', async () => {
    ...
    expect(assembly.sections.map(s => s.text)).toEqual([IDENTITY, 'You are a test harness.', 'Be precise.', 'cwd: /tmp'])
    expect(assembly.contexts).toEqual([
