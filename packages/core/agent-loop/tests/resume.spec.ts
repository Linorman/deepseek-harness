...
import SessionStore, { SESSION_FORMAT_VERSION, Session, SessionId, SessionPreparation } from '@clocky/clocky-session'
...
async function persistentHarness(adapter: MockAdapter): Promise<{ ctx: Context; root: string }> {
  const root = await mkdtemp(join(tmpdir(), 'clocky-resume-'))
  dirs.push(root)
...
describe('the session-persistence Agent Note: AgentLoop factory create/resume', () => {
  ...
  it('resume of a forked session preserves the lineage and seed boundary in the header', async () => {
