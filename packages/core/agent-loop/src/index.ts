 ...
 *
 * @module @clocky/clocky-agent-loop
 */
...
import { SessionId, SessionPreparation } from '@clocky/clocky-session'
import type { Session, SessionHeader } from '@clocky/clocky-session'
...
export class AgentLoop extends Service implements AgentFactory {
  ...
  async resume(ownerCtx: Context, options: ResumeAgentOptions): Promise<AgentHandle> {
    ...
    if (persistence === undefined) {
      throw new Error('cannot resume: session persistence is not configured (load a clocky-session-persistence backend)')
    }
