...
describe('local attachment service', () => {
  ...
  it('saves and reads through the service boundary', async () => {
    const clockyHome = await mkdtemp(join(tmpdir(), 'clocky-attachment-service-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { clockyHome })
      ...
      await rm(clockyHome, { recursive: true, force: true })
    }
  })

  it('commits a fully prepared image batch in input order', async () => {
    const clockyHome = await mkdtemp(join(tmpdir(), 'clocky-attachment-batch-success-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { clockyHome })
  ...
  it.each([3, 4] as const)('admits a 16-bit %s-channel PNG as an 8-bit normalized object', async (channels) => {
    const clockyHome = await mkdtemp(join(tmpdir(), 'clocky-attachment-16-bit-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { clockyHome })
  ...
  it('prepares every batch member before any write', async () => {
    const clockyHome = await mkdtemp(join(tmpdir(), 'clocky-attachment-batch-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { clockyHome, normalizedImageMaxBytes: 1 })
  ...
  it('validates without persisting: a rejected image leaves no storage root behind', async () => {
    const clockyHome = await mkdtemp(join(tmpdir(), 'clocky-attachment-validate-'))
    try {
      const service = new LocalAttachmentStore(new Context(), { clockyHome })
      ...
      const limited = new LocalAttachmentStore(new Context(), { clockyHome, maxImageBytes: 1 })
