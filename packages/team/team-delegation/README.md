...
The Consumer uses bounded event-driven drives and restart discovery. `maxOperationsPerDrive` bounds progress for one parent, `channelPageSize` bounds child-result reads, `teamPageSize` bounds Team discovery per pulse, and `pulseIntervalMs` controls the restart-safe discovery interval. `drive()` resolves after one bounded round; continued progress and coalesced events queue at most one later round per parent. Close cancels queued rounds, and failed rounds wait for another event, explicit drive or discovery pulse. Terminal child snapshots evict parent/channel routes while their ancestry still wakes parent settlement; stalled children retain routes. Discovery does not retain terminal history, and close releases the remaining route caches. Parent delegation proofs, child-creation proofs, channel-consent proofs, request-post proofs, and child-result proofs are retained only while their selected operation is live.
