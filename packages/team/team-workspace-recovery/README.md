...
Durable `release-requested` events trigger a coalesced, single-flight drive even without a pulse. A drive observes the configured Team bound, attempts every selected allocation despite sibling failures, and reports their aggregate after settlement. Unload stops event and timer admission, waits for accepted provider work and its durable confirmation, then revokes the proof source. A non-advancing Team-list continuation fails the drive with `TEAM_CURSOR_CONFLICT` instead of repeating the same page.
