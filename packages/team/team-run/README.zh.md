...
对于包含多个可并行 workstream 的 non-trivial objective，coordinator prompt 要求在等待前至少启动两个 worker task；适用范围包括 research、analysis、writing、planning、data、operations、coding、testing 及其组合。不修改 filesystem 的 task 保持空 scope；shared workspace writer 使用窄且互不重叠的 relative scope，只有一个 worker 可以拥有共享 artifact 时，则为其他 worker 安排只读 research／analysis／review task。重叠的 write scope 会由 scheduler 有意串行化。
...
`listDefaultWorkerTasks()`会返回通过该 coordinator authority 准入的每个非 workflow task 的紧凑 phase。`watchDefaultWorkerTasks()`会等待 Team cursor 前进，并用新 cursor 返回同样有界的 snapshot；其 signal 只会停止这一次本地 watch。
Provider 返回重复或回退的 Team cursor 时，本地 wait 会以 `TEAM_RUN_NOT_QUIESCENT` 失败，而不会重复提交同一 watch。
