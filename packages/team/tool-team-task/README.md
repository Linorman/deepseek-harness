...
`team_worker_pool_set(worker_count)` lets the coordinator choose the desired worker-pool size. For any non-trivial objective with multiple feasible workstreams—research, analysis, writing, planning, data, operations, coding, testing, or a mix—the coordinator should set at least two before starting independent tasks. TeamRun clamps it to the configured ceiling, activates new workers, retires idle excess workers, and returns requested/target/active/idle/busy/queued counts with `saturated: true` when the ceiling or current capacity is reached. This operation never waits for a busy worker; ready tasks can be admitted and remain queued for the scheduler.

`team_task_start(subject, instructions, read_scopes?, write_scopes?)` creates one bounded default-worker task. The caller supplies a concise subject, complete instructions, the expected deliverable and validation, and optional filesystem regions. For a non-trivial objective with multiple feasible workstreams, the coordinator should start at least two independent tasks before waiting. Use narrow, non-overlapping workspace-relative file scopes only for concurrent shared-workspace writers; leave scopes empty for research, analysis, writing, planning, data, operations, or review tasks that do not touch files, and pair a single safe writer with read-only work when one worker must own a shared artifact. The tool converts an absolute path under the coordinator's current workspace to the workspace-relative form, rejects paths outside it, and canonicalizes the workspace root as `.`. Omitted scope arrays become explicit empty arrays. The result contains `task_id`, the current task `phase`, and the review facts described below.

`team_task_wait(task_id)` waits for a task created through the same coordinator capability, including any configured review. It returns a compact terminal result: a completion summary, a retained failure code and message when available, or the terminal phase. Cancelling the tool call cancels only this local wait; it does not cancel task execution. The result excludes worker transcripts, activation details, Team identities, and lease facts.

`team_task_list()` returns the current compact phase for every non-workflow task created through this coordinator capability. `team_task_watch(after_cursor?)` waits for a Team cursor advance and returns the same bounded task snapshot with the new cursor; cancelling the call stops only this local watch. These operations expose phase and review facts without worker transcripts or lease progress.
...
`team_workflow_start(plan)` admits one complete JSON-serializable `TeamWorkflowPlan`. Each task template uses the same workspace-relative scope rule as `team_task_start`: an absolute path under the coordinator's current workspace is converted, and an outside path is rejected before admission. The plan contains explicit task templates and plan-local dependencies, bounded concurrency/attempt limits, a role-based versioned workflow channel graph, and a task-result projection. TeamRun validates the complete graph before creating any durable task or workflow channel, and returns a stable `plan_id`.
...
The scoped [`team_task_wait`](../../../docs/tool-catalog.md#team_task_wait) schema, followed by a compact terminal phase, completion summary or retained failure fact, and the policy and matching attempt review decision. Worker transcripts, activation identifiers, Team identifiers, and leases stay outside the model-visible result.

#### Token effect

One coordinator-scoped schema and one compact result per completed wait. No other Agent receives the schema.
