...
当 task Agent 的 turn 以 provider 的 `max-tokens` reason 结束时，client 会重新读取准确的 running lease，并排队一条有界 continuation，指出同一个 task 与 attempt，要求 Agent 调用 `team_task_report`。task terminal 或 activation 替换会清除 continuation budget；连续截断达到 `maxTaskOutputContinuations` 后会将仍在运行的 attempt 结算为有界 worker failure，不会形成无限唤醒循环，也不会等到 lease expiry。
