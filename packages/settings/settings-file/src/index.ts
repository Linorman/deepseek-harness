 ...
 * leaf-level diff.
 * @module @clocky/clocky-settings-file
 */
...
import { SettingsProvider, deepEqualJson, type SettingsNamespace } from '@clocky/clocky-settings'
...
export function resolveSpec(config: Config): ResolvedSpec {
  const filename = resolve(config.path ?? join(resolveClockyHome(config.clockyHome), 'settings.yaml'))
