 ...
 * process-local registry lives in `@clocky/clocky-jobs-local`.
 * @module @clocky/clocky-jobs
 */
...
export abstract class JobRegistry extends Service {
  constructor(ctx: Context) {
    ...
    if (new.target === JobRegistry) {
      throw new Error('@clocky/clocky-jobs is the abstract job registry seam; load an implementation such as @clocky/clocky-jobs-local instead')
    }
    super(ctx, 'jobs')
  }
