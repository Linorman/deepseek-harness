 ...
 * @module @clocky/clocky-jobs-local
 */

import { Context } from '@clocky/cordis'
import z from '@clocky/schemastery'
import type { Agent } from '@clocky/clocky-agent'
import { AnonymousEntries, ScopedLayers, scopeOf } from '@clocky/clocky-scope'
import type { ScopeLayer } from '@clocky/clocky-scope'
import { deadline, timeoutOf } from '@clocky/clocky-timeout'
import { JobRegistry, JobId } from '@clocky/clocky-jobs'
import type {
  JobDoneListener, JobKind, JobOutcome, JobRead, JobSnapshot, JobStart, JobStatus,
  JobsChangedListener,
} from '@clocky/clocky-jobs'
 ...
 * `@clocky/clocky-jobs` for the ownership, isolation, and lifecycle
...
export class LocalJobRegistry extends JobRegistry {
  ...
  start(spec: JobStart): JobId {
    if (!this.servesOwner(spec.owner)) {
      throw new Error('background jobs unavailable: no job controller serves this agent (load @clocky/clocky-tool-jobs in its composition)')
    }
  ...
  private ensureOwnerCleanup(owner: Agent): void {
    ...
    if (agents === undefined) {
      throw new Error('background job ownership requires the agent registry (load @clocky/clocky-agent)')
    }
