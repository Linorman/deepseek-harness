...
import { createUserMessage, CallId , createMessage, createToolResultMessage } from '@clocky/clocky-llm'
import SessionStore, { Session, SessionId } from '@clocky/clocky-session'
import SessionQueryEngine from '@clocky/clocky-session-query'
...
} from '@clocky/clocky-session-reference'
...
describe('session reference URI and inline mentions', () => {
  it('round-trips arbitrary session ids and replaces mentions with readable labels', () => {
    ...
    expect(parseSessionReferenceText('what is a clocky-session: URI?')).toEqual({
      text: 'what is a clocky-session: URI?',
      references: [],
    })
    expect(parseSessionReferenceText('see clocky-session:%%%')).toEqual({
      text: 'see clocky-session:%%%',
      references: [],
    })
  })
  ...
  it('rejects malformed explicit references and base64url-shaped bare candidates', () => {
    ...
    expect(() => parseSessionReferenceText('see clocky-session:IiJ')).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
    expect(() => parseSessionReferenceText('@[bad](clocky-session:%%%)')).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
    const nonString = `clocky-session:${Buffer.from(JSON.stringify({ id: 'x' })).toString('base64url')}`
    expect(() => decodeSessionReferenceUri(nonString)).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
    expect(() => decodeSessionReferenceUri('clocky-session:IiJ')).toThrow(expectCode('SESSION_REFERENCE_INVALID_REFERENCE'))
  })
})
...
describe('session reference discovery and preparation', () => {
  ...
  it('does not prepare a rejected pre-step and rejects malformed direct mentions', async () => {
    ...
    const malformed = createUserMessage({
      content: [{ type: 'text', text: '@[bad](clocky-session:not-canonical)' }],
      source: { kind: 'user' },
    })
