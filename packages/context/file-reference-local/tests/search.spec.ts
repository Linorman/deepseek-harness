...
async function workspace(): Promise<string> {
  const root = await mkdtemp(join(tmpdir(), 'clocky-file-autocomplete-'))
  roots.push(root)
...
describe('WorkspaceFileSearch', () => {
  it('lists live directory levels, descends, quotes spaces, and filters hidden/excluded entries', async () => {
    ...
    expect(await files.list('~/.clocky-file-autocomplete-missing/', signal)).toEqual([])
    expect(await files.list('../', signal)).toEqual([])
  ...
  it('does not traverse directory symlinks during direct completion', async () => {
    const root = await workspace()
    const outside = await mkdtemp(join(tmpdir(), 'clocky-file-autocomplete-outside-'))
    roots.push(outside)
