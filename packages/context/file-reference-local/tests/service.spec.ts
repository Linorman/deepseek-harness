...
import ToolRegistry, { defineContentToolFixture } from '@clocky/clocky-tools'
import { FILE_REFERENCE_PROMPT } from '@clocky/clocky-file-reference'
...
async function stubAgent(
  ...
  const root = await mkdtemp(join(tmpdir(), 'clocky-file-reference-service-'))
  roots.push(root)
