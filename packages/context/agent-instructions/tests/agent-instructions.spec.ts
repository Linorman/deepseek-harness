...
import LlmRuntime, { createUserMessage, CallId, type Message, type StreamChunk } from '@clocky/clocky-llm'
import SessionStore, { Session, SessionId, SESSION_FORMAT_VERSION, type SessionEvent, type UserMessage } from '@clocky/clocky-session'
import AgentRegistry, { agentEvents, Inbox, type Agent } from '@clocky/clocky-agent'
import AgentLoop from '@clocky/clocky-agent-loop'
import { FileSystem, FsTargetKey, FsVersion } from '@clocky/clocky-fs'
import type {
  FsDirEntry,
  FsEditOutcome,
  FsEditRequest,
...
import type {
  ToolExecution,
  ToolExecutionToken,
} from '@clocky/clocky-tools'
import * as ToolFs from '@clocky/clocky-tool-fs'
import {
  discoverBaselineInstructionFiles,
  loadBaselineInstructions,
  renderWorkspaceContext,
} from '@clocky/clocky-agent-instructions'
import {
...
async function tempRepo(): Promise<string> {
  return mkdtemp(join(tmpdir(), 'clocky-workspace-context-'))
}
...
describe('workspace context instruction discovery', () => {
  ...
  it('loads user-global first, then every root-to-cwd candidate in precedence order', async () => {
    ...
    try {
      ...
      const files = await discoverBaselineInstructionFiles({ cwd, clockyHome: home })

      expect(files.map(file => file.displayPath)).toEqual([
        '$CLOCKY_HOME/AGENTS.md',
        'AGENTS.md',
        'CLAUDE.md',
  ...
  it('re-reads content after a same-version, same-size rewrite', async () => {
    ...
    try {
      ...
      expect(await loadBaselineInstructions({ cwd, clockyHome: home, maxBytes: 65536 })).toBeUndefined()

      const leaf = join(cwd, 'AGENTS.md')
      await write(leaf, 'first')
      const first = await loadBaselineInstructions({ cwd, clockyHome: home, maxBytes: 65536 })
      expect(first?.text).toContain('first')
      const again = await loadBaselineInstructions({ cwd, clockyHome: home, maxBytes: 65536 })
      ...
      const second = await loadBaselineInstructions({ cwd, clockyHome: home, maxBytes: 65536 })
  ...
  it.skipIf(process.platform === 'win32')('skips a file that becomes unreadable after discovery without failing the request', async () => {
    ...
    try {
      ...
      const loaded = await loadBaselineInstructions({ cwd, clockyHome: home, maxBytes: 65536 })
  ...
  it('follows a symlinked instruction file to its target content', async () => {
    ...
    try {
      ...
      const loaded = await loadBaselineInstructions({ cwd: root, clockyHome: home, maxBytes: 65536 })
  ...
  it('follows a symlinked instruction file through ctx.fs to its target content', async () => {
    ...
    try {
      ...
      await mountWorkspaceContext(ctx, { clockyHome: home, maxBytes: 65536 })
  ...
  it('disables baseline loading when the byte budget is zero', async () => {
    ...
    try {
      ...
      await expect(loadBaselineInstructions({ cwd: root, clockyHome: home, maxBytes: 0 })).resolves.toBeUndefined()
      await expect(loadBaselineInstructions({ cwd: root, clockyHome: home, maxBytes: 65536, maxSourceBytes: 0 })).resolves.toBeUndefined()
      await expect(loadBaselineInstructions({
        cwd: root, clockyHome: home, maxBytes: 65536, maxSourceBytes: Infinity,
  ...
  it('defaults clockyHome and uses cwd itself as root when no project marker exists', async () => {
    const root = await tempRepo()
    const emptyHome = await tempRepo()
    // Isolate the default-home fallback: blank CLOCKY_HOME is treated as unset, and
    // the home dirs point at an empty dir so the default ~/.clocky holds no global
    // scope. Windows homedir() reads USERPROFILE (not HOME), so both must be
    // stubbed or a real ~/.clocky/AGENTS.md would otherwise leak in.
    vi.stubEnv('CLOCKY_HOME', '')
  ...
  it('honors CLOCKY_HOME when clockyHome is not configured explicitly', async () => {
    const root = await tempRepo()
  ...
  it('labels the default Clocky home as ~/.clocky when HOME points at the configured default', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      await write(join(home, '.clocky/AGENTS.md'), 'global default rule')

      // A set CLOCKY_HOME would override the homedir default and relabel the home.
      vi.stubEnv('CLOCKY_HOME', '')
      vi.resetModules()
  ...
  it('expands a configured ~/.clocky home to the operating-system home directory', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
      await write(join(home, '.clocky/AGENTS.md'), 'global tilde rule')
  ...
  it('deduplicates user-global instructions when clockyHome points at the project root', async () => {
    const root = await tempRepo()
    try {
      await mkdir(join(root, '.git'), { recursive: true })
      await write(join(root, 'AGENTS.md'), 'same file')

      const files = await discoverBaselineInstructionFiles({ cwd: root, clockyHome: root })

      expect(files).toEqual([{ absolutePath: join(root, 'AGENTS.md'), displayPath: '$CLOCKY_HOME/AGENTS.md' }])
    } finally {
      await rm(root, { recursive: true, force: true })
    }
  })
...
describe('workspace context request injection', () => {
  ...
  it('preserves a visible baseline when its source is unavailable during resume', async () => {
    ...
    try {
      ...
      await ctx.plugin(workspaceContext, { clockyHome: home, maxBytes: 65536 })
      const original = stubAgent(root)
      await composeBaselinePrefix(ctx, original)
  ...
  it('does not promote an unchanged budget-omitted baseline file during resume', async () => {
    ...
    try {
      ...
      const ctx = new Context()
      await mountWorkspaceContext(ctx, { clockyHome: home, maxBytes: 700 })
  ...
  it('recomposes the baseline when candidate precedence changes between resumes', async () => {
    ...
    try {
      ...
      await write(join(root, 'CLAUDE.md'), 'claude rule')
      await mountWorkspaceContext(originalCtx, { clockyHome: home, maxBytes: 65536 })
      const original = stubAgent(root)
  ...
  it('reuses an inserted but unadmitted baseline after session recovery and plugin reload', async () => {
    ...
    try {
      ...
      await write(join(root, 'AGENTS.md'), 'repo rule')
      const ctx = new Context()
      const fiber = await mountWorkspaceContext(ctx, { clockyHome: home, maxBytes: 65536 })
      const original = stubAgent(root)
  ...
  it.each([
    ...
    try {
      ...
      await resumedCtx.plugin(workspaceContext, { clockyHome: home, maxBytes })
      const resumed = stubAgent(root, [...original.session.events])
  ...
  it('reports a changed user-global instruction through the visible-scope reconcile path', async () => {
    ...
    try {
      ...
      const ctx = new Context()
      await mountFileToolsAndWorkspaceContext(ctx, { clockyHome: home, maxBytes: 65536 })
      const agent = stubAgent(root)
      await composeBaselinePrefix(ctx, agent)
  ...
  it('retains a visible baseline after a plugin remount', async () => {
    ...
    try {
      ...
      const ctx = new Context()
      const fiber = await mountFileToolsAndWorkspaceContext(ctx, { clockyHome: home, maxBytes: 65536 })
      const agent = stubAgent(root)
      await composeBaselinePrefix(ctx, agent)
  ...
  it('restores a compacted baseline on a hot plugin remount', async () => {
    ...
    try {
      ...
      const ctx = new Context()
      await ctx.plugin(LocalFileSystem, { cwd: '/' })
      const fiber = await ctx.plugin(workspaceContext, { clockyHome: home, maxBytes: 65536 })
      const agent = stubAgent(root)
      await composeBaselinePrefix(ctx, agent)
      const baseline = baselineEvents(agent)[0]
  ...
  it('deduplicates one AGENTS.md that is both user-global and the project-root candidate', async () => {
    ...
    try {
      ...
      const ctx = new Context()
      await mountWorkspaceContext(ctx, { clockyHome: root, maxBytes: 65536 })
      const agent = stubAgent(root)
  ...
  it.each([10, 120])('does not expose state markers when baseline content is omitted at %i bytes', async (maxBytes) => {
    ...
    try {
      ...
      const ctx = new Context()
      await mountWorkspaceContext(ctx, { clockyHome: home, maxBytes })
      const agent = stubAgent(root)
  ...
  it('keeps the direct provider API usable without an operation signal', async () => {
    ...
    try {
      ...
      const rendered = await loadBaselineInstructions({ cwd: root, clockyHome: home, maxBytes: 65536 }, fs)
  ...
  it('rejects a provider-sized instruction file before reading content', async () => {
    ...
    try {
      ...
      await ctx.plugin(workspaceContext, { clockyHome: home, maxBytes: 65536, maxSourceBytes: 4 })
  ...
  it('does not inject anything when maxBytes is zero', async () => {
    ...
    try {
      ...
      await write(join(root, 'AGENTS.md'), 'repo rule')
      const ctx = new Context()
      await mountWorkspaceContext(ctx, { clockyHome: home, maxBytes: 0 })
      const agent = stubAgent(root)
  ...
  it('does not inject an empty agent-instructions message when maxBytes is negative', async () => {
    ...
    try {
      ...
      await write(join(root, 'AGENTS.md'), 'repo rule')
      const ctx = new Context()
      await mountWorkspaceContext(ctx, { clockyHome: home, maxBytes: -1 })
      const agent = stubAgent(root)
  ...
  it('labels a custom clockyHome as CLOCKY_HOME instead of pretending it is ~/.clocky', async () => {
    const root = await tempRepo()
    const home = await tempRepo()
    try {
  ...
  it('does not repeat a candidate metadata probe during one discovery and read pass', async () => {
    ...
    try {
      ...
      const isolated = await import('@clocky/clocky-agent-instructions')
      await isolated.loadBaselineInstructions({ cwd: root, clockyHome: home, maxBytes: 65536 })
      observedStats.clear()
      await isolated.loadBaselineInstructions({ cwd: root, clockyHome: home, maxBytes: 65536 })
  ...
  it('skips an unavailable host candidate but still loads its available sibling', async () => {
    ...
    try {
      ...
      const rendered = await isolated.loadBaselineInstructions({ cwd: root, clockyHome: home, maxBytes: 65536 })
...
describe('dynamic nested workspace context injection', () => {
  ...
  it('skips visible baseline scopes when baseline scopes are excluded from reconciliation', async () => {
    ...
    try {
      ...
      agent.session.append('user/message', authoritative, { surfaceOp: 'append' })
      const resolved = resolveConfig({ clockyHome: home, maxBytes: 65536, localInstructionFileCandidates: [] })
      const cache: InstructionVersionCache = new WeakMap()
  ...
  it('does not mark omitted nested files as pending-loaded', async () => {
    ...
    try {
      ...
      await write(join(root, 'pkg/sub/file.txt'), 'subtree file')
      const ctx = new Context()
      await mountFileToolsAndWorkspaceContext(ctx, { clockyHome: home, maxBytes: 700 })
      const agent = stubAgent(root)
      await ctx.tools.execute({
        signal: testToolSignal,
  ...
  it('does not attach nested instructions when the byte budget is disabled', async () => {
    ...
    try {
      ...
      await write(join(root, 'pkg/deep/file.txt'), 'hello')
      const ctx = new Context()
      await mountFileToolsAndWorkspaceContext(ctx, { clockyHome: home, maxBytes: 0 })
  ...
  it('retries a nested instruction touch when only a truncated budget notice was rendered', async () => {
    ...
    try {
      ...
      fs.entries.set(join(root, 'pkg/file.txt'), { type: 'file', content: 'hello' })
      await ctx.plugin(ToolFs)
      await ctx.plugin(workspaceContext, { clockyHome: home, maxBytes: 20 })
      const agent = stubAgent(root)
...
describe('workspace context inbox synchronization', () => {
  ...
  it('holds back a dynamic change a one-byte positive render budget cannot represent', async () => {
    ...
    try {
      ...
      await ctx.plugin(workspaceContext, { clockyHome: home, maxBytes: 1 })
      const agent = stubAgent(root)
      ctx.emit('tools/result', stubToolExecution({
        signal: testToolSignal,
