 ...
 *
 * @module @clocky/clocky-agent-instructions/files
 */

import { createReadStream } from 'node:fs'
import { stat } from 'node:fs/promises'
import { dirname, isAbsolute, join, relative, resolve } from 'node:path'
import type { FileSystem, FsInfo, FsTarget, FsVersion } from '@clocky/clocky-fs'
import { assertNever } from '@clocky/clocky-llm'
import { clockyHomeDisplay } from '@clocky/clocky-home-paths'
...
async function discoverInstructionFiles(
  ...
  const userGlobal = join(config.clockyHome, USER_GLOBAL_FILE)
  ...
  switch (userGlobalProbe.kind) {
    case 'present':
      addFile({
        ...
        displayPath: userGlobalDisplayPath(config.clockyHome),
...
export async function probeScopeInstruction(
  ...
  const file: ProbedInstructionFile = {
    ...
    displayPath: directory === USER_GLOBAL_DIRECTORY
      ? userGlobalDisplayPath(resolved.clockyHome)
...
function userGlobalDisplayPath(clockyHome: string): string {
  return `${clockyHomeDisplay(clockyHome)}/AGENTS.md`
}
