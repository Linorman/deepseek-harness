...
const platformPackageName = `@clocky/node-addon-landlock-run-linux-${process.arch}`
...
const WORKSPACE_CLOSURE = [
  ...
  // consumer resolves it like any other @clocky peer (koffi arrives
  // from the registry).
...
describe.skipIf(!packable)('sandbox-local: packed-tarball distribution (publish-path rehearsal)', () => {
  beforeAll(async () => {
    const packDest = mkdtempSync(join(tmpdir(), 'clocky-pack-'))
    consumerDir = mkdtempSync(join(tmpdir(), 'clocky-packed-consumer-'))
    workDir = mkdtempSync(join(tmpdir(), 'clocky-packed-work-'))
    ...
    writeFileSync(join(consumerDir, 'consumer.mjs'), `
      ...
      import { launcherPath } from '@clocky/node-addon-landlock-run'
