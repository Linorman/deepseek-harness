 ...
 * bwrap forced off. Tests assert real world effects; consumer coverage lives in clocky-bash-sandbox.
...
async function tempDir(base: string): Promise<string> {
  const dir = await mkdtemp(join(base, 'clocky-landlock-e2e-'))
  tempDirs.push(dir)
  return dir
}
...
describe.skipIf(!landlockUsable)('sandbox-local: real Landlock confinement through the bundled launcher', () => {
  ...
  it('read-only denies a write beneath the host /dev (the /dev/shm tmpfs must stay untouched)', async () => {
    ...
    const workdir = await tempDir(tmpdir())
    const sandbox = await provider()
    const target = `/dev/shm/clocky-landlock-e2e-${process.pid}`
