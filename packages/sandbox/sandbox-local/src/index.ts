 ...
 * @module @clocky/clocky-sandbox-local
 */
...
} from '@clocky/node-addon-landlock-run'
import { Context } from '@clocky/cordis'
import z from '@clocky/schemastery'
import { assertNever } from '@clocky/clocky-llm'
import { SandboxProvider, SandboxUnavailableError } from '@clocky/clocky-sandbox'
import type { ConfinedArgv, ConfinedSandboxMode, RunnerFailureRule, SandboxEnforcement, SandboxPolicy } from '@clocky/clocky-sandbox'
import type { SessionId } from '@clocky/clocky-session'
import { AclWriteGrant, assertTempRootOutsideWorkspace, tempWriteSid, workspaceWriteSid } from '@clocky/clocky-sandbox-windows-acl'
...
const PLATFORM_CHAINS: Record<string, readonly SelectedRunner['runner'][]> = {
  ...
  darwin: ['seatbelt'],
  // The Windows restricted-token runner (@clocky/clocky-sandbox-windows-acl):
...
export class LocalSandboxProvider extends SandboxProvider {
  ...
  private windowsAclRunnerInvocation(): string[] {
    ...
    const builtEntry = this.internals.windowsAclRunnerEntry ?? fileURLToPath(import.meta.resolve('@clocky/clocky-sandbox-windows-acl/runner'))
    if (existsSync(builtEntry)) return [process.execPath, builtEntry]
    const sourceEntry = fileURLToPath(import.meta.resolve('@clocky/clocky-sandbox-windows-acl/src/runner.ts'))
