...
- id: command-compact
  name: '@clocky/clocky-command-compact'
```

The shipped `clocky` base mounts it beside `compaction-basic`, and the Web client provides the command adapter. Automation surfaces that compose no command adapter keep automatic compaction only.
