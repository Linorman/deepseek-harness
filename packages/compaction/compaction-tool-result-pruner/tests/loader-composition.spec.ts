...
describe('compaction-tool-result-pruner real Loader composition', () => {
  it('loads and resolves the flat YAML plugin shape', async () => {
    root = await mkdtemp(join(tmpdir(), 'clocky-compact-tool-result-prune-loader-'))
    ...
    context.loader.internal = {
      ...
      async import(specifier: string) {
        if (specifier === '@clocky/clocky-token-meter') return TokenMeter
        if (specifier === '@clocky/clocky-compaction-tool-result-pruner') return ToolResultPruner
