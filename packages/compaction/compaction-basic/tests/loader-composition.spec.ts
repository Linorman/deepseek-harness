...
async function loadYaml(lines: readonly string[]): Promise<Context> {
  root = await mkdtemp(join(tmpdir(), 'clocky-token-meter-loader-'))
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@clocky/clocky-token-meter', TokenMeter],
    ['@clocky/clocky-compaction-tool-result-pruner', ToolResultPruner],
    ['@clocky/clocky-compaction-basic', BasicCompactionEngine],
  ])
...
describe('real Loader composition', () => {
  it('loads the shipped token-meter, pruning, and compaction-basic YAML order', async () => {
    const loaded = await loadYaml([
      ...
      "- name: '@clocky/clocky-compaction-tool-result-pruner'",
      '  config:',
      '    thresholdChars: 100',
      '    headChars: 20',
      '    tailChars: 10',
