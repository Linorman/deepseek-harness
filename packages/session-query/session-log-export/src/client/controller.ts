...
import { createSnapshotStore, type SessionId, type SnapshotStore } from '@clocky/clocky-client-runtime/client'
...
export function sessionLogZipFilename(sessionId: SessionId): string {
  return `clocky-session-${String(sessionId).replace(/[^A-Za-z0-9_-]/g, '_')}.zip`
}
...
function hostBase(): string {
  ...
  return origin !== undefined && origin !== 'null' ? origin : 'http://clocky.internal'
}
