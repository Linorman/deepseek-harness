...
} from '@clocky/clocky-output-retention'
import type {
...
- **`TextRetainer` bounds byte-oriented text.** `head`, `tail`, and `headTail` preserve UTF-8 boundaries at `finish()`; `headTail` is the shape `clocky-spill-policy` uses to build a bounded preview around a spill-file notice.
