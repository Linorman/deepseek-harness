 ...
 *
 * @module @clocky/clocky-plan-mode
 */
...
import type { Session, SessionEvent, UserMessage } from '@clocky/clocky-session'
...
import type { CommandId } from '@clocky/clocky-commands'
  ...
  constructor(ctx: Context, config: PlanModeConfig = { section: '' }) {
    ...
    ctx.on('agent/pre-step', async (
      ...
      try {
        ...
        ctx.logger.warn('clocky-plan-mode: failed to append selected plan mode at step start: %o', error)
        return decision
      }
    ...
    ctx.effect(() => () => { disposed = true }, 'clocky-plan-mode: close service lifetime')
