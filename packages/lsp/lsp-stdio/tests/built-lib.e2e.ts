...
/**
 * Keyless built-artifact smoke: plain Node imports `@clocky/clocky-lsp` and
 * `@clocky/clocky-lsp-stdio` by name through their exports maps, spawns the fixture server, runs
...
describe.skipIf(!built)('built lib real load path (plain node)', () => {
  it('runs a query through lib/index.js and disposes cleanly, framing over the base protocol', async () => {
    ...
    const script = `
      ...
      const { default: Lsp } = await import('@clocky/clocky-lsp')
      const LspLocal = await import('@clocky/clocky-lsp-stdio')
      const { default: LocalFileSystem } = await import('@clocky/clocky-fs-local')
