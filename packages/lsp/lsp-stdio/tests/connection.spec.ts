...
import { LspConnection } from '@clocky/clocky-lsp-stdio'
import type { ConnectionWriter } from '@clocky/clocky-lsp-stdio/src/connection.ts'
...
describe('LspConnection', () => {
  ...
  it('forwards explicit CLOCKY_* env entries to the child', async () => {
    // A configured CLOCKY_* fact must reach the child: the seam scrubs only the
    // ambient namespace, and the explicit entry merges after that scrub. The
    // fixture echoes the named variable back as hover text.
    const conn = connect({ LSP_FAKE_ECHO_ENV: 'CLOCKY_LSP_TEST_FACT', CLOCKY_LSP_TEST_FACT: 'managed' })
