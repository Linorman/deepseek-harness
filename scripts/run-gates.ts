...
async function main(args: string[]): Promise<number> {
  ...
  const concurrencyOverride = process.env.CLOCKY_GATE_CONCURRENCY
  const maxConcurrency = concurrencyFromEnv('CLOCKY_GATE_CONCURRENCY', concurrencyDefault.workers)
  const concurrencySource = concurrencyOverride === undefined || concurrencyOverride === ''
    ? concurrencyDefault.source
    : '$CLOCKY_GATE_CONCURRENCY'
...
function nodeCompatGates(): Gate[] {
  const typecheck = flagEnabled('CLOCKY_NODE_COMPAT_SKIP_TYPECHECK')
    ? []
...
function nodeCompatSmokeGates(options: { cliSmoke?: boolean } = {}): Gate[] {
  const gates: Gate[] = [
    ...
    pnpmExec('clocky-source-launch-smoke', [
      'vitest',
      'run',
      'apps/cli/tests/source-launch.compat.spec.ts',
    ], { label: 'clocky source-launch smoke' }),
  ...
  if (options.cliSmoke) {
    gates.push(
      pnpmExec('cli-lazy-search-startup-smoke', [
        ...
        env: { CLOCKY_REQUIRE_BUILT_CLI_SMOKE: '1' },
        needs: ['build:web'],
      }),
    )
  }
...
function ciConsumerGates(): Gate[] {
  ...
  return [
    ...
    pnpmScript('doc-typecheck', 'doc-typecheck:contracts-ready', {
      needs: validatedBuild,
      env: { CLOCKY_DOC_TYPECHECK_USE_BUILD_OUTPUT: '1' },
    }),
    pnpmScript('node-next-types', 'verify-node-next-types', {
      label: 'node-next types',
      needs: validatedBuild,
...
function webSnapshotGate(needs: string[]): Gate {
  const workerRaw = process.env.CLOCKY_WEB_SNAPSHOT_WORKERS
  if (workerRaw !== undefined && workerRaw !== '') {
    ...
    if (!Number.isSafeInteger(workers) || workers < 2 || String(workers) !== workerRaw) {
      throw new Error(`run-gates: CLOCKY_WEB_SNAPSHOT_WORKERS must be an integer greater than 1, got ${JSON.stringify(workerRaw)}.`)
    }
    return pnpmScript('web-snapshot', 'test:web:ci', {
      label: 'web browser snapshot',
      displayCommand: `CLOCKY_SNAPSHOT=replay CLOCKY_WEB_SNAPSHOT_WORKERS=${workers} pnpm run test:web:ci`,
...
function lintGate(options: { needs?: string[] } = {}): Gate {
  const raw = process.env.CLOCKY_OXLINT_THREADS
  const script = 'lint:contracts-ready'
  return pnpmScript('lint', script, {
    ...raw === undefined || raw === ''
      ? {}
      : { displayCommand: `CLOCKY_OXLINT_THREADS=${raw} pnpm run ${script}` },
...
//
// CLOCKY_COVERAGE_MAX_WORKERS is the ordinary lane's worker budget, so the two
// parallel gates split it instead of each claiming it whole. When
// CLOCKY_COVERAGE_PARTITIONS is set, its single-worker processes replace the
...
// total of one (the serial reference jobs) also set CLOCKY_GATE_CONCURRENCY=1,
// which keeps the gates from overlapping at all.
// CLOCKY_COVERAGE_TEST_TIMEOUT_MS raises Vitest's per-test and expect.poll
...
function coverageWorkerArgs(): { instrumented: string[]; exempt: string[] } {
  const [flag] = positiveIntArg('CLOCKY_COVERAGE_MAX_WORKERS', '--maxWorkers')
...
function printResult(result: GateResult): void {
  const verbose = process.env.CLOCKY_GATE_VERBOSE === '1'
