...
const vendoredPackages = new Set([
  ...
  '@clocky/cosmokit',
  '@clocky/schemastery',
  '@clocky/cordis-plugin-loader',
  '@clocky/cordis-plugin-include',
  '@clocky/cordis-plugin-group',
  '@clocky/cordis-plugin-timer',
  '@clocky/cordis-plugin-hmr',
  '@clocky/cordis-plugin-logger-console',
])
const publicLandlockPackages = new Set([
  '@clocky/node-addon-landlock-run',
  '@clocky/node-addon-landlock-run-linux-arm64',
  '@clocky/node-addon-landlock-run-linux-x64',
])
...
const publicationSourceAllowlist: Readonly<Record<string, readonly string[]>> = {
  '@clocky/node-addon-landlock-run': ['src/main.c'],
}
const repositoryUrl = 'git+https://github.com/clocky/clocky.git'
/**
 ...
 */
const publishedRepositoryUrl = 'git+https://github.com/Linorman/clocky.git'
...
const appPackageFiles: Readonly<Record<string, readonly string[]>> = {
  ...
  '@clocky/clocky-web-frontend': ['dist', '!dist/**/*.map'],
}
...
const packageFileExtras: Readonly<Record<string, readonly string[]>> = {
  ...
  '@clocky/clocky-client-ui-primitives': ['lib/**/*.css'],
  '@clocky/clocky-client-web': ['lib/**/*.css'],
  '@clocky/clocky-client-ui-theme': ['lib/styles'],
  // The CPython side ships as source .py files, published as-is rather than built.
  '@clocky/clocky-code-runtime-python': ['py/**/*.py'],
  // The Python runtime uses a distinct closed-resolution bin; the public CLI
  // keeps config-owned bare-package resolution through lib/bin.js.
  '@clocky/clocky-sdk-jsonrpc-demo': ['lib/packaged-bin.js'],
  // The argv-prefix runner entry ships beside the lib as its own bundle;
  // sandbox-local resolves it through the package's ./runner export. tsdown
  // also shares its generated FFI code through a hashed runtime chunk.
  '@clocky/clocky-sandbox-windows-acl': ['lib/runner.js', 'lib/types-*.js'],
...
function expectedClockyPackageFiles(manifest: PackageManifest): readonly string[] {
  const declaredPatch = manifest.clocky?.bundle?.patch
...
function checkWorkspace({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  if (isPublicLandlockPackage) {
    ...
    // them, while the clocky family stays restricted until its own sequence goes
  ...
  if (manifest.name?.startsWith('@clocky/')) {
  ...
  if (dir.startsWith('apps/') && manifest.name?.startsWith('@clocky/')) {
  ...
  if (dir.startsWith('packages/') && manifest.name?.startsWith('@clocky/clocky-')) {
    const peer = manifest.peerDependencies?.['@clocky/cordis']
    ...
    const expectedFiles = expectedClockyPackageFiles(manifest)
...
function checkRepositoryVersion(): string[] {
  // The root carries the clocky release family's version, so a prerelease such as
  // 0.0.1-rc.1 is a valid state between `release:clocky` and its publication.
