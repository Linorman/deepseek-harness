...
const MERGE_HEAD = /declare module ['"](?:@clocky\/cordis|\.\/context\.ts)['"]/
 ...
 * BLOCK — a file may legally hold several `declare module '@clocky/cordis'` blocks
...
/** Every cordis module-merge body in `sf`: `declare module '@clocky/cordis'` (harness
...
function cordisModuleBodies(sf: ts.SourceFile): ts.ModuleBlock[] {
  ...
  for (const stmt of sf.statements) {
    ...
    if (stmt.name.text !== '@clocky/cordis' && stmt.name.text !== './context.ts') continue
...
/**
 * Every `key: Type` property a `declare module '@clocky/cordis'` Context merge
...
/**
 * Every event name a `declare module '@clocky/cordis'` Events merge declares in one
