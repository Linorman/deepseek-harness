...
- id: sessions
  ...
  config:
    root: !!js process.env.CLOCKY_SESSION_ROOT
    compression: 'none'
- id: code-runtime
...
def smoke_direct(base_url: str, executable: Path) -> None:
    with tempfile.TemporaryDirectory(prefix="clocky-direct-") as temporary:
        ...
        environment = {
            **os.environ,
            "CLOCKY_CORDIS_CONFIG": str(cordis),
            "CLOCKY_SESSION_ROOT": str(sessions),
            "CLOCKY_CWD": str(root),
        ...
        try:
            peer.send({"jsonrpc": "2.0", "id": "initialize", "method": "initialize", "params": {"cwd": str(root), "provider": "test-provider", "model": "smoke-model"}})
