...
describe('Vitest jsdom compatibility', () => {
  it('provides isolated browser storage instead of Node process storage', () => {
    ...
    localStorage.setItem('clocky-vitest-storage-probe', 'available')

    expect(localStorage.getItem('clocky-vitest-storage-probe')).toBe('available')
    localStorage.removeItem('clocky-vitest-storage-probe')
  })
})
