...
export interface CatalogEntry {
  /** npm package name, e.g. `@clocky/clocky-agent-loop`. */
  pkg: string
...
export function collectConfigCatalog(scanRoot: string = root): CatalogEntry[] {
  ...
  for (const manifestRel of globSync('packages/*/*/package.json', { cwd: scanRoot }).map(path => path.split(sep).join('/')).sort()) {
    ...
    const manifest = JSON.parse(readFileSync(resolve(scanRoot, manifestRel), 'utf8')) as { name?: string; os?: string[]; cpu?: string[]; private?: boolean }
    ...
    if (dir.startsWith('packages/compat/') && manifest.private === true) continue
