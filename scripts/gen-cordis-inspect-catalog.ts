...
import type { CordisCatalogModel, ServiceMethodEntry } from '@clocky/clocky-typert-generator'
...
const CLIENT_SERVICES: Readonly<Record<string, readonly string[]>> = {
  ...
  sessions: ['open', 'search', 'scope', 'binding'],
...
function main(): void {
  ...
  const source = projector.renderRuntimeApi(clientModel(model))
    .replaceAll('@clocky/clocky-tool-cordis/api-catalog', '@clocky/clocky-cordis-client-runner/client/api-catalog')
