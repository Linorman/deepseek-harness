...
function workspaceFixture(options: {
  ...
  const root = mkdtempSync(join(tmpdir(), 'clocky-project-reference-faces-'))
  roots.push(root)
  ...
  writeJson(join(shared, 'package.json'), { name: '@clocky/clocky-shared' })
  writeJson(join(shared, 'tsconfig.json'), {
    extends: '../../../tsconfig.base.json',
    references: [],
  })
  writeJson(join(split, 'package.json'), { name: '@clocky/clocky-split' })
...
describe('Project Reference compiler faces', () => {
  ...
  it('uses the referencing project face throughout the reachable graph', () => {
    ...
    writeJson(join(hostConsumer, 'package.json'), { name: '@clocky/clocky-host-consumer' })
    ...
    writeJson(join(clientConsumer, 'package.json'), { name: '@clocky/clocky-client-consumer' })
