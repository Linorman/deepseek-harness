 ...
 * owns: `clocky` and `vendor`. Each family carries its own version baseline, tag
...
const WORKSPACE_ROOT_PACKAGE = '@clocky/clocky-root'
...
export abstract class ReleaseFamily {
  ...
  members(root: string): ReleaseMember[] {
    ...
    for (const manifestPath of manifestPaths) {
      ...
      if (!name.startsWith('@clocky/')) throw new Error(`${normalized} must name an @clocky package`)
...
class ClockyFamily extends ReleaseFamily {
  readonly id = 'clocky'
  ...
  verifyVersions(members: readonly ReleaseMember[]): void {
    ...
    if (versions.size !== 1) {
      ...
      throw new Error(`clocky release members must share one version:\n${detail}`)
    }
  }
  ...
  readonly installedEntry = { packageName: '@clocky/clocky', binPath: 'lib/bin.js' }
}
...
class VendorFamily extends ReleaseFamily {
  ...
  tagPrefixFor(member: ReleaseMember): string {
    return `${this.tagPrefix}${member.name.replace('@clocky/', '')}-v`
  }
...
function releaseFamilies(): readonly ReleaseFamily[] {
  return [new ClockyFamily(), new VendorFamily()]
}
