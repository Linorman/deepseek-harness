...
function declaration(
  ...
  return {
    name: short.startsWith('@') ? short : '@clocky/clocky-client-' + short,
...
describe('source package uses', () => {
  it('counts type imports, module augmentations, dynamic imports, and JSX', () => {
    const uses = collectSourcePackageUses('feature.tsx', [
      "import type { A } from '@clocky/clocky-a/subpath'",
      "declare module '@clocky/clocky-client-ui-slots' {}",
      "const load = () => import('@clocky/clocky-b')",
...
describe('package modes', () => {
  ...
  it('requires seeded workspace packages to use staticLinked and preloads to name dynamic rows', () => {
    ...
    expect(found.join('\n')).toContain('has no dynamic clocky.client row')
  })
  ...
  it('requires every preloaded external to have a parser preload row', () => {
    ...
    expect(collectClientPackageViolations(facts([], {
      ...
      + '"@clocky/clocky-client-runtime/client" has no matching PARSER_PRELOAD_IDS row in '
...
describe('dependency sections', () => {
  it('accepts dynamic peer plus dev relationships, static dev inputs, and private dependencies', () => {
    ...
    const runtime = pkg('runtime', {
      inject: ['@clocky/clocky-client-feature'],
      sourceUses: {
        '@clocky/clocky-agent': ['packages/client/runtime/src/index.ts'],
        '@clocky/clocky-client-ui-slots': ['packages/client/runtime/src/client/slots.ts'],
        react: ['packages/client/runtime/src/client/view.tsx'],
      },
      dependencies: { immer: '^10.1.1' },
      peerDependencies: {
        [CORDIS]: 'workspace:^',
        '@clocky/clocky-agent': 'workspace:^',
        '@clocky/clocky-client-feature': 'workspace:^',
      },
      devDependencies: {
        [CORDIS]: 'workspace:^',
        '@clocky/clocky-agent': 'workspace:^',
        '@clocky/clocky-client-feature': 'workspace:^',
        '@clocky/clocky-client-ui-slots': 'workspace:^',
        react: '^18.2.0',
      },
    })
  ...
  it('rejects internal dependencies, static peers, and mismatched peer development ranges', () => {
    ...
    const subject = pkg('feature', {
      sourceUses: {
        '@clocky/clocky-agent': ['packages/client/feature/src/index.ts'],
        [slots.name]: ['packages/client/feature/src/view.tsx'],
      },
      dependencies: { '@clocky/clocky-agent': 'workspace:^' },
    ...
    expect(found).toHaveLength(2)
    expect(found.join('\n')).toContain('peer-installed Clocky relationship')
    expect(found.join('\n')).toContain('static client input')
  })

  it('requires every peer to have the same development range', () => {
    const subject = pkg('feature', {
      peerDependencies: { [CORDIS]: 'workspace:^', '@clocky/cordis-plugin-loader': 'workspace:^' },
    })
    expect(collectClientPackageViolations(facts([subject]))).toEqual([
      'packages/client/feature/package.json: peerDependencies.@clocky/cordis-plugin-loader'
      + ' is workspace:^, so devDependencies.@clocky/cordis-plugin-loader must use the same range;'
  ...
  it('keeps the web shell runtime inputs development-only', () => {
    const web = pkg('web', {
      ...
      staticLinked: true,
      runtimeSourceUses: {
        '@clocky/cordis-plugin-loader': ['packages/client/web/src/boot.ts'],
        react: ['packages/client/web/src/seed.ts'],
      },
      devDependencies: {
        [CORDIS]: 'workspace:^',
        '@clocky/cordis-plugin-loader': 'workspace:^',
        react: '^18.2.0',
      },
    })
  ...
  it('allows npm dependency cycles', () => {
    const a = pkg('a', {
      peerDependencies: { [CORDIS]: 'workspace:^', '@clocky/clocky-client-b': 'workspace:^' },
      devDependencies: { [CORDIS]: 'workspace:^', '@clocky/clocky-client-b': 'workspace:^' },
    })
    const b = pkg('b', {
      peerDependencies: { [CORDIS]: 'workspace:^', '@clocky/clocky-client-a': 'workspace:^' },
      devDependencies: { [CORDIS]: 'workspace:^', '@clocky/clocky-client-a': 'workspace:^' },
    })
...
describe('module requests', () => {
  it('accepts a dynamic row supplier and its client subpath', () => {
    const ui = declaration('ui', { external: ['@clocky/clocky-client-slots/client'] })
  ...
  it('rejects an explicit baseline request', () => {
    ...
    expect(collectClientPackageViolations(facts([], {
      ...
      ui.manifest + ': clocky.client.external repeats baseline module "react"; remove the explicit declaration',
    ])
  })

  it('rejects duplicates, empty values, self-requests, and missing suppliers', () => {
    const ui = declaration('ui', {
      external: ['', '@clocky/clocky-client-ui', '@clocky/clocky-missing', '@clocky/clocky-missing'],
      inject: ['', '@clocky/clocky-a', '@clocky/clocky-a'],
    })
    const found = collectClientPackageViolations(facts([], { declarations: [ui] }))
    expect(found).toHaveLength(6)
    expect(found.join('\n')).toContain('clocky.client.external contains an empty value')
    expect(found.join('\n')).toContain('clocky.client.inject contains an empty value')
  ...
  it('rejects synchronous module-request cycles but ignores inject cycles', () => {
    const a = declaration('a', {
      external: ['@clocky/clocky-client-b'],
      inject: ['@clocky/clocky-client-b'],
    })
    const b = declaration('b', {
      external: ['@clocky/clocky-client-a'],
      inject: ['@clocky/clocky-client-a'],
    })
    const found = collectClientPackageViolations(facts([], { declarations: [a, b] }))
    expect(found).toHaveLength(1)
    expect(found[0]).toContain('synchronous clocky.client.external cycle')
  })
})
...
describe('manifest declarations', () => {
  it('reports malformed arrays without hiding other packages', () => {
    ...
    const files: Record<string, unknown> = {
      'packages/g/a/package.json': {
        name: '@f/a', clocky: { client: { external: 'react', inject: ['@f/b', 1] } },
      },
      'packages/g/b/package.json': { name: '@f/b', clocky: { client: {} } },
    }
    ...
    expect(result.malformed).toEqual([
      'packages/g/a/package.json: @f/a clocky.client.external must be a string array',
      'packages/g/a/package.json: @f/a clocky.client.inject must be a string array',
    ])
  })
  ...
  it('fixes unambiguous dependency sections and declaration entries', () => {
    ...
    roots.push(root)
    const subject = pkg('feature', {
      external: ['', 'react', '@clocky/clocky-client-feature', '@clocky/clocky-missing'],
      inject: ['', '@clocky/clocky-agent', '@clocky/clocky-agent'],
      sourceUses: {
        '@clocky/clocky-agent': ['packages/client/feature/src/index.ts'],
        '@clocky/clocky-client-ui-slots': ['packages/client/feature/src/view.tsx'],
      },
      dependencies: {
        [CORDIS]: 'workspace:^',
        '@clocky/clocky-agent': 'workspace:*',
    ...
    const manifest = {
      name: subject.name,
      clocky: { client: { external: subject.external, inject: subject.inject, platform: 'web' } },
    ...
    const fixed = JSON.parse(readFileSync(join(root, subject.manifest), 'utf8')) as {
      clocky: { client: { external: string[]; inject: string[] } }
      dependencies?: Record<string, string>
      peerDependencies: Record<string, string>
      devDependencies: Record<string, string>
    }
    expect(fixed.clocky.client).toMatchObject({
      external: ['@clocky/clocky-missing'],
      inject: ['@clocky/clocky-agent'],
    })
