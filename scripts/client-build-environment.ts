...
const CLIENT_BUILD_ENV_PREFIX = 'CLOCKY_CLIENT_'

/** Non-public selector used by build orchestration to request a named client profile. */
export const CLIENT_BUILD_PROFILE_SELECTOR = 'CLOCKY_BUILD_CLIENT_PROFILE'

/** Public client environment required by official Clocky artifacts. */
...
const CLIENT_COMMIT_HASH_VARIABLE = 'CLOCKY_CLIENT_COMMIT_HASH'

/** Repository-relative path of the complete client build record. */
export const CLIENT_BUILD_RECORD_PATH = '.clocky-build/client-build-environment.json'
...
): Readonly<Record<`CLOCKY_CLIENT_${string}`, string>> {
  return {
    CLOCKY_CLIENT_COMMIT_HASH: repositoryCommitHash(root, environment),
 ...
 * @returns defined `CLOCKY_CLIENT_*` values only.
 */
...
export function resolveClientBuildEnvironment(
  ...
  if (profile === 'official') {
    ...
    return { CLOCKY_CLIENT_COMMIT_HASH: commitHash, ...OFFICIAL_CLIENT_BUILD_ENVIRONMENT }
  }
...
export function assertClientBuildEnvironment(
  ...
  expected: Readonly<Record<`CLOCKY_CLIENT_${string}`, string>>,
): void {
