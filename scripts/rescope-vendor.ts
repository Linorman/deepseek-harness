/**
 * Rescope the vendored Cordis packages into the `@clocky` scope, and undo
 ...
 * `@clocky/clocky-tool-cordis`, and `cordiverse/cordis`, and makes the
...
const RENAMES: readonly Rename[] = [
  { directory: 'cordis', upstream: 'cordis', scoped: '@clocky/cordis' },
  { directory: 'cosmokit', upstream: 'cosmokit', scoped: '@clocky/cosmokit' },
  { directory: 'schemastery', upstream: 'schemastery', scoped: '@clocky/schemastery' },
  { directory: 'loader', upstream: '@cordisjs/plugin-loader', scoped: '@clocky/cordis-plugin-loader' },
  { directory: 'include', upstream: '@cordisjs/plugin-include', scoped: '@clocky/cordis-plugin-include' },
  { directory: 'group', upstream: '@cordisjs/plugin-group', scoped: '@clocky/cordis-plugin-group' },
  { directory: 'timer', upstream: '@cordisjs/plugin-timer', scoped: '@clocky/cordis-plugin-timer' },
  { directory: 'hmr', upstream: '@cordisjs/plugin-hmr', scoped: '@clocky/cordis-plugin-hmr' },
  { directory: 'logger-console', upstream: '@cordisjs/plugin-logger-console', scoped: '@clocky/cordis-plugin-logger-console' },
]
...
const POSTCONDITIONS: readonly PostCondition[] = [
  { file: 'vendor/cordis/package.json', text: '"name": "@clocky/cordis"', count: 1 },
  { file: 'vendor/hmr/package.json', text: '"name": "@clocky/cordis-plugin-hmr"', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '@clocky\\/cordis', count: 1 },
  { file: 'scripts/cordis-walk.ts', text: '!== \'@clocky/cordis\'', count: 1 },
  { file: 'scripts/gen-scoped-events.ts', text: '=== \'@clocky/cordis\'', count: 1 },
  { file: 'packages/typert/generator/src/analyzer.ts', text: '!== \'@clocky/cordis\'', count: 2 },
  { file: 'scripts/check-workspace-constraints.ts', text: '?.[\'@clocky/cordis\']', count: 2 },
  { file: 'packages/boot/app-boot/tsdown.config.ts', text: '[\'@clocky/cordis-plugin-include\']', count: 1 },
  { file: 'tsconfig.base.json', text: '"@clocky/cordis-plugin-loader": ["./vendor/loader/src"]', count: 1 },
  // The vendored README owns this required entry; reject its deletion or duplication.
  { file: 'vendor/README.md', text: '17. **`@clocky` rescope**', count: 1 },
...
const EXACT_EDITS: readonly ExactEdit[] = [
  {
    ...
    replace: 'const MERGE_HEAD = /declare module [\'"](?:@clocky\\/cordis|\\.\\/context\\.ts)[\'"]/',
    expect: 1,
  },
  {
    ...
    replace: `    const peer = manifest.peerDependencies?.['@clocky/cordis']
    const dev = manifest.devDependencies?.['@clocky/cordis']

    if (!peer) errors.push(\`\${label}: @clocky/cordis must be a peerDependency\`)
    if (!dev) errors.push(\`\${label}: @clocky/cordis must also be a devDependency\`)
    if (peer && dev && peer !== dev) {
      errors.push(\`\${label}: @clocky/cordis peer (\${peer}) and dev (\${dev}) ranges must match\`)`,
    expect: 1,
  },
  {
    id: 'knip-bundle-base',
...
`,
  ...
  },
  {
    ...
    find: '      if (!isVendored && !name.startsWith(\'@clocky/\')) {',
  ...
  },
  {
    ...
    replace: 'whose lazy `require(\'@clocky/cosmokit\')` can race',
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: '@clocky/cordis-plugin-timer  timer service',
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'vendored packages are rescoped ([mapping](docs/rescope.md)) and `private: true`. `@clocky/cordis` is a peerDependency (+ dev) of every harness package.',
    expect: 1,
  },
  {
    // The client purity gate reads `@clocky/` as "another plugin package".
    ...
    replace: `/**
 * Vendored framework libraries: rescoped into @clocky, so the gate below
 ...
 */
const VENDORED_LIBRARY = /^@clocky\\/(cosmokit|schemastery)(\\/|$)/
...
/** Generated descriptor/codec contribution with no shared runtime identity. */`,
  ...
  },
  {
    ...
    replace: "import { Context } from '@clocky/cordis'",
    expect: 1,
  },
  {
  ...
  },
  {
    ...
    replace: 'The Cordis framework and its foundation libraries are source-vendored into this repository rather than consumed from npm, and republished under the \\`@clocky\\` scope. All are MIT-licensed',
    expect: 1,
  },
  {
...
\${vendored.map(row => \`| \\\`\${row.npmName}\\\` | \\\`\${row.upstreamName}\\\` | [\${row.upstream.replace('https://', '')}](\${row.upstream}) | MIT |\`).join('\\n')}`,
  ...
  },
  {
    ...
    replace: 'parseVendoredRows(\'| `cordis/` | `@clocky/cordis` | cordis | 4.0.0 | https://example.com | `abc123` |\\n\')',
    expect: 1,
  },
  {
  ...
  // rescoped into @clocky, so the consumer installs this repository's
  ...
  },
  {
    ...
    writeFileSync(join(consumerDir, 'package.json'), JSON.stringify({ name: 'clocky-packed-consumer', private: true, type: 'module' }))
