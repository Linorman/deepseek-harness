 ...
 * silently changes composition. Example configs and the clocky Web composition
...
// These example files are overlays consumed by the built clocky app, so their bare
...
const CHOOSER_PACKAGE = '@clocky/clocky-host-directory-picker-auto'
 ...
 * `clocky.client` block, and the node half of a surface plugin is an empty
 ...
 * `clocky.client` declaration disagree.
 */
function validateClientHalvesDeclared(): string[] {
  return globSync('packages/client/*/package.json', { cwd: root }).flatMap((manifestPath) => {
    const manifest = readManifest(manifestPath) as PackageManifest & {
      ...
      clocky?: { client?: unknown }
    }
    const shipsClient = manifest.exports !== undefined && Object.hasOwn(manifest.exports, './client')
    const declaresClient = manifest.clocky?.client !== undefined
    if (shipsClient === declaresClient) return []
    return [shipsClient
      ? `${manifestPath}: exports "./client" but declares no clocky.client, so its browser half is never served`
      : `${manifestPath}: declares clocky.client but exports no "./client" entry to serve`]
  })
}
 ...
 *
 * Both have happened. `shell-env` in a preset realm left `CLOCKY_WEB_URL` reaching
...
function validateEntry(value: unknown, file: string, path: string): void {
  ...
  if (value.name !== '@clocky/cordis-plugin-include') return
...
function validateAppResolution(): string[] {
  ...
  // clocky app's own dependency surface — the profile module fallback mirrors it.
  ...
  for (const manifestPath of bundleManifests) {
    ...
    const patch = manifest.clocky?.bundle?.patch
...
export function bundleManifestPaths(repoRoot: string = root): string[] {
  return globSync('packages/*/*/package.json', { cwd: repoRoot })
    .filter(path => typeof readManifest(path, repoRoot).clocky?.bundle?.patch === 'string')
 ...
 * the tsconfig `paths` facade to a `.ts`/`.tsx` source file. The `clocky` source
