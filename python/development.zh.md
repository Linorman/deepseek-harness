...
uv run --project python/sdk python scripts/smoke-python-runtime.py \
  --scenario sdk-minimal --exe dist-exe/clocky-jsonrpc-agent-pkg-macos-arm64
```

其中两个场景会比对 `scripts/snapshots/python-sdk-single-exe/` 下已提交的期望输出。`minimal/model-visible.json` 固定了签入的极简组合所组装的系统提示词、对外公布的工具 schema 以及模型可见消息，因此插件一旦贡献出计划外的系统分段或 user 消息，该任务即失败；它会丢弃动态运行时上下文快照——同一组合在 macOS 上会发出它，在 Linux 上不会（[#2488](https://github.com/clocky/clocky/issues/2488)）。`advanced/` 固定 SDK 结果与持久化的会话日志。重新运行对应场景时加上 `--update-snapshots`，并在提交前审阅该差异。
...
- 设置 `CLOCKY_RUNTIME_MODE=node`，在系统 Node `>=22.19` 上使用已构建的 Node 载体。构建脚本会刷新该载体，但分发物绝不会包含或自动选择它。
...
根目录 `package.json` 的版本是两个 Python 分发包的权威版本。暂存脚本会将该版本注入两个 wheel 包，并将 SDK 固定到同版本的 `clocky-runtime-bin`。
...
python scripts/build-python-release.py --package runtime --platform macos-arm64 --runtime-exe dist-exe/clocky-jsonrpc-agent-pkg-macos-arm64 --output-dir dist-python
pip install \
  "dist-python/clocky_sdk-$version-py3-none-any.whl" \
  "dist-python/clocky_runtime_bin-$version-py3-none-macosx_14_0_arm64.whl"
```
