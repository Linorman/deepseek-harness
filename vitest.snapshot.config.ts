...
const snapshotMaxConcurrency = positiveIntFromEnv(
  'CLOCKY_SNAPSHOT_MAX_CONCURRENCY',
...
if (process.env.CLOCKY_SNAPSHOT === 'record') {
  try {
...
export default defineConfig({
  ...
  test: {
    ...
    include: [
      ...
      ...(process.env.CLOCKY_EXAMPLE_MODE === 'lib' ? ['apps/web/tests/**/*.snapshot.ts'] : []),
    ...
    testTimeout: 120_000,
    hookTimeout: 30_000,
    fileParallelism: (process.env.CLOCKY_SNAPSHOT || 'replay') === 'replay' && snapshotMaxConcurrency > 1,
