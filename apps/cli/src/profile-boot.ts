/**
 * Shared profile boot for every `clocky` surface: resolve the profile, stack its
 * patch layers (bundle layers in `clocky.profile.bundles` order, the profile's
 ...
 * @module @clocky/clocky/profile-boot
 */
...
import { CLOCKY_LAUNCH_ENVIRONMENT_KEY, type LaunchEnvironmentSnapshot } from '@clocky/clocky-launch-environment'
...
/**
 * The home-level user patch layer (`$CLOCKY_HOME/cordis.patch.yml`), applied
 * over every profile's own layer. Resolved per call, not at module load:
 * `$CLOCKY_HOME` may be set by the test or launcher after import.
 * @returns the absolute patch-file path.
 */
export function homePatchPath(): string {
  return join(resolveClockyHome(), PROFILE_PATCH_FILENAME)
}

/** Absolute path of this clocky installation's package.json (both anchors: src/ and lib/ sit one level under apps/cli). */
export const INSTALL_ANCHOR = fileURLToPath(new URL('../package.json', import.meta.url))

/** The session-telemetry row id the CLOCKY_TELEMETRY_DISABLED switch targets. */
const TELEMETRY_ROW_ID = 'session-telemetry-otel'

/** The empty root entry list every profile tree patches over. */
const PROFILE_ROOT_CONFIG = `# clocky profile root — an empty entry list. The tree is composed as patches:
# each bundle in package.json's clocky.profile.bundles, then cordis.patch.yml, then any
 ...
 * switch set.
 * @param disabledEnv - the raw `CLOCKY_TELEMETRY_DISABLED` value (`undefined` when unset).
...
interface ComposedProfile {
  ...
  /** The home-level user layer (`$CLOCKY_HOME/cordis.patch.yml`), applied after the profile's own. */
 ...
 * `clocky.profile.bundles` order (the base bundle gates the shell stacks by
 * platform on its own rows), the profile's user layer, the home-level user
 * layer (`$CLOCKY_HOME/cordis.patch.yml` — machine-local preferences that apply
...
function composeProfile(
  ...
  // The writable root the roster appends is `clocky-agent-presets`' own, so a
  ...
  const telemetryPatch = resolveTelemetryPatch(process.env.CLOCKY_TELEMETRY_DISABLED, rows.has(TELEMETRY_ROW_ID))
...
export async function runProfile(options: RunProfileOptions): Promise<{ ctx: Context; shutdown: ProcessShutdown }> {
  ...
  const ctx = await boot(NAME, rootConfig, structuredClone(allPatches(composed)), (hostCtx) => {
    ...
    hostCtx.provide(CLOCKY_LAUNCH_ENVIRONMENT_KEY, options.environment)
    ...
    try {
      ...
      if (ctx.get('hmr') === undefined) {
        if (ctx.get('timer') === undefined) {
          await ctx.loader.create({ name: '@clocky/cordis-plugin-timer' })
        }
        await ctx.loader.create({ name: '@clocky/cordis-plugin-hmr', config: { root: [] } })
      }
