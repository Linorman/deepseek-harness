/**
 * `clocky plugin --profile <name> <args...>` — profile plugin management as a
 * thin pnpm forwarder: initialize the profile on first use, run
 * `pnpm <args...>` in the profile directory, then reconcile the
 * `clocky.profile.bundles` layer list against the installed state (a dependency
 * resolving to a package that declares `clocky.bundle` joins the layer stack; a
 ...
 * gained its `clocky.bundle` declaration in a newer version.
 * @module @clocky/clocky/plugin
 */
 ...
 * @returns true when the package manifest declares `clocky.bundle`.
 */
function exportsPatch(packageName: string, profileDir: string): boolean {
  ...
  return manifest.clocky?.bundle?.patch !== undefined
}

/**
 * Reconcile `clocky.profile.bundles` against the installed state: pnpm has
 * already written the real installed names (so a git/path/tarball/alias spec
 * on the command line reconciles by its true package name) and materialized
 * the packages. A dependency that resolves to a `clocky.bundle`-declaring
...
function reconcilePlugins(before: ProfileManifest, profileDir: string): void {
  ...
  const plugins = after.clocky?.profile?.bundles ?? []
  let changed = false
  for (const packageName of dependencies) {
    ...
    if (isBundle && !plugins.includes(packageName)) {
      ...
      process.stderr.write(
        `${NAME}: warning: ${packageName} declares no clocky.bundle — installed as a plain dependency, not a profile layer `
  ...
  for (const packageName of [...plugins]) {
    ...
    // bundles (clocky-base and friends) are not dependencies.
  ...
  if (!changed) return
  after.clocky = { ...after.clocky, profile: { ...after.clocky?.profile, bundles: plugins } }
 ...
 * @param cwd - the directory `clocky` was invoked from.
...
/**
 * Run one `clocky plugin` invocation: init if needed, forward to pnpm, reconcile.
