...
import { parseClockyArgs } from '../src/args.ts'

const parse = (argv: string[]) => parseClockyArgs(argv, '1.2.3')
...
describe('parseClockyArgs', () => {
  ...
  it('routes the plugin pnpm forwarder', () => {
    ...
    expect(parse(['plugin', '--profile', 'tui', 'why', '@clocky/cordis']))
      .toEqual({ mode: 'plugin', profile: 'tui', args: ['why', '@clocky/cordis'] })
