...
import { startMockLlmServer } from '@clocky/clocky-llm-mock-server'
...
async function runBuiltBin(
  ...
  const result = await execa(process.execPath, [clockyBin, ...args], {
    input: '',
    timeout: 25_000,
    killSignal: 'SIGKILL',
    reject: false,
  ...
  if (result.timedOut) {
    throw new Error(`clocky built bin did not exit within 25s. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
  }
...
async function waitForFile(file: string): Promise<void> {
  ...
  while (!existsSync(file)) {
    if (Date.now() >= deadline) throw new Error(`clocky profile lifecycle marker did not appear: ${file}`)
 ...
 * clocky.profile.bundles, no clocky-base — proving out-of-box composition machinery without
 * booting the entire product tree.
 */
function createProfileLifecycleFixture(): ProfileLifecycleFixture {
  const home = mkdtempSync(join(tmpdir(), 'clocky-profile-lifecycle-'))
  ...
  writeFileSync(join(bundleDir, 'plugin.mjs'), [
    ...
    "  writeFileSync(join(process.env.CLOCKY_HOME, 'config-echo'), String(config.generation ?? 'bundle-default'))",
  ...
  writeFileSync(join(bundleDir, 'package.json'), JSON.stringify({
    name: 'clocky-lifecycle-bundle',
    version: '0.0.0',
    type: 'module',
    clocky: { bundle: { patch: './cordis.patch.yml' } },
  }, undefined, 2))
  const profileDir = join(home, 'profiles', 'lifecycle')
  mkdirSync(join(profileDir, 'node_modules'), { recursive: true })
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    name: 'clocky-profile-lifecycle',
    private: true,
    dependencies: {},
    clocky: { profile: { bundles: ['clocky-lifecycle-bundle'] } },
  }, undefined, 2))
  // Hand-place the "installed" bundle where profile resolution finds it.
  writeFileSync(join(profileDir, 'cordis.patch.yml'), '[]\n')
  const linkTarget = join(profileDir, 'node_modules', 'clocky-lifecycle-bundle')
  mkdirSync(join(profileDir, 'node_modules'), { recursive: true })
  try {
...
function startProfileLifecycle(fixture: ProfileLifecycleFixture, args: readonly string[] = []) {
  return execa(process.execPath, [clockyBin, '--profile', 'lifecycle', ...args], {
    cwd: fixture.home,
    input: '',
    reject: false,
    env: {
      CLOCKY_HOME: fixture.home,
...
function createEnvironmentProbeProfile(home: string, project: string, baseURL: string): void {
  ...
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    name: 'clocky-profile-environment-probe',
    private: true,
    dependencies: {},
    clocky: { profile: { bundles: ['@clocky/clocky-base'] } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'cordis.patch.yml'), [
    '- id: llm-pi-ai',
    '  config:',
...
/** Write an explicit provider selection for a profile test. */
function writeProviderPatch(home: string, baseURL: string): void {
 ...
 * `@clocky/clocky-cmdline` and `commander` through the profile module
 * fallback, exactly as an installed out-of-tree bundle does.
 */
function createStartupFixture(): StartupFixture {
  const home = mkdtempSync(join(tmpdir(), 'clocky-profile-startup-'))
  ...
  const bundleDir = join(profileDir, 'node_modules', 'clocky-startup-bundle')
  mkdirSync(bundleDir, { recursive: true })
  writeFileSync(join(bundleDir, 'startup.mjs'), [
    "import { Command } from 'commander'",
    "import { parseCmdline } from '@clocky/clocky-cmdline'",
  ...
  writeFileSync(join(bundleDir, 'witness.mjs'), [
    ...
    'export function apply(ctx, config = {}) {',
    "  writeFileSync(join(process.env.CLOCKY_HOME, 'witness'), String(config.generation ?? 'bundle-default'))",
    '}',
    '',
  ].join('\n'))
  writeFileSync(join(bundleDir, 'cordis.patch.yml'), [
  ...
  writeFileSync(join(bundleDir, 'package.json'), JSON.stringify({
    name: 'clocky-startup-bundle',
    version: '0.0.0',
    type: 'module',
    clocky: { bundle: { patch: './cordis.patch.yml' } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
    name: 'clocky-profile-startup',
    private: true,
    dependencies: {},
    clocky: { profile: { bundles: ['clocky-startup-bundle'] } },
  }, undefined, 2))
  writeFileSync(join(profileDir, 'cordis.patch.yml'), '[]\n')
  return {
    home,
...
function startStartupProfile(fixture: StartupFixture, args: readonly string[]) {
  return execa(process.execPath, [clockyBin, '--profile', 'startup', ...args], {
    cwd: fixture.home,
    input: '',
    reject: false,
    timeout: 25_000,
...
describe.skipIf(!existsSync(clockyBin))('clocky BUILT bin (node lib/bin.js, no tsx)', () => {
  it('requires --profile and rejects removed commands', async () => {
    ...
    expect(help.stdout).toContain('clocky --profile web')
    expect(help.stdout).toContain('clocky plugin --profile')
  ...
  it('routes help and usage errors without activating startup-dependent rows', async () => {
    const home = mkdtempSync(join(tmpdir(), 'clocky-app-help-'))
    try {
      ...
      expect(web.stdout).toContain('Usage: clocky --profile web')
      expect(web.stdout).toContain('--port <port>')
      expect(web.stdout).not.toContain('clocky web: http://')
      ...
      expect(wildcardHost.stderr).not.toContain('clocky web: http://')
      ...
      expect(headlessHelp.stdout).toContain('Usage: clocky --profile headless')
  ...
  it('runs the headless profile through its app-owned task positional', async () => {
    ...
    try {
      writeProviderPatch(home, server.baseURL)
  ...
  it('does not load a project environment for --version', async () => {
    const project = mkdtempSync(join(tmpdir(), 'clocky-version-project-'))
  ...
  it('fails loud on a nonexistent profile with the plugin-command hint', async () => {
    const home = mkdtempSync(join(tmpdir(), 'clocky-missing-profile-'))
    try {
      const result = await runBuiltBin(['--profile', 'nope'], { CLOCKY_HOME: home })
      expect(result.code).toBe(1)
      expect(result.stderr).toContain('profile "nope" does not exist')
      expect(result.stderr).toContain('clocky plugin --profile nope add')
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, 30_000)
  ...
  it('uses the launching endpoint and managed credential through the published entry', async () => {
    ...
    const home = mkdtempSync(join(tmpdir(), 'clocky-home-environment-'))
    const project = mkdtempSync(join(tmpdir(), 'clocky-home-project-'))
    writeFileSync(join(home, '.credentials.yaml'), `version: 1\nrefs:\n  DEEPSEEK_API_KEY: ${apiKey}\n`, { mode: 0o600 })
    createEnvironmentProbeProfile(home, project, server.baseURL)
    try {
  ...
  it('reports a patch-overlay boot failure without hanging', async () => {
    ...
    // refresh drain: clocky exited 13 with no diagnostic instead of settling
    // ([Agent Note](../../../.agents/notes/implemented/bug-fix/2026-08-03-hmr-initial-scan-boot-deadlock.md)).
    const home = mkdtempSync(join(tmpdir(), 'clocky-invalid-patch-'))
    try {
  ...
  it('fully settles a custom profile, hot-reloads its patch layer with removal reverting, and disposes on a signal', async () => {
    ...
    try {
      ...
      // The home-level user layer ($CLOCKY_HOME/cordis.patch.yml) is live too
  ...
  it('anchors a relative add spec to the invoking directory, not the profile', async () => {
    // `clocky plugin --profile x add .` from a plugin checkout must install THAT
    // checkout — pnpm's cwd is the profile directory, so an un-anchored `.`
    // would self-link the profile.
    const home = mkdtempSync(join(tmpdir(), 'clocky-plugin-anchor-'))
    const checkout = mkdtempSync(join(tmpdir(), 'clocky-plugin-checkout-'))
    try {
      ...
      const result = await execa(process.execPath, [clockyBin, 'plugin', '--profile', 'anchor', 'add', '.'], {
        cwd: checkout,
        input: '',
        timeout: 60_000,
        killSignal: 'SIGKILL',
        reject: false,
        env: { CLOCKY_HOME: home },
      })
      expect(result.exitCode).toBe(0)
      const manifest = JSON.parse(readFileSync(join(home, 'profiles', 'anchor', 'package.json'), 'utf8')) as {
        dependencies: Record<string, string>
        clocky: { profile: { bundles: string[] } }
      }
      expect(Object.keys(manifest.dependencies)).toEqual(['anchored-bundle'])
      expect(manifest.clocky.profile.bundles).toContain('anchored-bundle')
      ...
      expect(afterRemove.clocky.profile.bundles).not.toContain('anchored-bundle')
    } finally {
      rmSync(home, { recursive: true, force: true })
  ...
  it('activates a dependency that gained clocky.bundle in a later update', async () => {
    ...
    // version declares clocky.bundle. Simulated without a registry: hand-place
    // the installed package, flip its manifest, and run a benign pnpm verb.
    const home = mkdtempSync(join(tmpdir(), 'clocky-plugin-update-'))
    try {
      const profileDir = join(home, 'profiles', 'up')
      const installed = join(profileDir, 'node_modules', 'late-bundle')
      mkdirSync(installed, { recursive: true })
      writeFileSync(join(profileDir, 'package.json'), JSON.stringify({
        name: 'clocky-profile-up',
        private: true,
        dependencies: { 'late-bundle': 'file:./late-bundle' },
        clocky: { profile: { bundles: ['@clocky/clocky-base'] } },
      }))
      writeFileSync(join(profileDir, 'cordis.patch.yml'), '[]\n')
      // v1: no clocky manifest — a plain dependency.
      writeFileSync(join(installed, 'package.json'), JSON.stringify({ name: 'late-bundle', version: '1.0.0' }))
      const first = await runBuiltBin(['plugin', '--profile', 'up', 'root'], { CLOCKY_HOME: home })
      expect(first.code).toBe(0)
      let manifest = JSON.parse(readFileSync(join(profileDir, 'package.json'), 'utf8')) as { clocky: { profile: { bundles: string[] } } }
      expect(manifest.clocky.profile.bundles).toEqual(['@clocky/clocky-base'])
      // v2: the installed package now declares clocky.bundle (an update landed).
      writeFileSync(join(installed, 'package.json'), JSON.stringify({
        name: 'late-bundle', version: '2.0.0', clocky: { bundle: { patch: './cordis.patch.yml' } },
      }))
      writeFileSync(join(installed, 'cordis.patch.yml'), '[]\n')
      const second = await runBuiltBin(['plugin', '--profile', 'up', 'root'], { CLOCKY_HOME: home })
      expect(second.code).toBe(0)
      manifest = JSON.parse(readFileSync(join(profileDir, 'package.json'), 'utf8')) as { clocky: { profile: { bundles: string[] } } }
      expect(manifest.clocky.profile.bundles).toEqual(['@clocky/clocky-base', 'late-bundle'])
    } finally {
      rmSync(home, { recursive: true, force: true })
    }
  }, 30_000)
  ...
  describe('config dump', () => {
    let home: string
    beforeEach(() => { home = mkdtempSync(join(tmpdir(), 'clocky-dump-bin-')) })
    afterEach(() => { rmSync(home, { recursive: true, force: true }) })

    it('prints the web profile bundle layers without a user layer', async () => {
      const { stdout, code, stderr } = await runBuiltBin(['--profile', 'web', '--dump-default-config'], { CLOCKY_HOME: home })
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@clocky/clocky-agent-loop'")
    ...
    it('prints the headless profile without Host or browser layers', async () => {
      ...
      expect(code).toBe(0)
      expect(stderr).toBe('')
      expect(stdout).toContain("name: '@clocky/clocky-headless'")
    ...
    it('composes the profile user layer and a --patch overlay in order', async () => {
      ...
      const init = await runBuiltBin(['--profile', 'web', '--dump-default-config'], { CLOCKY_HOME: home })
