 ...
 * the REAL shipped bundle layers (clocky-base + clocky-web-app resolved from the
...
import { composeEntries, initProfile, loadProfile, PROFILES_DIR } from '@clocky/clocky-app-boot'
...
describe('the shipped shell composition (real bundle layers)', () => {
  ...
  // resolve from the REAL clocky-base/clocky-web-app packages through it, so this
  ...
  it('composes the confined pwsh roster on win32 and the bash roster on POSIX from the same rows', () => {
    home = mkdtempSync(join(tmpdir(), 'clocky-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'web'), ['@clocky/clocky-base', '@clocky/clocky-web-app'])
    const profile = loadProfile('clocky', 'web', anchor, home)
    ...
    for (const name of ['@clocky/clocky-pwsh-sandbox', '@clocky/clocky-tool-pwsh']) {
  ...
  it('base-only profiles carry both stacks with the same platform gating', () => {
    home = mkdtempSync(join(tmpdir(), 'clocky-windows-home-'))
    initProfile(join(home, PROFILES_DIR, 'base-only'), ['@clocky/clocky-base'])
    const profile = loadProfile('clocky', 'base-only', anchor, home)
    const warnings: string[] = []
    const rows = composeEntries(
      profile.layers.map(layer => layer.patches),
      message => warnings.push(message),
