...
const clockyBinScript = fileURLToPath(new URL('../src/bin.ts', import.meta.url))
...
markers = [b"clocky-test: never-dispose ready", b"clocky-test: never-dispose started"]
output = bytearray()
marker_index = 0
...
async function runHeadlessPtySmoke(): Promise<string> {
  const cwd = await mkdtemp(join(tmpdir(), 'clocky-headless-shutdown-'))
  try {
    ...
    await writeFile(join(profileDir, 'package.json'), JSON.stringify({
      name: 'clocky-profile-headless',
      private: true,
      dependencies: {},
      clocky: { profile: { bundles: ['@clocky/clocky-base', '@clocky/clocky-headless'] } },
    }, undefined, 2))
    ...
    const launch = resolveExampleLaunch({
      ...
      env: {
        ...
        CLOCKY_TEST_SHUTDOWN_ARM_FILE: join(cwd, 'shutdown-armed'),
      },
    })
    ...
    if (result.timedOut) {
      throw new Error(`clocky headless PTY driver did not exit. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
    if (result.failed) {
      throw new Error(`clocky headless PTY driver exited ${String(result.exitCode)}. stdout:\n${result.stdout}\nstderr:\n${result.stderr}`)
    }
...
describe.skipIf(process.platform === 'win32')('headless process shutdown (real Loader tree in a PTY)', () => {
  it('lets a second Ctrl+C force exit while the first signal is draining', async () => {
    ...
    expect(output).not.toContain('clocky: observing at ')
    expect(output).toContain('clocky-test: never-dispose ready')
    expect(output).toContain('clocky-test: never-dispose started')
