 ...
 * web profile (clocky-base + clocky-web-app bundle patches, auto-initialized).
...
const requireBuiltArtifacts = process.env.CLOCKY_REQUIRE_BUILT_CLI_SMOKE === '1'
...
function runBuiltWeb(cwd: string): Promise<{ stdout: string; stderr: string; code: number }> {
  return new Promise((resolveRun, rejectRun) => {
    const env: NodeJS.ProcessEnv = {
      ...process.env,
      DEEPSEEK_API_KEY: 'clocky-cli-smoke-dummy-key',
    ...
    child.stdout.on('data', (chunk: string) => {
      stdout += chunk
      if (!settled && /clocky web: http:\/\/127\.0\.0\.1:\d+/u.test(stdout)) {
        settled = true
        child.kill('SIGTERM')
      }
    })
...
describe.skipIf(!requireBuiltArtifacts)('built CLI lazy-search startup', () => {
  it('boots and disposes the shipped composition with full-text search off by default', async () => {
    ...
    const cwd = await mkdtemp(join(tmpdir(), 'clocky-cli-lazy-search-'))
    try {
      const result = await runBuiltWeb(cwd)
      expect(result.stdout).toMatch(/clocky web: http:\/\/127\.0\.0\.1:\d+/u)
