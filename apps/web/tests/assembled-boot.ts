...
import { bootInjections, orderByModuleGraph } from '@clocky/clocky-client-modules'
import type { ClientModuleLoaderTarget, WebBootEntry } from '@clocky/clocky-client-modules/client'
import { AppWebEntry } from '@clocky/clocky-client-web'
...
const appBoot = await import(pathToFileURL(webBundleResolver.resolve('@clocky/clocky-app-boot')).href) as unknown as BootComposition
...
function resolveClientExport(packagePath: string, pkg: ClientPackageManifest): string {
  ...
  if (relative === undefined) {
    throw new Error(`assembled boot: ${pkg.name ?? packagePath} declares clocky.client without a ./client export`)
  }
  return resolve(dirname(packagePath), relative)
}

/** Derive the assembled browser graph from the same bundle patches and package declarations as `clocky web`. */
function loadAssembledPlugins(): readonly AssembledPlugin[] {
  ...
  for (const entry of entries) {
    ...
    const declaration = pkg.clocky?.client
...
interface FixtureWindow extends Window {
  __CLOCKY_BOOT__?: { rev: string; entries: WebBootEntry[] }
...
export function mountAssembledApp(search = '?fixture'): void {
  ...
  win.__CLOCKY_BOOT__ = { rev: 'fx', entries: PLUGINS.map(({ bundlePath: _bundlePath, ...plugin }) => plugin) }
  const [facadeRow] = bootInjections(win.__CLOCKY_BOOT__)
  ...
  for (const id of ['@clocky/clocky-client-modules', '@clocky/clocky-client-runtime']) {
 ...
 * the snapshot gate's `CLOCKY_SNAPSHOT` mode (`record` re-runs the scenarios from
 * scratch, `refresh` re-derives the expected text from the existing ones).
 */
export const REFRESHING_GOLDEN = process.env.CLOCKY_SNAPSHOT === 'record' || process.env.CLOCKY_SNAPSHOT === 'refresh'
