...
// composition and wire. The model adapter is clocky-llm-replay with override
...
import type { RetryPolicyConfig } from '@clocky/clocky-llm'
import { deriveReplayScript, parseSessionLog } from '@clocky/clocky-llm-replay'
...
describe('web e2e: live-turn interactions (cancel / error / retry)', () => {
  ...
  async function launch(
    ...
    if (buildOverride !== undefined) {
      ...
      sidecarDir = await mkdtemp(join(tmpdir(), 'clocky-web-e2e-sidecar-'))
