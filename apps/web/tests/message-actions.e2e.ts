...
// completed-turn-tail case (zero model calls) and pins the settled
 ...
 * copy/clock; the second is the completed turn tail.
...
function completedTailFixture(raw: string): string {
  ...
  const tail = [
    ...
    at({ type: 'assistant/message', data: { turn: 2, step: 1, content: [{ type: 'text', text: 'DONE' }], provenance: { provider: 'test-provider', model: 'test-model' } }, sourceEventSeqs: [], surfaceOp: 'append' }),
...
describe('web e2e: message IconActions and clocks on settled history', () => {
  ...
  it.skipIf(MODE === 'record')('shows Copy controls on settled message tails', async () => {
