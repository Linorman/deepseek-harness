...
const STANDALONE_ERROR = 'apps/web is not a standalone application: bare Vite cannot inject window.__CLOCKY_BOOT__. '
  + 'From a repository checkout, run `pnpm clocky web`; an installed package uses `clocky web`. '
  + 'For client-plugin HMR, run `pnpm clocky web` together with `pnpm run dev:web`.'
const DEFAULT_CLIENT_TITLE = 'Clocky'
...
function clientDocumentTitle(): Plugin {
  const title = escapeHtmlText(process.env.CLOCKY_CLIENT_TITLE ?? DEFAULT_CLIENT_TITLE)
  return {
    name: 'clocky-client-document-title',
    transformIndexHtml(html) {
      return html.replace('<title>Clocky</title>', `<title>${title}</title>`)
    },
  }
}
...
function rejectStandaloneServe(): Plugin {
  return {
    name: 'clocky-reject-standalone-web-serve',
    config(_config, env) {
