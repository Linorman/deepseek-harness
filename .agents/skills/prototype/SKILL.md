...
Use the coherent component, screen, or flow the user requested as the comparison unit. For a multi-component request such as a dashboard, keep its related pieces in each variant; do not silently reduce the task to one widget. Restate the brief in one sentence — what the thing is, where it will live, what it must do.
...
Present the working set before requesting a choice. The choice belongs to the user unless they already selected a direction or explicitly delegated selection; in that case, explain the choice and continue to promotion:
