...
Apply this guidance directly to the user's current UI task, including when the skill is selected automatically. If the user invokes only the skill name with no task or usable context, briefly ask what interface or interaction they want to work on. Do not insert a readiness-only response into an ongoing task.
...
Use the user's requested format. For several comparable changes, a `Before | After | Why` table is useful; for a single finding, use a concise paragraph with its location, effect, and suggested correction. Report only supported findings and omit empty sections. A review produces recommendations; an authorized implementation request continues through edits and verification.
