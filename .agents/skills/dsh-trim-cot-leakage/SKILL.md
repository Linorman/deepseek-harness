...
1. Resolve scope and exclusions per [dsh-prose-standard](../dsh-prose-standard/SKILL.md) from the request or current task; never touch `vendor/`, `.agents/notes/archived/`, or recorded fixtures and snapshots — recorded model output and sealed history keep their original voice.
