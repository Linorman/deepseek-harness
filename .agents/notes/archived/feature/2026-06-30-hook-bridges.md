# Agent Note: clocky-hooks-claude-code + clocky-hooks-codex — the Claude Code / Codex hook bridges
...
Matcher semantics, exit-code handling, and merge precedence live in `clocky-hook-protocol`; each bridge only parses config, builds dialect payloads, and maps outcomes. Per-file coverage includes config branches plus end-to-end mappings through a real loop, `clocky-bash-local`, and shell scripts, while a real-Loader smoke guards the package export shape. Native plugins bypass the wire protocol and return typed decisions directly.
