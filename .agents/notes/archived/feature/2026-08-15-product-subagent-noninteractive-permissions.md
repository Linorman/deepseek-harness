...
| Diagnostic type and byte limit | `clocky-subagent` | Consumers receive a bounded optional field separate from assistant output. |
| Foreground and Job presentation | `clocky-tool-subagent` and the generic Job runtime | Scheduling choice does not change the underlying failure fact. |
| Process cancellation and quiescence | Product Provider and `clocky-subprocess` | Result settlement still precedes idempotent whole-tree disposal. |
