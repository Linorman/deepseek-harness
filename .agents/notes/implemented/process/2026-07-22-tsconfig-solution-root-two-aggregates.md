...
- The typecheck and build references lists drifted apart (`packages/compat/command-goal` was in the typecheck graph but missing from the build graph).
...
Client projects disable source-of-project-reference redirection for typed lint. Referenced declarations omit implementation-only imports that would otherwise load Host `Context` augmentations into the Client program. Public lint runs the existing Host-then-Client typecheck preparation, and CI lint depends on that Client check. An isolated four-project regression checks both valid Client calls and rejected Host-only calls without relying on workspace build residue.
