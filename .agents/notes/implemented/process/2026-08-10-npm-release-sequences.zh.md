...
`packages/*/*` 与 `apps/*` 组成 `@clocky/clocky` 的运行面；`vendor/*` 是九个 rescope 过的 Cordis 框架包，各自带着上游的版本号；`native/landlock-run/packages/*` 是 Linux 平台包，有自己的 workflow。三组的版本基线、变更节奏和构建要求都不同：clocky 随产品迭代，vendor 只在同步上游或改动本地修改时才动，native 需要 musl 工具链和逐架构构建。把它们塞进一条发布流水线，等于每次产品发版都要重发框架和原生二进制。

挡路的还有两处硬门。全部 217 个 workspace manifest 都是 `private: true`，`npm publish` 直接拒绝。更隐蔽的是 933 条 clocky 兄弟包之间硬写的 `peerDependencies: "^0.0.1"`：`pnpm pack` 只替换 `workspace:` 协议，不动语义范围，而 `^0.0.1` 等于 `>=0.0.1 <0.0.2`——发 `0.0.2` 落不进去，发 `0.0.1-rc.1` 也落不进去（semver 规定不带预发布段的范围排除预发布版本）。这些条目至今没出事，只因为版本一直停在 `0.0.1`。
...
`packages/`、`vendor/`、`native/` 各自一条 bump 序列、各自一次发布，不共享版本号、不共享触发、不互相等待。发 clocky 不重发 vendor，发 vendor 不重发 native。

| 序列 | 成员 | 版本基线 | tag | workflow |
|---|---|---|---|---|
| clocky | 发布集：非 experimental 的 `packages/*/*` + `apps/*`；私有实验性包仅加入共享版本 bump | 发布集、私有 clocky 包与 workspace 根共用一个 `0.0.x` | `clocky-v<版本>` | `release.yml`（pack）/ `release-publish.yml`（发布） |
...
三组一律发到 npmjs.com 的 `@clocky` scope，且 access 按序列而非按 scope 区分：vendored 框架与 native 包是 `public`，clocky 族是 `restricted`（[理由](2026-08-13-public-vendor-and-native-sequences.zh.md)）。没有任何发布路径传 `--access`——一个选项无法服务级别互不相同的序列，且会覆盖真正拥有该级别的 manifest。
...
`release:clocky` 接受 `major`、`minor`、`patch` 或显式版本号，把同一个版本写进可发布族、`packages/*/*` 下的每个私有包**以及 workspace 根**。私有包不会获得发布 tag，仍位于 pack 与 publish 之外；它们跟随版本是因为 workspace 约束要求每个 clocky 包的版本等于根版本。根的检查接受预发布段。像 `0.0.1-rc.1` 这样的预发布号先把 pack、已安装产物探针和一次真实私有发布跑通，数字版本随后。dist-tag 沿用 `landlock-run-release.yml` 已有的判定：版本带预发布段就 `--tag next`，否则进 `latest`。
...
`scripts/check-workspace-constraints.ts` 要求这个协议，所以新包无法再引入硬写的范围；同理，invariant companion 规则要求 `@clocky/clocky-invariants` 用 `workspace:^`。
...
clocky 族套用仓库的发布 payload 策略（拒绝源码与声明映射）。vendored 族保留上游 payload，因为那些 manifest 导出 `./src/*`，去掉 `src` 会发出一个导出映射指向不存在文件的包。
...
clocky 的验证会一并安装 vendored 族的 pack 产物。harness 的包把 vendored 框架声明成 peer，而那些包属于另一条序列，无凭据的 job 无法从私有 registry 取到——所以 clocky 的 `pack` job 为验证而打包 vendored 族，发布的仍只有 clocky 那一份。发布工作流（`release-publish.yml`）重新打包当前树，只发布 clocky 族。

验证还会打一份 Landlock entry 的 tarball——`clocky-sandbox-local` 把它声明为普通 `dependencies`——并保持启用可选依赖，使 `koffi` 这类与宿主匹配的 native prebuild 能在 consumer 中安装，而不是在 consumer 内编译。npm 会按 `os`/`cpu` metadata 过滤 Landlock platform package；当前宿主需要的 platform package 会在 release lane 包含它时通过 `--from` 提供。因此验证按目录内容读取 tarball，而不是读发布顺序：一个目录可能只装着为满足跨序列依赖而打出来的包，任何发布顺序都不描述它。clocky release rehearsal 会上传这些完全相同的 tarball 作为 consumer input，并在 Linux、macOS、Windows 上运行隔离安装探针；Ubuntu 仍是可发布 bytes 的唯一 pack 来源。
...
一个 pull request 会为两条序列跑完整的 pack（无凭据），并把打包好的 clocky tarball 装进一次性 consumer，用普通 Node 驱动 `clocky --version`。这个探针刻意只有一条命令：它证明 `files` 选出了完整 payload、发布出去的范围可解析，不涉及任何交互行为。
...
- **首发是一次大步。** 九个 vendored 包与整个 clocky 集一次发出，任何 payload 缺陷都会集中在同一次发布里暴露——这正是先用预发布版本把完整链路走一遍的理由。
