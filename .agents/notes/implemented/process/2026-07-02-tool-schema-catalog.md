...
Shipped product tool packages under `packages/*/tool-*`, each booted with its default config, including `clocky-tool-bash` (`bash`), `clocky-tool-jobs` (`job_output`, `job_list`, `job_kill`), and `clocky-tool-subagent` (`subagent`). Example-only tools are excluded.
