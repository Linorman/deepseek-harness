...
`packages/*/*` and `apps/*` form the runtime surface of `@clocky/clocky`; `vendor/*` holds nine rescoped Cordis framework packages, each carrying its upstream version; `native/landlock-run/packages/*` holds Linux platform packages with their own workflow. The three differ in version baseline, change rate, and build requirements: clocky moves with the product, vendor moves only when upstream is re-synced or a local modification changes, and native needs a musl toolchain and one build per architecture. Forcing them through one pipeline means every product release republishes the framework and the native binaries.

Two hard blockers sat in the way. All 217 workspace manifests set `private: true`, which npm refuses to publish. The subtler one was 933 hand-written `peerDependencies: "^0.0.1"` entries between sibling clocky packages: `pnpm pack` substitutes the `workspace:` protocol but leaves semver ranges alone, and `^0.0.1` means `>=0.0.1 <0.0.2` — it excludes `0.0.2`, and semver excludes prereleases from a range without a prerelease of its own, so it excluded `0.0.1-rc.1` too. Those entries never failed only because the version never left `0.0.1`.
...
`packages/`, `vendor/`, and `native/` each have one bump sequence and one publication, sharing no version, no trigger, and no waiting. Releasing clocky does not republish vendor; releasing vendor does not republish native.

| Sequence | Members | Version baseline | Tag | Workflow |
|---|---|---|---|---|
| clocky | Publish set: non-experimental `packages/*/*` + `apps/*`; private experimental packages join only the shared version bump | one version for the publish set, private clocky packages, and workspace root, `0.0.x` | `clocky-v<version>` | `release.yml` (pack) / `release-publish.yml` (publish) |
...
`release:clocky` accepts `major`, `minor`, `patch`, or an explicit version, and writes one version across the publishable family, every private package under `packages/*/*`, **and the workspace root**. Private packages receive no release tag and remain outside pack and publish; they follow the version because the workspace constraint requires every clocky package's version to equal the root's. The root check accepts a prerelease segment. A prerelease such as `0.0.1-rc.1` drives pack, the installed-artifact probe, and one real private publication before numbered versions follow. The dist-tag decision is the one `landlock-run-release.yml` already made: a version with a prerelease segment publishes under `--tag next`, anything else takes `latest`.
...
|---|---|---|
| `@clocky/cordis` | 4.0.0-rc.7 | 4.0.1 |
| `@clocky/cordis-plugin-loader` | 1.0.0-rc.5 | 1.0.1 |
| `@clocky/cosmokit` | 1.8.1 | 1.8.2 |
| `@clocky/schemastery` | 3.18.0 | 3.18.1 |
| `@clocky/cordis-plugin-hmr` | 1.0.15 | 1.0.16 |
| `@clocky/cordis-plugin-include` | 1.0.4 | 1.0.5 |
| `@clocky/cordis-plugin-timer` | 1.1.2 | 1.1.3 |
| `@clocky/cordis-plugin-group` | 1.0.0 | 1.0.1 |
| `@clocky/cordis-plugin-logger-console` | 1.0.0 | 1.0.1 |
...
`scripts/check-workspace-constraints.ts` requires the protocol, so a new package cannot reintroduce a hand-written range; the invariant-companion rule requires `workspace:^` for `@clocky/clocky-invariants` for the same reason.
...
The clocky family applies the repository's publication payload policy, which rejects sources and declaration maps. The vendored family keeps upstream's payload, because those manifests export `./src/*` and dropping `src` would publish an export map pointing at absent files.
...
A clocky verification installs the vendored family's pack output too. The harness packages declare the vendored framework as a peer, those packages live in another sequence, and the credential-free job cannot fetch them from a private registry — so the clocky `pack` job packs the vendored family for verification while publishing only the clocky set. The publish workflow (`release-publish.yml`) repacks the current tree and publishes only the clocky set.

The verification also packs the Landlock entry, which `clocky-sandbox-local` declares as a plain dependency, and keeps optional dependencies enabled so host-matched native prebuilds such as `koffi` are installed instead of being compiled in the consumer. npm filters the Landlock platform packages by their `os`/`cpu` metadata; a current-platform package is supplied through `--from` when the release lane includes it. The verification therefore reads a directory by its contents rather than a pack order, because a directory can hold tarballs packed only to satisfy a cross-sequence dependency. The clocky release rehearsal uploads those exact tarballs as consumer inputs and runs the isolated install probe on Linux, macOS, and Windows; Ubuntu remains the sole pack source for publishable bytes.
...
A pull request runs the full pack for both sequences without credentials and installs the packed clocky tarballs into a throwaway consumer, where plain Node drives `clocky --version`. That probe is deliberately one command: it proves `files` selected a complete payload and that the published ranges resolve, and says nothing about interactive behavior.
...
- **The first publication is one large step.** Nine vendored packages and the whole clocky set publish at once, so any payload defect surfaces in a single release, which is why a prerelease version drives the complete path first.
