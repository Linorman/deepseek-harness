...
|---|---|---|
| `@clocky/clocky-typert-protocol` | Declares only the minimal `ctx.typert` protocol | `TypertRemoteService`, decorators, binding fallback, descriptors, lookup/Context, and the Remote map; no dependency on the compiler, Zod, Connection, or Browser |
...
Business packages depend only on the lightweight `@clocky/clocky-typert-protocol`. It provides `TypertRemoteService` and declaration protocols for decorators, the binding fallback, lookup, Remote Scope, and descriptors, without depending on the TypeScript compiler, Zod, HTTP, or the Client runtime.
...
In SRC mode, the decorator may record the prototype, method name, and invocation mode in a `WeakMap` internal to `clocky-typert-protocol`. It writes no custom properties to a Service instance, prototype, constructor, or method function.
...
- `@clocky/clocky-typert-protocol`: lightweight protocols for decorators, bindings, lookup, Remote Scope, and descriptors.
...
- `@clocky/clocky-api-remotes`: the BFF layer; owns the Host Agent/Session resolver, selects Client `/remote` contributions, and exposes the merged Remote types to business packages through the shared `TypertClientRemote` contract.
