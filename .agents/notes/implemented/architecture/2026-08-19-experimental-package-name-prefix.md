...
Every package directly below `packages/experimental/` uses the `@clocky/clocky-experimental-*` npm prefix. The workspace constraints gate discovers those manifests and rejects a missing prefix alongside the existing `private: true` and omitted-`publishConfig` requirements.
