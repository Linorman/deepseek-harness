...
仓库还曾在两种含义下使用 `SDK`。受支持的 Python 和 TypeScript 客户端使用 JSON-RPC SDK 协议。项目整体是 Clocky，而不是 SDK 项目。已移除的 SDK 项目工具链使宽泛的含义失去依据，但文案和名称仍保留了部分旧用法。
...
`SDK` 表示受支持的 Python 和 TypeScript SDK 所使用、基于 JSON-RPC 的客户端／服务器协议。仓库保留 `@clocky/clocky-sdk-client`、`@clocky/clocky-sdk-protocol` 和协议身份 `clocky-sdk-runtime`；JSON-RPC 服务器属于同一系列。Clocky 本身不是 SDK，已移除的项目生成器、启动器、辅助工具和启动器遥测包继续保持不存在。
...
不得为了避免重复而删除有意保留的供应商限定词。`clocky-subagent-clocky-sdk` 表示 Clocky SDK 提供方，可避免与其他 SDK 混淆。其私有类改名为 `SdkSubagentProvider`，因为类名还需要说明它提供什么。
...
| 旧名称 | 当前名称 | 理由 |
|---|---|---|
| `@clocky/clocky-jsonrpc` | `@clocky/clocky-sdk-jsonrpc-server` | 它是 SDK 协议的服务器端。单独使用 `jsonrpc` 只说明编码；`sdk-jsonrpc-server` 则同时说明所属系列、机制和职责。 |
| `HarnessSdkServer` | `HarnessSdkJsonRpcServer` | 该类是 JSON-RPC 服务器的一种实现，并不代表所有可能的 SDK 服务器。 |

保留 `@clocky/clocky-sdk-client`、`@clocky/clocky-sdk-protocol` 和 `clocky-sdk-runtime`。排除 `@clocky/create-sdk`、`@clocky/clocky-scripts`、`@clocky/clocky-helper` 和 `@clocky/clocky-telemetry`；单独的移除决策负责删除这些包及其支撑依赖图。
...
| `@clocky/clocky-bash`, `ctx.bash` | `@clocky/clocky-shell`, `ctx.shell` | PowerShell 已经实现该 seam。此项能力是 shell 执行，而不是 Bash。 |
...
| `@clocky/clocky-bash-env`, `ctx.bashEnv`, `BashEnvRegistry` | `@clocky/clocky-shell-env`, `ctx.shellEnv`, `ShellEnvRegistry` | Bash 和 PowerShell 工具共享该环境注册表。 |
...
| `@clocky/clocky-pty`, `ctx.pty`, `PtyService` | `@clocky/clocky-terminal`, `ctx.terminals`, `TerminalSessionService` | 调用方管理多个具名终端会话，而不是通过该服务分配原始 PTY。 |
| 公开的高层 `Pty*` 会话和后端名称 | `Terminal*` 名称 | 公开抽象是终端会话。保留底层 `SubprocessTerminal*` 名称，因为它们已经说明底层机制。 |
| `@clocky/clocky-pty-local`, `LocalPtyBackend` | `@clocky/clocky-terminal-bash`, `BashTerminalBackend` | 该提供方依赖 Bash 提示符和 shell 行为。`local` 隐藏了实际方言。 |
| `@clocky/clocky-tool-pty` | `@clocky/clocky-tool-terminal` | 面向模型的工具已使用 `terminal_*`；包应采用相同的产品名词。 |
...
| 旧名称 | 当前名称 | 理由 |
|---|---|---|
| `@clocky/clocky-lsp-local` | `@clocky/clocky-lsp-stdio` | 该提供方通过可替换的文件系统和子进程服务，以 stdio 传输 LSP。它不一定在本地运行。 |
| `packages/tasks/` | `packages/jobs/` | 该系列负责脱离前台运行的工具作业。`jobs` 简短，并可避免与用户任务或 todo 概念冲突。 |
| `@clocky/clocky-tasks`, `ctx.tasks`, `TaskService` | `@clocky/clocky-jobs`, `ctx.jobs`, `JobRegistry` | 该服务注册、拥有、观察、等待并取消多个后台作业。它是注册表，而不是通用任务服务。 |
...
| `@clocky/clocky-tasks-local`, `LocalTaskService` | `@clocky/clocky-jobs-local`, `LocalJobRegistry` | 这是作业注册表的进程内提供方。此处的 `local` 有明确含义，因为作业和回调都存在于同一进程。 |
| `@clocky/clocky-tool-tasks` | `@clocky/clocky-tool-jobs` | 消费方控制作业注册表，应使用相同的领域名词。 |
...
| `@clocky/clocky-client-ui-task`、`client/ui-task/` | `@clocky/clocky-client-ui-jobs`、`client/ui-jobs/` | 该客户端包呈现后台作业集合，而不是一项用户任务。 |
...
| 旧名称 | 当前名称 | 理由 |
|---|---|---|
| `@clocky/clocky-client-ui-slash`, `ui-slash/` | `@clocky/clocky-client-ui-input-trigger`, `ui-input-trigger/` | 客户端处理 `/`、`@`、键盘仲裁、候选菜单和程序化启动，并非只处理斜杠命令。 |
...
| `@clocky/clocky-agent-tool-mode`，插件 `tool-mode` | `@clocky/clocky-agent-tool-presentation`，插件 `tool-presentation` | 该插件改变工具向模型呈现的方式，而不改变执行行为。保留局部 `Config.mode` 和 `ToolPresentationMode`。 |
| `packages/interaction/permission/` | `packages/interaction/permission-presets/` | 该包拥有沙箱与审批设置的具名组合，而不负责执行权限。 |
| `@clocky/clocky-permission`, `ctx.permission`, `PermissionService` | `@clocky/clocky-permission-presets`, `ctx.permissionPresets`, `PermissionPresetService` | 该服务选择并持久化预设。沙箱和审批服务负责执行结果。 |
| `@clocky/clocky-client-ui-permission` | `@clocky/clocky-client-ui-permission-presets` | UI 编辑和选择权限预设。 |
| `docs/subsystems/permission.md` | `docs/subsystems/permission-presets.md` | 该页面记录预设选择，而不是权限执行。 |
| `@clocky/clocky-user-interaction`, `user-interaction/` | `@clocky/clocky-user-questions`, `user-questions/` | 该 seam 仅支持批量问题和答案。审批、命令和目录选择属于其他交互 seam。 |
| `ctx.userInteraction`, `UserInteractionService`, `UserInteractionProvider`, `UserInteractionError` | `ctx.userQuestions`, `UserQuestionService`, `UserQuestionProvider`, `UserQuestionError` | 这些名称说明唯一受支持的交互形式。保留 `AskUserQuestion*`、`ask_user_question` 工具和 `@clocky/clocky-tool-ask-user`。 |
...
| 旧名称 | 当前名称 | 理由 |
|---|---|---|
| `packages/typert/type-meta/`, `@clocky/clocky-type-meta` | `typert/protocol/`, `@clocky/clocky-typert-protocol` | 该包拥有 Typert Remote 协议、装饰器、绑定、编解码器、查找逻辑和上下文约定。它不是通用类型元数据。 |
...
保留 `@clocky/clocky-tools` 和 `ctx.tools`。保留 `@clocky/clocky-api-gateway`、其 `gateway/` 目录、`ctx.typertGateway` 以及 `TypertGatewayService`；该服务是真正的 API 网关。其内部的 `TypeRT*` 标识符仍应遵循 `Typert*` 拼写规则。

### 工作区指令、遥测、身份和启动环境

| 旧名称 | 当前名称 | 理由 |
|---|---|---|
| Host `ctx.workspace` | Host `ctx.workspaceRegistry` | `WorkspaceRegistry` 拥有多个工作区，但 Client `ctx.workspaces` 已经使用不兼容的类型。即使二者运行时上下文独立，两份声明仍会在编译时合并进同一个 Cordis `Context` 接口。职责后缀明确指出 host 服务，并避免该冲突。保留 `@clocky/clocky-workspace`、`WorkspaceRegistry`、`Workspace` 和 `workspace.*` 协议名称。 |
| `@clocky/clocky-workspace-context`, `context/workspace-context/` | `@clocky/clocky-agent-instructions`, `context/agent-instructions/` | 该包为 agent（智能体）加载分层的 `AGENTS.md` 和 `CLAUDE.md` 文件。它并非通用工作区上下文。 |
...
| `TelemetryOtel`、`TelemetryMode`，插件 `telemetry-otel` | `OpenTelemetrySessionBackend`、`SessionTelemetryMode`，插件 `session-telemetry-otel` | 提供方名称同时说明 OpenTelemetry 机制和会话作用域。保留包名 `clocky-session-telemetry` 和 `clocky-session-telemetry-otel`。 |
| `docs/subsystems/telemetry.md` | `docs/subsystems/session-telemetry.md` | 该页面记录会话遥测，而不是仓库级可观测性。 |
| `session/user-id/`、`@clocky/clocky-user-id` | 已删除；无替代包 | 该随机跨服务关联 ID 已无当前消费方。它既不属于 Session 领域，也不是经过身份验证的用户身份。 |
| `USER_ID_FILE_NAME`、`.userid`、反馈标签 `User` | 随关联 ID 一并删除 | 文件和 UI 不再暴露匿名身份，也不再生成仓库创建的 OTel `user.id`。 |
| `util/environment/`, `@clocky/clocky-environment` | `util/launch-environment/`, `@clocky/clocky-launch-environment` | 该包在启动时捕获一份不可变的分层快照。它不是通用环境 API。 |
| 公开的 `Environment*`、`createEnvironmentSnapshot`、`environmentOf`、`CLOCKY_ENVIRONMENT_KEY` | `LaunchEnvironment*`、`createLaunchEnvironmentSnapshot`、`launchEnvironmentOf`、`CLOCKY_LAUNCH_ENVIRONMENT_KEY` | 这些名称说明快照的生命周期和用途。 |
...
| `@clocky/clocky-compact-basic`、`BasicCompactService`、公开的 `BasicCompact*` | `@clocky/clocky-compaction-basic`、`BasicCompactionEngine`、对应的 `BasicCompaction*` | `basic` 朴素但准确。`compaction-llm` 没有增加信息，因为当前实现系列已使用 LLM。 |
| `@clocky/clocky-compact-tool-result-prune`, `ToolResultPruneService`, `ctx.toolResultPrune` | `@clocky/clocky-compaction-tool-result-pruner`, `ToolResultPruner`, `ctx.toolResultPruner` | 该插件是剪除工具结果的执行主体。名词 `pruner` 说明了这一职责。 |
...
| `@clocky/clocky-settings-local`, `SettingsLocal` | `@clocky/clocky-settings-file`, `FileSettingsProvider` | 该实现通过文件系统 seam 以文件为后端。`file` 说明机制，`local` 则不能。 |
...
| `@clocky/clocky-session-export`, `session-export/`, Loader id `session-export`, `ctx.sessionExport` | `@clocky/clocky-session-log-export`, `session-log-export/`, Loader id `session-log-download`, `ctx.sessionLogDownload` | npm 包名使用 Session 日志导出语义，因为 npm 禁止包名包含 `download`。Loader id 与浏览器 API 保留 `download`，因为它们描述浏览器副作用。 |
...
| 旧名称 | 当前名称 | 理由 |
|---|---|---|
| `HttpServerService`, `ctx.httpServer` | `WebServer`, `ctx.webServer` | 该服务器拥有 HTTP 路由和 WebSocket 升级路由。`Web` 可以同时涵盖两者；此处的 `Http` 作用域过窄。保留 `packages/host/webserver`、`@clocky/clocky-host-webserver`、`WebRoute` 和 `WebUpgradeRoute`。 |
...
| `@clocky/clocky-session-title-first-message-llm`，触发周期 `first-message` | `@clocky/clocky-session-title-first-prompt-llm`，触发周期 `first-prompt` | 触发条件是第一条用户提示词，而不是会话日志中的任意消息。 |
| `@clocky/clocky-session-title-all-messages-llm`，触发周期 `all-user-messages` | `@clocky/clocky-session-title-all-prompts-llm`，触发周期 `all-prompts` | 后端根据用户提示词刷新。`all messages` 会错误地包含助手消息和工具事件。 |
| `@clocky/clocky-code-runtime-worker`, `WorkerCodeRuntime` | `@clocky/clocky-code-runtime-worker-thread`, `WorkerThreadCodeRuntime` | 该实现使用 Node 工作线程。单独的 `worker` 作用域过宽。 |
...
| 旧名称 | 当前名称 | 理由 |
|---|---|---|
| `@clocky/clocky-fs-policy` | `@clocky/clocky-fs-observation-policy` | 该包定义哪些文件系统观察可以授权后续操作。它不是完整的文件系统策略或沙箱策略。 |
...
| 旧名称 | 当前名称 | 理由 |
|---|---|---|
| `@clocky/clocky-repeat-tool-guard`，插件／来源 `repeat-tool-guard` | `@clocky/clocky-repeat-tool-reminder`，插件／来源 `repeat-tool-reminder` | 该插件向模型添加提醒，并不阻止工具调用，也不执行防护决策。 |
| `@clocky/clocky-timeout-policy` | `@clocky/clocky-tool-call-timeout-policy` | 完整的 `tool-call` 限定词说明该策略限制的对象，而不会把插件称为面向模型的工具。保留其 `guard/timeout-policy/` 目录和插件 id `timeout-policy`；`packages/*/tool-*` 目录约定仍只适用于注册工具的包。 |
...
| `InvariantService` | `InvariantRegistry` | 该对象拥有已注册的不变量检查。保留 `@clocky/clocky-invariants` 和 `ctx.invariants`。 |
...
| 旧名称 | 当前名称 | 理由 |
|---|---|---|
| `util/paths/`, `@clocky/clocky-paths` | `util/home-paths/`, `@clocky/clocky-home-paths` | 这些辅助函数解析 Harness 主目录下的路径，并非通用路径库。已准确说明返回路径的函数名保持不变。 |
| `util/retention/`, `@clocky/clocky-retention` | `util/output-retention/`, `@clocky/clocky-output-retention` | 该策略保留命令和工具输出，而不是通用数据保留框架。 |
| `E2BSandboxService` | `E2BRuntime` | 该类创建、复用和释放文件系统与子进程适配器所使用的 E2B 执行环境。它比单个沙箱句柄的职责更广，又比通用所有者更具体。保留 `@clocky/clocky-e2b`、`ctx.e2b` 和 `e2b/` 组。 |
| `@clocky/clocky-frontend-static` | `@clocky/clocky-host-frontend-static` | 该包是提供前端资源的 Host 插件。此前缀可将它与前端应用代码区分开。 |
...
| `@clocky/clocky-jsonrpc-demo` | `@clocky/clocky-sdk-jsonrpc-demo` | 该示例演示通过 JSON-RPC 使用运行时 SDK，属于 SDK 的唯一含义。 |
| `@clocky/clocky-frontend` | `@clocky/clocky-web-frontend` | 该应用是 Web 前端。保留其物理目录 `apps/web/`。 |

保留 atomic-write、brand、native-command、timeout 实用工具、目录选择器、`clocky-base`、`clocky-web-app`、应用启动、CLI（命令行界面）名称，以及 `headless` 包、组合包和示例身份。`headless` 是预期的产品本质，未来也可以支持不止一次性执行。
...
| `@clocky/clocky-client-ui-model` | `@clocky/clocky-client-ui-model-selection` | 该包控制会话的模型选择。单数 `model` 名称作用域过宽。 |
...
| `@clocky/clocky-client-ui-models` | `@clocky/clocky-client-ui-settings-models` | 该包拥有 Models 设置面板。保留 `ModelsSettingsStore`；它保存一个具有数据操作和订阅能力的设置视图模型，确实是存储。 |
| `@clocky/clocky-client-ui-plugin-config`、`client/ui-plugin-config/` | `@clocky/clocky-client-ui-settings-plugins`、`client/ui-settings-plugins/` | 该包拥有 Plugins 设置分区，而不是通用的插件配置系统。目标名称归入 `ui-settings-*` 系列，并采用该分区的复数产品名。 |
...
| `@clocky/clocky-client-ui-question` | `@clocky/clocky-client-ui-user-questions` | UI 呈现用户问题 seam，而不是任意问题领域。 |
| `@clocky/clocky-client-ui-command`, `ui-command/` | `@clocky/clocky-client-ui-commands`, `ui-commands/` | 该包呈现并运行一组命令。 |
...
- 保留 `@clocky/clocky-api-gateway`、`ctx.typertGateway` 和 `TypertGatewayService`。
- 保留会话投影名称。投影并不只是归约函数。
- 保留 `@clocky/clocky-session-stats`、`sessionStats` 和 `SessionStatsProjection`。这些名称准确表示全会话统计数据及承载它们的持续维护读模型。
...
- 保留 `clocky-subagent-clocky-sdk` 和提供方 id `clocky-sdk`；重复的限定词可避免歧义。
...
- 保留 `@clocky/clocky-client-ui-directory-picker-native`；其后缀说明它是在重命名后的 `-browse` 变体旁使用原生选择器的呈现。保留 `SURFACE_PACKAGES`；在目录选择器自动选择器中，它是客户端呈现端面的包映射，并与 `BACKEND_PACKAGES` 对照。
- 保留 `@clocky/clocky-host-plugin-inventory`、`ctx.pluginInventory`、`pluginInventory/list` Remote 以及 `PluginInventory*` 载荷类型。它们准确命名由 Host 拥有的只读清单；只有适配器类和作用域过宽的客户端呈现名称需要修改。
...
- 保留 `@clocky/clocky-message-feedback` 能力、`messageFeedback` Remote、assistant-action entry id `feedback`、hook key `feedback` 和 locale namespace `feedback`。它们所在的接口已经把作用域限定为消息反馈或本地 assistant-message slot。只修改作用域过宽的 Client 包名和导出的 UI 名称。
...
**将 `clocky-compact-basic` 重命名为 `clocky-compaction-llm`。**不予采纳。`LLM` 没有在当前后端系列中增加区别。`basic` 意图更克制，也不会声称存在一个实际并不存在的算法。
...
- 配对的包创建指南包含职责词约定，`packages/AGENTS.md` 链接到该约定，术语表记录选定用词和 `Typert` 拼写，根项目文案将产品称为 Clocky，而不是 Clocky SDK。
