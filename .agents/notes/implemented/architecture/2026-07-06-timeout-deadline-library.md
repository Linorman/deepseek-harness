...
| Concern | Owner |
|---|---|
| Validate request hint and clamp default/max | `clocky-timeout` (`clampTimeout`) — pure arithmetic plus the shared positive-finite request contract |
| Arm one-shot timer, abort on deadline, carry reason, fuse with upstream cancel | `clocky-timeout` (`deadline`) |
| Arm and rearm only around outstanding iterator demand, including out-of-band activity | `clocky-timeout` (`idleWatchdog`) |
| Clear the timer | `clocky-timeout` (`[Symbol.dispose]` on either primitive) |
| Classify the first abort reason after abort | `clocky-timeout` (`timeoutOf`) |
