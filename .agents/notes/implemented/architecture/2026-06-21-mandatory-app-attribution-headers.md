...
LLM provider requests should identify the product making them. That is useful for provider-side support, abuse investigation, compatibility debugging, and traffic analytics. The shared attribution helper in `clocky-llm` supplies the `User-Agent` used by the provider adapter ([packages/llm/llm-pi-ai/src/adapter.ts](../../../../packages/llm/llm-pi-ai/src/adapter.ts)); an adapter that omits the helper would otherwise lose attribution silently.
...
The provider-neutral identity is owned by `clocky-llm` (`packages/llm/llm/src/attribution.ts`), not by individual adapters. `AppIdentity` contains only public product facts needed to build `User-Agent`, and the default `APP_IDENTITY` values:

- product token for `User-Agent`: `agent-harness`
- version: read from the owning package's manifest via `createRequire`, never a hand-copied constant
- app URL: `https://example.invalid/agent-harness`
...
| DeepSeek provider endpoint | `User-Agent` for app attribution only. Do not send provider-specific request identity or OpenRouter-only headers under this decision. |
...
- `clocky-llm` documents the mandatory `User-Agent` attribution contract for `LlmAdapter` authors (`LlmAdapter` JSDoc, package README, and the adapter-contract section of `docs/subsystems/llm-streaming.md`).
...
- `clocky-llm-pi-ai` sends the same `User-Agent` through pi-ai's `StreamOptions.headers` hook and its mock-server suite asserts the exact value.
...
**SDK-named token.** Considered for the `User-Agent` token because the supported runtime client stack uses the SDK name. The product token won because it identifies the application without calling the complete product an SDK.
