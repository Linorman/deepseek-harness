...
A single `HarnessError extends Error` base in `clocky-llm` (the leaf package every other imports — no new dependency edge): a stable `code` distinct from `message`, `cause` chaining via `ErrorOptions`, and `name` defaulting to the subclass. `isHarnessError` narrows at seams.

- `LlmError` and `ToolArgsError` (clocky-tools) extend it, keeping their existing codes.
