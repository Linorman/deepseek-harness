...
The repository also used `SDK` in two meanings. The supported Python and TypeScript clients use the JSON-RPC SDK protocol. The project as a whole is Clocky, not an SDK project. The removed SDK project toolchain made the broad meaning obsolete, but prose and names preserved parts of it.
...
`SDK` means the JSON-RPC-based client/server protocol used by the supported Python and TypeScript SDKs. The repository keeps `@clocky/clocky-sdk-client`, `@clocky/clocky-sdk-protocol`, and the wire identity `clocky-sdk-runtime`; the JSON-RPC server belongs to the same family. Clocky itself is not an SDK, and the removed project generator, launcher, helper, and launcher telemetry packages stay absent.
...
Do not remove an intentional vendor qualifier to avoid repetition. `clocky-subagent-clocky-sdk` names the Clocky SDK provider and avoids confusion with another SDK. Its private class becomes `SdkSubagentProvider` because the class also needs to say what it provides.
...
|---|---|---|
| `@clocky/clocky-jsonrpc` | `@clocky/clocky-sdk-jsonrpc-server` | It is the server half of the SDK protocol. `jsonrpc` alone names an encoding; `sdk-jsonrpc-server` gives the family, mechanism, and role. |
| `HarnessSdkServer` | `HarnessSdkJsonRpcServer` | The class is one JSON-RPC server implementation, not every possible SDK server. |

Keep `@clocky/clocky-sdk-client`, `@clocky/clocky-sdk-protocol`, and `clocky-sdk-runtime`. Exclude `@clocky/create-sdk`, `@clocky/clocky-scripts`, `@clocky/clocky-helper`, and `@clocky/clocky-telemetry`; the separate removal decision deletes them and their support graph.
...
| `@clocky/clocky-bash`, `ctx.bash` | `@clocky/clocky-shell`, `ctx.shell` | PowerShell already implements this seam. The capability is shell execution, not Bash. |
...
| `@clocky/clocky-bash-env`, `ctx.bashEnv`, `BashEnvRegistry` | `@clocky/clocky-shell-env`, `ctx.shellEnv`, `ShellEnvRegistry` | The environment registry is shared by Bash and PowerShell tools. |
...
| `@clocky/clocky-pty`, `ctx.pty`, `PtyService` | `@clocky/clocky-terminal`, `ctx.terminals`, `TerminalSessionService` | Callers manage multiple named terminal sessions. They do not allocate raw PTYs through this service. |
...
| `@clocky/clocky-pty-local`, `LocalPtyBackend` | `@clocky/clocky-terminal-bash`, `BashTerminalBackend` | The provider depends on Bash prompt and shell behavior. `local` hides the actual dialect. |
| `@clocky/clocky-tool-pty` | `@clocky/clocky-tool-terminal` | The model-facing tools are already `terminal_*`; the package should use the same product noun. |
...
| Former | Current | Reason |
|---|---|---|
| `@clocky/clocky-lsp-local` | `@clocky/clocky-lsp-stdio` | The provider speaks LSP over stdio through replaceable filesystem and subprocess services. It is not necessarily local. |
...
| `@clocky/clocky-tasks`, `ctx.tasks`, `TaskService` | `@clocky/clocky-jobs`, `ctx.jobs`, `JobRegistry` | The service registers, owns, observes, waits for, and cancels multiple background jobs. It is a registry, not a general task service. |
...
| `@clocky/clocky-tasks-local`, `LocalTaskService` | `@clocky/clocky-jobs-local`, `LocalJobRegistry` | This is the process-local provider of the job registry. Here `local` is meaningful because the jobs and callbacks live in one process. |
| `@clocky/clocky-tool-tasks` | `@clocky/clocky-tool-jobs` | The consumer controls the job registry and should use the same domain noun. |
...
| `@clocky/clocky-client-ui-task`, `client/ui-task/` | `@clocky/clocky-client-ui-jobs`, `client/ui-jobs/` | The client package presents the background-job collection. It is not one user task. |
...
| Former | Current | Reason |
|---|---|---|
| `@clocky/clocky-client-ui-slash`, `ui-slash/` | `@clocky/clocky-client-ui-input-trigger`, `ui-input-trigger/` | The client handles `/`, `@`, keyboard arbitration, candidate menus, and programmatic launch. It is not only slash commands. |
...
| `@clocky/clocky-agent-tool-mode`, plugin `tool-mode` | `@clocky/clocky-agent-tool-presentation`, plugin `tool-presentation` | The plugin changes how tools are presented to the model. It does not change execution behavior. Keep local `Config.mode` and `ToolPresentationMode`. |
...
| `@clocky/clocky-permission`, `ctx.permission`, `PermissionService` | `@clocky/clocky-permission-presets`, `ctx.permissionPresets`, `PermissionPresetService` | The service selects and persists presets. Sandbox and approval services enforce the result. |
| `@clocky/clocky-client-ui-permission` | `@clocky/clocky-client-ui-permission-presets` | The UI edits and selects permission presets. |
...
| `@clocky/clocky-user-interaction`, `user-interaction/` | `@clocky/clocky-user-questions`, `user-questions/` | The seam supports question batches and answers only. Approval, commands, and directory picking are separate interaction seams. |
| `ctx.userInteraction`, `UserInteractionService`, `UserInteractionProvider`, `UserInteractionError` | `ctx.userQuestions`, `UserQuestionService`, `UserQuestionProvider`, `UserQuestionError` | These names state the one supported interaction form. Keep `AskUserQuestion*`, the `ask_user_question` tool, and `@clocky/clocky-tool-ask-user`. |
...
| Former | Current | Reason |
|---|---|---|
| `packages/typert/type-meta/`, `@clocky/clocky-type-meta` | `typert/protocol/`, `@clocky/clocky-typert-protocol` | The package owns the Typert Remote protocol, decorators, bindings, codecs, lookups, and context contracts. It is not generic type metadata. |
...
Keep `@clocky/clocky-tools` and `ctx.tools`. Keep `@clocky/clocky-api-gateway`, its `gateway/` folder, `ctx.typertGateway`, and `TypertGatewayService`; that service is a real API gateway. Its internal `TypeRT*` identifiers still follow the `Typert*` spelling rule.

### Workspace instructions, telemetry, identity, and launch environment

| Former | Current | Reason |
|---|---|---|
| Host `ctx.workspace` | Host `ctx.workspaceRegistry` | `WorkspaceRegistry` owns multiple workspaces, but Client `ctx.workspaces` already has an incompatible type. Both declarations merge into the same Cordis `Context` interface at compile time even though their runtime contexts are separate. The role suffix states the host service and avoids that collision. Keep `@clocky/clocky-workspace`, `WorkspaceRegistry`, `Workspace`, and `workspace.*` wire names. |
| `@clocky/clocky-workspace-context`, `context/workspace-context/` | `@clocky/clocky-agent-instructions`, `context/agent-instructions/` | The package loads hierarchical `AGENTS.md` and `CLAUDE.md` files for the agent. It is not general workspace context. |
...
| `TelemetryOtel`, `TelemetryMode`, plugin `telemetry-otel` | `OpenTelemetrySessionBackend`, `SessionTelemetryMode`, plugin `session-telemetry-otel` | The provider name states both the OpenTelemetry mechanism and session scope. Keep the package names `clocky-session-telemetry` and `clocky-session-telemetry-otel`. |
...
| `session/user-id/`, `@clocky/clocky-user-id` | Removed; no replacement package | The random cross-service correlation id has no current consumer. It was neither a Session concern nor an authenticated user identity. |
| `USER_ID_FILE_NAME`, `.userid`, feedback label `User` | Removed with the correlation id | The file and UI no longer expose an anonymous identity or a repository-created OTel `user.id`. |
| `util/environment/`, `@clocky/clocky-environment` | `util/launch-environment/`, `@clocky/clocky-launch-environment` | The package captures one immutable layered snapshot at launch. It is not a general environment API. |
| Public `Environment*`, `createEnvironmentSnapshot`, `environmentOf`, `CLOCKY_ENVIRONMENT_KEY` | `LaunchEnvironment*`, `createLaunchEnvironmentSnapshot`, `launchEnvironmentOf`, `CLOCKY_LAUNCH_ENVIRONMENT_KEY` | The names state the snapshot's lifetime and purpose. |
...
| `@clocky/clocky-compact-basic`, `BasicCompactService`, public `BasicCompact*` | `@clocky/clocky-compaction-basic`, `BasicCompactionEngine`, corresponding `BasicCompaction*` | `basic` is plain but honest. `compaction-llm` adds no information because LLM use is already part of the current implementation family. |
| `@clocky/clocky-compact-tool-result-prune`, `ToolResultPruneService`, `ctx.toolResultPrune` | `@clocky/clocky-compaction-tool-result-pruner`, `ToolResultPruner`, `ctx.toolResultPruner` | The plugin is an actor that prunes tool results. The noun `pruner` names that role. |
...
| `@clocky/clocky-settings-local`, `SettingsLocal` | `@clocky/clocky-settings-file`, `FileSettingsProvider` | The implementation is file-backed through the filesystem seam. `file` states the mechanism; `local` does not. |
...
| Former | Current | Reason |
|---|---|---|
| `HttpServerService`, `ctx.httpServer` | `WebServer`, `ctx.webServer` | The server owns HTTP routes and WebSocket upgrade routes. `Web` leaves room for both; `Http` is too narrow here. Keep `packages/host/webserver`, `@clocky/clocky-host-webserver`, `WebRoute`, and `WebUpgradeRoute`. |
...
| `@clocky/clocky-session-title-first-message-llm`, cadence `first-message` | `@clocky/clocky-session-title-first-prompt-llm`, cadence `first-prompt` | The trigger is the first user prompt, not any message in the session log. |
| `@clocky/clocky-session-title-all-messages-llm`, cadence `all-user-messages` | `@clocky/clocky-session-title-all-prompts-llm`, cadence `all-prompts` | The backend refreshes from user prompts. `all messages` wrongly includes assistant and tool events. |
| `@clocky/clocky-code-runtime-worker`, `WorkerCodeRuntime` | `@clocky/clocky-code-runtime-worker-thread`, `WorkerThreadCodeRuntime` | The implementation uses a Node worker thread. `worker` alone is too broad. |
...
| Former | Current | Reason |
|---|---|---|
| `@clocky/clocky-fs-policy` | `@clocky/clocky-fs-observation-policy` | The package defines which filesystem observations authorize later effects. It is not the complete filesystem or sandbox policy. |
...
| `InvariantService` | `InvariantRegistry` | The object owns registered invariant checks. Keep `@clocky/clocky-invariants` and `ctx.invariants`. |
...
| Former | Current | Reason |
|---|---|---|
| `util/paths/`, `@clocky/clocky-paths` | `util/home-paths/`, `@clocky/clocky-home-paths` | The helpers resolve paths under the Harness home. They are not a general path library. Keep the individual function names when they already state the returned path. |
| `util/retention/`, `@clocky/clocky-retention` | `util/output-retention/`, `@clocky/clocky-output-retention` | The policy retains command and tool output. It is not a general data-retention framework. |
| `E2BSandboxService` | `E2BRuntime` | The class creates, reuses, and disposes the E2B execution environment used by filesystem and subprocess adapters. It is broader than one sandbox handle and narrower than a generic owner. Keep `@clocky/clocky-e2b`, `ctx.e2b`, and the `e2b/` group. |
| `@clocky/clocky-frontend-static` | `@clocky/clocky-host-frontend-static` | The package is the Host plugin that serves the frontend assets. The prefix distinguishes it from frontend application code. |
...
| `@clocky/clocky-jsonrpc-demo` | `@clocky/clocky-sdk-jsonrpc-demo` | The example demonstrates the runtime SDK over JSON-RPC. It belongs to the one SDK meaning. |
| `@clocky/clocky-frontend` | `@clocky/clocky-web-frontend` | The application is the web frontend. Keep its physical `apps/web/` folder. |

Keep atomic-write, brand, native-command, timeout utility, directory-picker, `clocky-base`, `clocky-web-app`, app boot, CLI names, and the `headless` package, bundle, and example identity. `headless` is the intended product essence and may later support more than one-shot execution.
...
| `@clocky/clocky-client-ui-model` | `@clocky/clocky-client-ui-model-selection` | The package controls the model selection for a session. The singular `model` name is too broad. |
...
| `@clocky/clocky-client-ui-models` | `@clocky/clocky-client-ui-settings-models` | This package owns the Models settings panel. Keep `ModelsSettingsStore`; it holds one settings view model with data operations and subscriptions and is a real store. |
| `@clocky/clocky-client-ui-plugin-config`, `client/ui-plugin-config/` | `@clocky/clocky-client-ui-settings-plugins`, `client/ui-settings-plugins/` | This package owns the Plugins settings section, not a general plugin-configuration system. The target joins the `ui-settings-*` family and uses the section's plural product name. |
...
| `@clocky/clocky-client-ui-question` | `@clocky/clocky-client-ui-user-questions` | The UI presents the user-question seam, not an arbitrary question domain. |
| `@clocky/clocky-client-ui-command`, `ui-command/` | `@clocky/clocky-client-ui-commands`, `ui-commands/` | The package presents and runs a collection of commands. |
| `@clocky/clocky-client-ui-directory-picker`, `client/ui-directory-picker/`, Loader id `ui-directory-picker`, `client-ui-directory-picker-invariant` | `@clocky/clocky-client-ui-directory-picker-browse`, `client/ui-directory-picker-browse/`, Loader id `ui-directory-picker-browse`, `client-ui-directory-picker-browse-invariant` | The Client packages now contain separate `browse` and `native` directory-picker presentations. The unqualified package is the browse implementation, not their shared definition. The target matches the Host backend family and changes no boundary. |
...
- Keep `@clocky/clocky-api-gateway`, `ctx.typertGateway`, and `TypertGatewayService`.
- Keep session projection names. A projection is not only a reducer function.
- Keep `@clocky/clocky-session-stats`, `sessionStats`, and `SessionStatsProjection`. They accurately name whole-session statistics and the maintained read model that carries them.
...
- Keep `clocky-subagent-clocky-sdk` and provider id `clocky-sdk`; the repeated qualifier prevents ambiguity.
...
- Keep `@clocky/clocky-client-ui-directory-picker-native`; its suffix names the native-chooser presentation beside the renamed `-browse` variant. Keep `SURFACE_PACKAGES`; within the directory-picker auto selector it is the package map for the Client presentation half, contrasted with `BACKEND_PACKAGES`.
- Keep `@clocky/clocky-host-plugin-inventory`, `ctx.pluginInventory`, the `pluginInventory/list` Remote, and the `PluginInventory*` payload types. They accurately name the Host-owned read-only inventory; only the adapter class and the overly broad Client presentation names change.
...
**Rename `clocky-compact-basic` to `clocky-compaction-llm`.** Rejected. `LLM` adds no distinction in the current backend family. `basic` is less ambitious and does not claim an algorithm that does not exist.
...
- The paired package-creation guide contains the role-word contract, `packages/AGENTS.md` links to it, the terminology table records the chosen words and `Typert` spelling, and root project prose calls the product Clocky rather than Clocky SDK.
