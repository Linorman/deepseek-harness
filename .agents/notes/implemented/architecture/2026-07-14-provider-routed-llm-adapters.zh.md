...
`clocky-llm` 按精确模型名称注册适配器。插件在 Cordis 启动时提供模型列表，`LlmRuntime` 为列表中的每个字符串保存一个适配器，`GenerateOptions.model` 同时选择适配器与提供方模型。两个随附的适配器都只面向相同的两个 DeepSeek 模型时，这种方式可以工作，但它混淆了两个独立决策：由哪个上游提供方承接请求，以及该提供方应运行哪个模型。
...
pi-ai 的通用流选项不支持停止序列。若 Harness `stop` 选项已定义，`clocky-llm-pi-ai` 会以 `UNSUPPORTED_OPTION` 拒绝请求，不会静默忽略，也不会增加第二套提供方专用 payload 实现。
...
JSON-RPC 运行时显式接收提供方与模型，并要求请求的提供方已由插件组合注册适配器。缺少提供方会直接失败，不会猜测实现。
...
**增加 `backend + provider + model`。** backend 键可以让同一提供方的多个实现共存，并按请求切换。最终采用的部署规则是一个提供方对应一个适配器所有者：同一上游的不同实现属于由插件组合选定的替代项。第三个路由维度会增加每个请求与配置的负担，却没有当前消费方。

**让 `clocky-llm-pi-ai` 自动注册所有 pi-ai 提供方。** 这种方式会占用部署无意暴露的环境凭据和提供方名称，并可能与其他适配器冲突。显式配置可以审查能力和凭据范围。
...
- 模型选择不再改变 Cordis 插件图。目录型路由可以接受启动后选择的任意已安装目录模型，而声明式路由则解析其 profile 中列出的模型。
...
- pi-ai 的通用流 API 无法表达停止序列，因此 `clocky-llm-pi-ai` 会拒绝停止序列。
...
- 无密钥的 agent loop/会话测试和 ACP 快照覆盖持久化提供方/模型元数据、恢复与 fork 传播、工作流/subagent 覆盖，以及不变的用户可见 transcript（文本记录）；在凭据可用时，提供方 e2e 测试保留真实流式输出与工具后续调用覆盖率。
