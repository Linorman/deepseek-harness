...
- Persistence stays a plugin concern; the in-memory store ships in clocky-session.
...
- Derivation cost grows with log length — compaction (clocky-compaction) is the intended mitigation, not log mutation.
