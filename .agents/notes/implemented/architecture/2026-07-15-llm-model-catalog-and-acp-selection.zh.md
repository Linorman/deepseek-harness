...
模型发现不能变成请求校验。声明式路由可以把其配置端点实际提供的模型 ID 转发出去，而继承 pi-ai 目录的路由则以已安装目录作为权威依据。将共享目录视为白名单，会破坏提供方路由需要保留的私有端点能力。
...
`clocky-llm-pi-ai` 将每个已配置提供方的 `getModels(provider)` 返回的已安装条目映射为提供方无关的目录。对继承目录的路由，请求时目录查询仍是权威依据，未知模型仍以 `UNKNOWN_MODEL` 失败；声明式路由则公布其 profile 中列出的模型。条目改善已知目录模型或已配置模型的选择体验，而请求校验仍由选中的路由负责。
...
`installModelSelection`（位于 `clocky-agent`）为前端拥有的选择安装 agent 作用域的 `system-prompt/assemble` 与 `agent/request` 监听器。提示词组装在每个步骤对所选组合做一次快照，在下游提示词监听器之后覆写组装出的 `provider` 与 `model` 变量；请求监听器在下游请求监听器之后应用同一快照。因此，发生在异步组装期间的选择会从下一个步骤生效，而不会让提示词文本与路由分裂。其他调用配置字段保持不变。
...
**将目录设为强制白名单。** 这与声明式路由的模型列表和私有部署冲突。请求的权威校验本就属于被选中的适配器。
...
- pi-ai 适配器会暴露其已安装的提供方目录；声明式 profile 会列出已配置选项，并保留其路由的请求校验。
...
单元测试覆盖目录值副本与格式错误的元数据、pi-ai 目录投影、提供方／模型请求路由，以及提示词变量对齐；监听器安装在 agent 作用域的上下文中，因此能够实现 agent 间隔离。ACP 传输测试独立验证固定提供方／模型的转发行为；TUI 套件覆盖选择器交互与基于请求头的恢复。
