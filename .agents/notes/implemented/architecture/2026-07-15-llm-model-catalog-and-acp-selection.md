...
Model discovery cannot become request validation. A declared route may forward model ids that its configured endpoint serves, while pi-ai's installed catalog is authoritative for routes that inherit it. Treating one shared catalog as a whitelist would remove the private-endpoint behavior that provider routing was designed to preserve.
...
`clocky-llm-pi-ai` maps each configured provider's installed `getModels(provider)` entries into the neutral catalog. Its request-time catalog lookup remains authoritative for catalog-backed routes and rejects unknown models with `UNKNOWN_MODEL`; a declared route instead advertises the models stated in its profile. The entries improve selector UX for known catalog or configured models, while request validation remains owned by the selected route.
...
**Make catalogs mandatory whitelists.** This conflicts with declared-route model lists and private deployments. The selected adapter already owns authoritative request validation.
...
- pi-ai adapters expose their installed provider catalogs; declared profiles list configured choices and retain their route's request validation.
