...
Local delivery may begin only after a durable activation binding exists. An offline epoch can cold-resume the same Session under a new ActivationId without losing prior auditability. The controller owns accepted handles and releases them during shutdown; provider unload still does not revoke handles already returned to the controller.
