...
- Each materialized Session has exactly one `SessionInputShell` (the facade), created and torn down with the session scope; the local Team task draft owns its own input shell before a coordinator Session exists. `ConversationRoot` is itself the `session-maybe` resident shell, holding HeroShell, the composer stack, and the chain-fallback frame. It always owns the same scrollport and composer seat; separate strict-session header and body outlets fill those fixed regions after a coordinator Session appears.
- The composer bar is one `session-maybe` slot entry rendered unconditionally: with neither a Session nor a Team draft the same InputBar renders inert (machine faces absent, `disabled` owner prop); a Team draft gives that instance its local machine, and idempotent `teams.start` refreshes and opens the resulting coordinator Session. The textarea DOM survives the no-session → Team-draft → coordinator transition and every later phase flip; `ConversationRoot`, the Hero, and the layout skeleton hold throughout.
...
- Sending unifies in the hub defaultSink: after an optimistic draft clear, a Team-draft submit calls idempotent `teams.start`, refreshes, and opens its coordinator Session; a session-scoped submit goes only through `session.prompt` with `mode:'queue'` (the Web UI has no steer entry; host-wire `mode:'steer'` remains outside this machine). Backfill happens only when it fails and the live draft is still empty — a user who has kept typing is never overwritten. No Draft materialize or attach transaction exists.
- The Team draft remains local until `teams.start` succeeds; success refreshes the retained session list and opens the returned coordinator Session, while failure retains the same draft and error context for retry.
...
- One resident conversation shell carries no-session/Team-draft/active: no session → Team draft preserves ConversationRoot, Hero, scrollport, composer seat, InputBar, and textarea; only the strict header and body outlets gain content after the coordinator Session opens. The Team-draft → coordinator transition and later phase changes also keep the InputBar and textarea. EmptyState and the controlled intent chain (`sessions.updateIntent`/`updatePendingPrompt`/`workspaces.sendSession`) are deleted along with their last consumer.
