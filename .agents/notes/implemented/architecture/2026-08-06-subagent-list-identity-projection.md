...
It hangs beside the existing `subagentTiming` ([projection.ts](../../../../packages/compat/subagent/src/projection.ts), [projection-types.ts](../../../../packages/compat/subagent/src/projection-types.ts)), under key `subagent`:
...
`listChildren`'s ([list-children.ts](../../../../packages/compat/subagent/src/list-children.ts)) enumeration goes through no query service: the two sources `ctx.sessions.list()` and `ctx.get('sessionPersistence')?.list()` merge by id, with a live record overriding the same-id persisted record wholesale and no header consistency check. Everything enumeration needs is header facts:
...
For each enumerated child, mode/label retrieval walks a three-rung ladder — compute-and-discard, no cache of its own, no write-back:
...
- An unknown parent: the old implementation threw not-found through session-query ('parent session … was not found'); the subagent-owned merge now yields an empty subset for a nonexistent parent, so enumeration returns an empty list — a silent change of semantics and wording, recorded as explicitly accepted.
...
Consuming surfaces: the optional `list_agents` tool keeps its model schema and description while narrowing its load requirement by dropping `sessionQuery`. `SessionHeader.origin` continues to fence generic ownership, so pre-#1569 data without it is not treated as subagent-owned. Shipped Host and client surfaces expose neither a `subagents` domain nor a direct-child history projection.
...
| subagent | list-children.ts and its types | Rewritten as subagent-owned enumeration plus the projection-ladder four-state mapping; the session-query dependency, per-child event reads, and in-place classification machinery deleted; error code `SUBAGENT_CONTROL_SESSION_QUERY_UNAVAILABLE` replaced by `SUBAGENT_CONTROL_PROJECTIONS_UNAVAILABLE`; new optional dependency clocky-session-projection-cache (pure read acceleration, skipped when absent) |
| host/apiproxy | api-proxy.ts | `hasSubagentDescriptor` deleted; generic ownership checks look only at `header.origin`; the public subagents domain is absent |
...
| wire/client | — | The public subagents domain and direct-child GUI projection are absent |
