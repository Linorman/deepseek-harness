...
The former subagent and model-authored workflow families are now private compatibility packages with explicit example consumers. The Clocky release family and Python runtime carrier exclude them; the product tool catalog skips their private tool manifests; and the legacy cutover gate checks default configurations, carrier dependencies, catalog entries, public release members, and packed tarballs. The current release pack contains 249 public Clocky tarballs and no compatibility tarball, while AgentRuntime/ACP/SDK packages remain publishable and unchanged.
