# Agent Note: Omit runtime invariants from shipped clocky config
...
The built CLI config-dump test checks both shipped surfaces and rejects either the service entry or any `@clocky/clocky-*/invariant` entry.
...
- Ordinary `clocky` TUI and Web runs install no invariant listeners or trace state and cannot fail through `InvariantError`.
