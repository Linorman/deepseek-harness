...
- Boundaries: in-process one-shot, continuable, and workflow-spawned children are enforced through the shared helpers; `subagent-acp` and `clocky-sdk` children run in external processes under their own provider composition.
...
- `packages/subagent/subagent-in-process-driver/tests/inheritance.spec.ts` and `packages/subagent/subagent/tests/continuation-inheritance.spec.ts` pin the delegated `approval/policy` event for one-shot and continuable children.
