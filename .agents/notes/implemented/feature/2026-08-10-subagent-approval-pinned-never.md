...
- `packages/compat/subagent-in-process-driver/tests/inheritance.spec.ts` and `packages/compat/subagent/tests/continuation-inheritance.spec.ts` pin the delegated `approval/policy` event for one-shot and continuable children.
