...
No current Team-product snapshot covers a forced durability failure, a child with several follow-up turns, or report-before-settlement ordering.
...
- `foldConsumedWork()` replaces `clocky-session`'s `findLastMessageTurnEnd()` and moves to `clocky-agent`, which owns the inbox marker it reads; the one-shot in-process path folds the same answer and does not classify a cut-short one-shot child as `completed`.
...
- A keyless headless Loader snapshot pins default background start → manager-authored settlement notice → final parent answer without polling or child `report`; no current Team-product snapshot covers default report-before-settlement ordering.
