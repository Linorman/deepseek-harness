...
The session header is the place where per-session background activity lives: [`clocky-client-ui-jobs`](../../../../packages/client/ui-jobs/README.md) contributes the background-job action to `conversation.session.header.actions`. What was missing was any channel at all that carried task state to a browser.
