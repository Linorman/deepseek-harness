...
- A Windows composition using `clocky-tool-pwsh` now shows its shell calls exactly as bash calls look in the Web UI: cwd-headed terminal card, raw output, exit-status pill, run-state dot, and the red failure treatment on non-zero exits.
- `parseExitStatus` becomes public contract surface on `@clocky/clocky-shell`; `clocky-tool-bash/src/render.ts` keeps re-exporting it, so no bash-tool consumer changes.
