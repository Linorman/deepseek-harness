...
| 已接受的用户图片 | `CLOCKY_HOME` 下的不可变对象加 `ImageAttachmentRef` | 在 `agent.send()` 或 `agent.steer()` 能够追加所属用户事件前，宿主提交每张图片。 |
| 结构化模型图片输出 | `CLOCKY_HOME` 下的不可变对象加 `ImageAttachmentRef` | 提供方适配器在发出已完成的图片块或助手消息事件前提交字节。事件中禁止出现临时 URL、路径和 base64。 |
...
本地附件后端解析显式的 `clockyHome`，再尝试 `$CLOCKY_HOME`，最后使用 `~/.clocky`。它把内容寻址对象存储在 `$CLOCKY_HOME/attachments/v1/objects/<prefix>/<sha256>` 下，目录和文件均使用仅属主权限。每个进程在某个 home 上首次保存时，会创建该 home，并把每个祖先项同步到文件系统根；不能把「存在」视为持久化，因为另一个进程可能仍处于 `mkdir` 与父目录 `fsync` 之间。随后写入临时文件、同步、原子发布，并通过发布路径上的目录同步使其持久化（POSIX；Windows 依赖文件系统元数据日志）后，服务才返回引用。内容摘要编码在不透明的 `sha256:<digest>` 标识中。准入过程通过应用方向、删除元数据、转换为 8-bit sRGB/sRGBA，并在独立的尺寸与字节限制下保持宽高比来准备与提供方无关的主版本。读取时校验摘要、字节长度和已记录元数据。路由特定的确定性请求版本单独缓存；完整策略记录在[统一图片主版本与请求版本](2026-08-20-unified-image-request-pipeline.zh.md)中。
...
`clocky-llm-pi-ai` 适配器在请求时解析 `ctx.attachments`，递归转换每个保留的图片引用，包括嵌套在工具结果中的引用，并且仅为声明支持图片输入的模型生成提供方原生图片内容。它从每个持久规范化附件派生相同的确定性路由请求版本，在考虑 base64 扩张的请求预算内内联携带，并把被省略的图片替换为确定性的文本占位符。纯文本模型和未声明图片能力的模型保持纯文本。在请求时解析服务，可避免 Cordis 加载顺序将可选附件服务的可用性固化。适配器不得展平或静默跳过保留图片；不支持的角色与模型会以类型化的 `UNSUPPORTED_CONTENT` 失败。
...
已实现能力包括只准备一次的共享批量准入、与提供方无关的主版本、确定性请求版本、稳定裁剪句柄、角色无关图片块、pi-ai 输入转换、Web／ACP／MCP 持久化顺序、Web 上传与读取协议、条件式 ACP 图片支持、带持久图片投影的无损 MCP 结果、Code Mode 丰富结果转发、有界 Web 请求、草稿与历史图片 UI、压缩处理，以及组装后的无密钥覆盖。
...
临时存储适合在发送前使用，也适合通过操作系统接收剪贴板文件的原生客户端。但它不适合在消息被接受后继续使用：清理不在 harness 的控制范围内，路径因宿主而异，恢复或 fork 后的会话也可能比文件存在得更久。提案允许临时暂存，但会在追加事件前将已接受的字节复制进 `CLOCKY_HOME`。
...
- 客户端单元测试覆盖粘贴与拖放、混合剪贴板文本、仅图片发送、草稿恢复、顺序、草稿、会话作用域和应用层级的对象 URL 清理，以及一项在释放后才完成的延迟历史读取；keyless 的组装后构建产物通道（`apps/web/tests/image-display.snapshot.ts`，`CLOCKY_EXAMPLE_MODE=lib pnpm run test:snapshot`）覆盖经授权附件路由渲染的历史用户与助手图片画廊、原图 lightbox，以及 composer 粘贴缩略图条。
- 适配器与压缩测试覆盖确定性 pi-ai 请求版本、纯文本投影、递归嵌套在工具结果中的图片、共享摘要请求版本，以及明确拒绝图片输出。
- 适配器与压缩测试覆盖确定性 pi-ai 请求版本、纯文本投影、递归嵌套在工具结果中的图片、共享摘要请求版本，以及明确拒绝图片输出。
- 需要凭据的实际 API 测试会覆盖配置的提供方路由和内联图片转换；生产适配器不包含自定义文件上传路径。
- 需要凭据的实际 API 测试会覆盖配置的提供方路由和内联图片转换；生产适配器不包含自定义文件上传路径。
