...
- `clocky-subagent` gains optional peer types on `clocky-sandbox-policy` and `clocky-user-approval` (the `ctx.get` pattern the one-shot driver used); `clocky-subagent-in-process-driver` drops its policy-service peers and type imports entirely and delegates to the shared helpers.
- The continuable suite (`packages/compat/subagent/tests/continuation-inheritance.spec.ts`) pins fresh-start seeding, pre-await capture, default omission, cold-resume snapshot stability, and fork-seed precedence; the keyless headless `subagent-inheritance` Loader snapshot pins the child's delegated sandbox event and read-only runtime context through the assembled app.
- Out-of-process providers (`acp`, `clocky-sdk`, `claude-code`, `codex`) support no continuable children (`prepareContinuable` absent), and their one-shot children keep their own deployment policy (`inheritsParentContext = false`); cross-process policy propagation remains out of scope.
