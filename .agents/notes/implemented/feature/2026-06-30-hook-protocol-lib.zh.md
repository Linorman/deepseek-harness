# Agent Note: clocky-hook-protocol——Claude Code / Codex 钩子协议格式共享核心库
...
钩子子系统需要一个供运行外部 shell hook 的桥接实现使用的共享库。参考实现（`~/repos/refs/claude-code`、`~/repos/refs/codex`）表明一个决定性事实：**Codex 有意重新实现了 CC 钩子协议的一个子集。** 它的引擎读取相同的 `hooks.json`，使用相同的 matcher-group 形状、相同的 exit-code/structured-stdout 输出约定，以及相同的命令钩子执行模型。Codex 的源码甚至以 Claude 的引擎命名，并在注释中标注了「有意偏离」之处。共享库将这些协议原语集中在一个位置。

本 Agent Note 引入 `@clocky/clocky-hook-protocol`，一个**库**（不是插件——它不注册也不注入任何东西），持有与方言无关的协议原语。共享与方言专属行为之间的分界是本设计的重心。

## 决策

`packages/hooks/` 分组包含作为纯库的 `hook-protocol`。它负责四类原语和 `hook/*` 会话事件；桥接实现负责真正不同的部分。
...
**方言专属（桥接实现）：** 构建每个事件的 stdin payload、方言环境变量，并将方言无关的 `HookOutput`/`MergedHookOutcome` 映射为 harness 各扩展点专属的类型化 Decision（`PreToolDecision`、`PreStepDecision`、`ContinuationDecision`、`PostToolDecision`）。
...
桥接实现以原子方式解析配置、构建方言 payload、调用共享的 runner 与合并逻辑、映射 decision、追加 `hook/*`。协议测试覆盖每种 matcher 模式与诊断、exit-code 与编解码器字段、runner 接线、合并优先级和审计辅助函数，逐文件 100% 覆盖率；`updatedInput` 已解析但仅记录日志并发出警告，直到 [input-rewrite 提案](../../proposed/feature/2026-06-30-pre-tool-input-rewrite.zh.md)落地。
