...
把含 emoji、中日韩文字或全角标点的 Key 粘进 Web 模型设置页，保存会报成功。首个轮次随即失败，报错为 `Cannot convert argument to a ByteString because the character at index 7 has a value of 55357 which is greater than 255`——其中的下标与码点是 UTF-16 内部细节，不附带任何可执行动作，却泄露了 Key 中某一个字符的码点。提供方请求路径之所以产出这句，是因为 `fetch` 在 [discovery.ts](../../../../packages/llm/llm-pi-ai/src/discovery.ts) 的 `try` 内部构造 `Bearer` header，而那个 `catch` 把一切失败都标为 `TRANSPORT`；该标签又在 `DEFAULT_RETRYABLE_CODES` 之中，于是一个永久且确定的故障还会被重试三次。
...
空白字符能通过每一道检查。`ProviderEditor` 判的是 `keyDraft.length`，于是三个空格构成的 Key 会被存下，随后以 `Bearer` 加若干空格去认证。两个提供方请求路径都不检查来自凭据或环境的 Key——而那正是 Models 页写入的路径，也就是用户真正走的路径。
...
**解析得到的值只含空白。** 每个提供方请求路径都将其视为非法，因为它无法为请求鉴权。在浏览器中，这同样是字段级失败：字段是人刚刚敲过字的地方，静默丢弃他敲进去的内容永远不是正确答案。

因此 `normalizeApiKey` 接受 `string`，而绝非 `string | undefined`。

### 规则住在哪里

`normalizeApiKey` 是 `clocky-llm` Service Definition 的一个模块，与已经承担共享 header 事务的 [attribution.ts](../../../../packages/llm/llm/src/attribution.ts) 并列。pi-ai 的请求与探测路径都依赖该 seam 且都需要这条规则，因此它拥有两个当前消费方而非一个预设消费方。它返回 trim 后的值，或一个原因（`empty`、`illegalCharacters`）。

pi-ai 的请求与探测路径同样都需要那句完全相同的「拒绝一个已存储凭据」的诊断。`LlmError` 声明在 Service Definition 的 `index.ts` 中，因此 `assertUsableApiKey(raw, pkg, ref)` 就住在它旁边，两个路径都不再各留一份。断言模块本身保持零依赖：把 `LlmError` 引入 `api-key.ts` 会与 `index.ts` 对它的再导出成环。

客户端无法引入其中任何一个：client 包只 reference client 包，因此 `packages/client/ui-settings-models` 在自己的 `apiKey.ts` 中镜像这个断言并持有本地化文案，正如 `validateModels` 镜像 host 侧的 catalog model schema。两侧在注释中互相指名。

### 各处分别做什么

| 位置 | 行为 |
|---|---|
| `clocky-llm` | 拥有 `normalizeApiKey`、`assertUsableApiKey` 与 `INVALID_CREDENTIAL_CODE`，后者刻意不进 `DEFAULT_RETRYABLE_CODES`。 |
...
`ProviderEditor` 服务 pi-ai 布局与通用回退，`CustomProviderCard` 为手工声明的路由承载同一套判定。
...
**在 pi-ai 的请求与探测模块中各留一个抛错 helper。** 最初的计划正是各留一份，差别仅在消息中的包名前缀，并配一个重复检测豁免来放行这一对。在实现之前即被否决：`LlmError` 声明在 Service Definition 中，因此该包完全可以自己拥有这句诊断，而那里的一个豁免恰恰会掩盖它本要遮掩的重复。

**在请求 `catch` 中嗅探 `TypeError`。** 这只是事后归类 ByteString 失败，header 构造本身仍无防护。它依赖 Node 错误消息的措辞，因而会随运行时版本静默失效；它也帮不到 pi-ai SDK 请求路径。在交出 Key 之前就拒绝，则对请求与探测路径同时有效。
...
`packages/llm/llm-pi-ai/tests/` 经真实凭据 seam（而非 stub）覆盖已存储凭据路径与探测路径，包括不带 Key 的探测不会发出 `authorization` 标头。
