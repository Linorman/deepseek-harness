...
**Change retry defaults with the server** — rejected because the server reveals existing semantics rather than deciding policy. The provider adapter classifies truncated streams, while the retry plugin continues to apply the configured failure-code policy.
