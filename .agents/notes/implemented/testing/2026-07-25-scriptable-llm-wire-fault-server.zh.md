...
`@clocky/clocky-llm-mock-server` 是一个私有支持包，提供可导入的 Node HTTP 服务器。仓库内的 `pnpm run mock:llm` 源码入口提供一个用于手动故障注入的独立进程；该包不公开可安装的二进制命令。它接受兼容 OpenAI 的根路径和 `/v1` chat-completions 路径，校验可选的 bearer token，捕获请求，并对每个已接受请求消耗一个显式行为。脚本耗尽时会明确报错；只有设置 `repeatLast` 才会重复最后一个行为。
...
服务器只报告协议层事实，不判断是否可重试。真实组合测试让请求依次经过 `clocky-llm-pi-ai`、`clocky-agent-loop` 和 `clocky-llm-retry`：连接遭拒、硬断开、部分输出后重置、空闲超时、正常截断的流以及合法的无内容完成，都会由适配器分类，并在现有默认策略下恢复。该包不会改变这些策略。

## 验证

包测试覆盖所有请求行为、跨分片 UTF-8 请求解码、不消耗脚本的 HTTP 校验、脚本耗尽与重复、停滞连接清理、CLI 解析及延迟边界、IPv6 base URL、随机 seed 可复现性、权重校验、单结果遥测、生命周期清理，以及逐文件覆盖率门禁下的配套不变式插件。重试集成套件通过真实 HTTP/SSE（Server-Sent Events）适配器，验证准确的请求次数、带编号的重试步骤、请求体完全一致、失败的部分分片不会泄漏、语义空结果恢复、截断流分类、超时恢复、监听器延迟启动后从真实连接遭拒中恢复，以及有界重试耗尽。
...
**随服务器一起修改默认重试策略**：不予采纳。服务器用于揭示既有语义，而非决定策略。截断流由提供方适配器分类，重试插件继续执行配置的失败码策略。
