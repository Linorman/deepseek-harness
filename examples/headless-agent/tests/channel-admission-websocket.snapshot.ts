...
const configPath = join(fixtures, 'headless-channel-admission-websocket.cordis.yml')
const driver = 'headless-channel-admission-websocket-driver.ts'
const expected = join(import.meta.dirname, 'snapshots/channel-admission-websocket/output.expected.json')
...
describe('WebSocket channel admission through the headless Loader', () => {
  for (const backend of ['json', 'sqlite'] as const) {
    it(`blocks dispatch until actual endpoint consent and then delivers with Session receipts on ${backend}`, async () => {
      ...
      const root = await mkdtemp(join(repository, '.tmp/channel-admission-websocket-loader-'))
      try {
