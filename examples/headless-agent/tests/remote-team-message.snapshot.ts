...
const configPath = join(fixtures, 'headless-remote-team-message.cordis.yml')
const driver = 'headless-remote-team-message-driver.ts'
const expected = join(import.meta.dirname, 'snapshots/remote-team-message/output.expected.json')
...
async function inspect(root: string, kind: Backend) {
  ...
  try {
    ...
    try {
      ...
      const [subset, toolBroadcast, broadcast] = envelopes
      expect(subset?.audience).toHaveLength(1)
      expect(object(subset?.payload).content).toEqual([{ type: 'text', text: 'PRIVATE_SUBSET' }])
      expect(toolBroadcast?.audience).toBeNull()
      expect(object(toolBroadcast?.payload).content).toEqual([{ type: 'text', text: 'REMOTE_BROADCAST' }])
      ...
      for (const [participantId, binding] of bindings) {
        ...
        if (received.length > 0) {
          ...
          expect(JSON.stringify(log.events).includes('PRIVATE_SUBSET')).toBe(received.length === 3)
...
describe('remote Team text tools through the headless Loader', () => {
  for (const backend of ['json', 'sqlite'] as const) {
    it(`blocks dispatch until actual endpoint consent and then delivers with Session receipts on ${backend}`, async () => {
      ...
      const root = await mkdtemp(join(repository, '.tmp/remote-team-message-loader-'))
      try {
        const target = join(root, '.clocky/profiles/headless/snapshot-fixtures')
        await mkdir(target, { recursive: true })
        await symlink(join(repository, 'examples/node_modules'), join(target, 'node_modules'), process.platform === 'win32' ? 'junction' : 'dir')
