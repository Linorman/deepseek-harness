...
async function run(ctx: Context) {
  ...
  const remoteParticipantId = slow.id
  ...
  try {
    ...
    return { remoteInvitation: 'websocket-v6', admission: ['pending', 'acknowledged', 'active'], pendingDispatch: 'rejected', adapter: 'direct-v4', members: 3, explicitToolMessages: 1, broadcastMessages: 1,
