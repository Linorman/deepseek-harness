...
class MessageModel extends LlmAdapter {
  ...
  async *stream(options: GenerateOptions): AsyncIterable<StreamChunk> {
    if (options.purpose !== 'session-title') {
      this.requests += 1
      if (this.target !== undefined && this.requests === 1) {
        const id = CallId('direct-v4-subset')
        const args = JSON.stringify({ channel_id: this.target.channelId, text: 'PRIVATE_SUBSET',
          audience: [this.target.recipientId], delivery: 'context' })
...
async function run(ctx: Context) {
  ...
  try {
    ...
    assert.equal(subsetRows.length, 1, 'the real team_message tool must append one Envelope')
    ...
    // All v4 endpoints must acknowledge before the channel becomes active; a
    // context delivery receipts the connected slow endpoint without waking it.
    assert.equal((await records(ctx, channelId)).filter(record => record.type === 'channel/receipt'
      && record.participantId === slow.id).length, 1)
    assert.equal(slowModel.requests, 0)
    assert.equal(incoming(fastAgent).length, 2)
    assert.equal(incoming(slowAgent).length, 1)
    ...
    assert.equal(finalRecords.filter(record => record.type === 'channel/envelope').length, 2)
    assert.equal(finalRecords.filter(record => record.type === 'channel/receipt').length, 3)
    return { adapter: 'direct-v4', members: 3, explicitToolMessages: 1, broadcastMessages: 1,
      fastSources: 2, delayedSources: 1, receipts: 3, orderedContent: ['text', 'image', 'text'],
...
export const name = 'direct-v4-driver'
/** Real message, attachment and activation owners required by this scenario. */
export const inject = ['teams', 'llm', 'sessions', 'teamLinks', 'teamActivations', 'attachments']
