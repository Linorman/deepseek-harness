...
import { codingHarness, hasRealModel, realModel, TODO_SYSTEM_PROMPT, waitForIdle } from './harness.ts'
...
describe.skipIf(!hasRealModel)('todo_write: real model records a plan', () => {
  it('appends a todo/write event with the model-produced task list', async () => {
    workdir = await mkdtemp(join(tmpdir(), 'clocky-todo-write-e2e-'))
    ctx = await codingHarness(workdir, { persona: TODO_SYSTEM_PROMPT })
    const agent = ctx.agentLoop.create(SessionId('e2e-todo'), realModel)
