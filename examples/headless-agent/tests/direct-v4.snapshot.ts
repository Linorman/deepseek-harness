...
const configPath = join(fixtures, 'headless-direct-v4.cordis.yml')
const driver = 'headless-direct-v4-driver.ts'
const expected = join(import.meta.dirname, 'snapshots/direct-v4/output.expected.json')
...
describe('direct v4 through the headless Loader', () => {
  for (const backend of ['json', 'sqlite'] as const) {
    it(`delivers tool subsets and delayed broadcasts with real Session receipts on ${backend}`, async () => {
      await mkdir(join(repository, '.tmp'), { recursive: true })
      const root = await mkdtemp(join(repository, '.tmp/direct-v4-loader-'))
      try {
        ...
        throw new Error(`Direct v4 Loader evidence: ${evidence}`, { cause: error })
