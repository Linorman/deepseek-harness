/** A real read-only filesystem denial creates a Host approval wait and a durable cancellation on JSON and SQLite. */
...
const driver = 'headless-human-approval-driver.ts'
const expected = join(import.meta.dirname, 'snapshots/human-approval/producer.expected.json')
...
describe('pending human approval through the formal Host gateway', () => {
  for (const backend of ['json', 'sqlite'] as const) {
    it(`admits one real worker approval and retains public cancellation on ${backend}`, async () => {
      await mkdir(join(repository, '.tmp'), { recursive: true })
      const root = await mkdtemp(join(repository, '.tmp/ha-'))
      ...
      try {
        ...
        const launch = resolveExampleLaunch({ srcBin: join(repository, 'apps/cli/src/bin.ts'),
          ...
          env: {
            ...
            CLOCKY_APPROVAL_BACKEND: backend, CLOCKY_PERMISSION_MODE: 'read-only', CLOCKY_TELEMETRY_DISABLED: '1', CLOCKY_SNAPSHOT: 'replay', TSX_DISABLE_CACHE: '1',
        ...
        throw new Error(`Human approval failure evidence: ${evidence}`, { cause: error })
