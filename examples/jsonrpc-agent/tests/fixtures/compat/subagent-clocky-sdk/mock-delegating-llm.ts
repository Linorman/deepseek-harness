...
class MockDelegatingAdapter extends LlmAdapter {
  async * stream(options: GenerateOptions): AsyncIterable<StreamChunk> {
    ...
    if (toolResultText.length === 0) {
      ...
      yield { type: 'tool-call-delta', index: 0, id: CallId('call-delegate'), name: 'legacy_subagent', argumentsDelta: args }
      yield { type: 'block-end', index: 0, block: { type: 'tool-call', id: CallId('call-delegate'), name: 'legacy_subagent', arguments: args } }
