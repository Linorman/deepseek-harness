...
The public package family belongs to the `@clocky` scope and uses the `node-addon-landlock-run` package prefix; platform packages append platform information only:

```text
@clocky/node-addon-landlock-run
@clocky/node-addon-landlock-run-<platform>
```
